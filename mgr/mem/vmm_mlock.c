...
static int do_lock(OBJECT * obp, off64_t off, struct pa_quantum *pq, unsigned num, void *d)
{
    do {
...
int ms_lock(ADDRESS * adp, struct map_set *ms)
{
    struct mm_map *mm;
...
int vmm_mlock(PROCESS * prp, uintptr_t vaddr, size_t len, int flags)
{
    ADDRESS *adp;
    int r;
    struct map_set ms;
...
static int lock_pulse(message_context_t * ctp, int code, unsigned _flags, void *handle)
{
    pid_t pid;
    pid_t aspace_pid;
    int tid;
