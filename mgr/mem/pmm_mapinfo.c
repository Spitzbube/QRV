...
static int
pmm_check(uintptr_t vaddr, struct mem_phys_entry *mem, unsigned memaddr,
          struct mem_phys_entry **closest, unsigned *addr)
{
    if (vaddr >= memaddr) {
            ...
            size_t *contigp)
{
    ...
    if (!mem && (!closest || (!(closest->flags & MAP_ELF) && closest->reloc == 0))) {
        ...
        if (text_tmp.size && pmm_check(vaddr, &text_tmp, prp->kdebug.text_addr, &closest, &addr)) {
            ...
            if (data_tmp.size
                && pmm_check(vaddr, &data_tmp, prp->kdebug.data_addr, &closest, &addr)) {
                mem = &data_tmp;
            }
        }
    }
