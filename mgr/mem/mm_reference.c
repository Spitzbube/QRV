...
static int pq_purge_cache(void *dst, size_t len, void *d)
{
    CPU_CACHE_CONTROL(((struct data *) d)->mm->obj_ref->adp, dst, len,
                      MS_INVALIDATE | MS_INVALIDATE_ICACHE);
    return EOK;
}
...
condition_cache(struct mm_map *mm, struct pa_quantum *pq, unsigned num, struct data *data)
{
    ...
    if (mm->mmap_flags & PROT_NOCACHE) {
        ...
        if (mm_flags & MM_FLAG_MULTIPLE_DCACHE_LEVELS) {
            ...
            r = pte_temp_map(mm->obj_ref->adp, data->va, pq, mm, num << QUANTUM_BITS,
                             pq_purge_cache, data);
            mm->mmap_flags |= PROT_NOCACHE;
            if (r != EOK)
                return r;
    ...
    if (mm->mmap_flags & PROT_NOCACHE) {
        mm->mmap_flags &= ~PROT_NOCACHE;
        r = pte_temp_map(mm->obj_ref->adp, data->va, pq, mm, num << QUANTUM_BITS, pq_purge_cache,
                         data);
        mm->mmap_flags |= PROT_NOCACHE;
        if (r != EOK)
            return r;
...
static int pq_init_run(void *dst, size_t len, void *d)
{
    struct data *data = d;
    struct mm_map *mm;
    struct pa_quantum *pq;
...
static int pq_init(OBJECT * obp, off64_t off, struct pa_quantum *pq, unsigned num, void *d)
{
    struct data *data = d;
    int r;
    struct mm_map *mm;
...
#define INIT_MPW_FLAGS	(MPW_HOLES|MPW_SYSRAM)
    ...
    for (;;) {
        ...
        for (;;) {
            if (num == 0)
                return EOK;
            if (INIT_REQUIRED(mm->mmap_flags, pq))
                break;
        ...
        if (r != EOK)
            return r;
        r = pte_temp_map(mm->obj_ref->adp, data->va, data->pq, mm, i << QUANTUM_BITS, pq_init_run,
                         d);
        if (r != EOK)
            return r;
        num -= i;
...
static int pq_map(OBJECT * obp, off64_t off, struct pa_quantum *pq, unsigned num, void *d)
{
    struct data *data = d;
    struct mm_map *mm;
    ...
    if (pq->blk == PAQ_BLK_FAKE) {
        ...
        if (r == EOK) {
            ...
            if (!(mmap_flags & PROT_NOCACHE) && (mm->mmap_flags & PROT_NOCACHE)) {
                ...
                CPU_CACHE_CONTROL(or->adp, (void *) start, data->va - start,
                                  MS_INVALIDATE | MS_INVALIDATE_ICACHE);
                r = pte_map(or->adp, start, data->va - 1, mmap_flags | PROT_NOCACHE, or->obp, paddr,
                            0);
            }
        }
    } else {
        ...
        for (;;) {
            ...
            for (;;) {
                ...
                if (!(mmap_flags & PROT_EXEC) && data->icache_check
                    && (pq->flags & PAQ_FLAG_HAS_INSTRS)) {
                    pq->flags &= ~PAQ_FLAG_HAS_INSTRS;
...
static int pq_copy_run(void *dst, size_t len, void *d)
{
    struct data *data = d;
    struct mm_map *mm;
...
static int pq_copy(OBJECT * obp, off64_t off, struct pa_quantum *pq, unsigned num, void *d)
{
    struct data *data = d;
    struct mm_map *mm;
    int r;
...
static int privatize(struct data *data)
{
    struct map_set ms;
...
#if 0
    new->mmap_flags =
        (mm->mmap_flags & ~(MAP_PHYS | MAP_ELF | MAP_NOSYNCFILE)) | MAP_SYSRAM | MAP_ANON;
#else
                 ...
                 unsigned flags, struct map_set *ms)
{
    ...
    switch (obp->hdr.type) {
    case OBJECT_MEM_SHARED:
        if (obp->mem.mm.flags & MM_SHMEM_IFS)
            break;
        // fall through
    case OBJECT_MEM_FD:
    case OBJECT_MEM_TYPED:
