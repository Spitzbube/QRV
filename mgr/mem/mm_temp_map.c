...
pte_temp_map(ADDRESS * adp, uintptr_t user_va, struct pa_quantum *pq,
             struct mm_map *mm, size_t len, int (*func)(void *, size_t, void *), void *d)
{
    paddr_t paddr;
    uintptr_t end;
    int r;
...
#define MAX_TEMP_MAPPING (64*1024)
    if (piece > MAX_TEMP_MAPPING)
        piece = MAX_TEMP_MAPPING;
    ...
    if (piece <= adp->tmap_size &&
        ((adp->tmap_base >= (user_va + len)) || ((adp->tmap_base + adp->tmap_size) <= user_va))) {
