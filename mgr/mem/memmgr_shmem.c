...
int shmem_create(OBJECT * obp, void *d)
{
    mem_map_t *msg = d;
    int r;
...
size_t shmem_name(OBJECT * obp, size_t max, char *dest)
{
    return pathmgr_object_pathname(obp, max, dest);
}


int shmem_devctl(resmgr_context_t * ctp, io_devctl_t * msg, OBJECT * obp)
{
    union {
        unsigned ptr;
    ...
    case DCMD_MEMMGR_MEMOBJ:
        ...
        if ((obp->mem.mm.flags & MM_SHMEM_SPECIAL) || (obp->mem.mm.pmem != NULL)) {
            ...
            r = -1;
                   ...
                   && ((data->memobj.flags & (SHMCTL_ANON | SHMCTL_PHYS)) == SHMCTL_PHYS)) {
            // Should check permissions on /dev/mem, but this more
            // restrictive "must be root" check will do for now..
            r = -1;
        } else {
            memobj_lock(obp);
            if ((obp->mem.mm.flags & (SHMCTL_PHYS | SHMCTL_GLOBAL | SHMCTL_HAS_SPECIAL))
                && (info.cred.euid != 0)) {
                r = -1;
            } else {
                ...
                if (obp->mem.mm.flags & SHMCTL_ANON) {
                    if ((obp->mem.mm.flags & (SHMCTL_PHYS | SHMCTL_LAZY)) ==
                        (SHMCTL_PHYS | SHMCTL_LAZY)) {
                ...
                } else if ((obp->mem.mm.flags & SHMCTL_PHYS)
                           && !(data->memobj.size & (memmgr.pagesize - 1))
                           && !(data->memobj.offset & (memmgr.pagesize - 1))) {
