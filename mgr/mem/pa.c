...
static void pa_memclass_init(memclass_attr_t * attr, const char *sysmem_name,
...
static memclass_attr_t sysram_attr = {
    ...
    STRUCT_FLD(limits)
        { STRUCT_FLD(alloc)
         { STRUCT_FLD(size) { STRUCT_FLD(min) __PAGESIZE, STRUCT_FLD(max) memsize_t_INFINITY}} },
};
...
#ifdef PA_DEBUG
void dump_q_list(struct pa_quantum *qp)
{
    struct pa_quantum *np;

    if (!qp)
        kprintf("Run is EMPTY!\n");
    while (qp != NULL) {
        np = qp->u.inuse.next;
        kprintf("Run %08x r/f/b/p %d/%08x/%d/%08x\n",
                (unsigned) qp, qp->run, qp->flags, qp[qp->run - 1].run, qp->u.inuse.qpos);
        qp = np;
    }
}

void debug_dump_blkheadrun(struct block_head *bh)
{
...
void debug_verify_blk(struct block_head *bh)
{
    unsigned i;
...
static void *early_alloc(struct sysram_list *sl, unsigned size)
{
    unsigned max = 0;
    paddr_t max_size = 0;
    paddr_t curr;
...
#error code does not support CPU_SYSTEM_PADDR_START != 0
    ...
    for (i = 0; i < sl->count; ++i) {
        if (sl->list[i].start < CPU_SYSTEM_PADDR_END) {
            ...
            if (end > CPU_SYSTEM_PADDR_END)
                end = CPU_SYSTEM_PADDR_END;
...
static int find_last_paddr(struct asinfo_entry *as, char *name, void *data)
{
    paddr64_t end;
...
static int make_sysram_list(struct asinfo_entry *as, char *name, void *d)
{
    ...
    if (start_addr <= last_paddr) {
        ...
        if (se.start < se.end) {
            ...
            for (;;) {
                if (i == 0)
                    break;
                if (data->list[i - 1].start > se.start)
                    break;
...
static struct pa_quantum *dequeue_run(struct block_head *bh, struct pa_quantum *rtf)
{
...
static void enqueue_run(struct block_head *bh, struct pa_quantum *rtq)
{
...
static unsigned
init_block(struct block_head **bhp, unsigned blk_idx, struct sysram_entry *se,
...
struct pa_quantum *pa_getrunhead(struct pa_quantum *pq)
{
    struct pa_quantum *curr = pq;

    if (IS_BACKQ(curr->run))
        return (curr + curr->run);
    if (IS_HEADQ(curr->run))
        return (curr);
...
static paddr_t pa_largest_pow2(paddr_t size)
{
    paddr_t max;
    ...
    for (
            ...
            max =
            ((size >> 32) & 0xffffffffU) ? ((paddr_t) 0x1 << (_PADDR_BITS - 1)) : ((paddr_t) 0x1 <<
                                                                                   32);
#else
...
static void kerext_pa_alloc(void *d)
{
...
//}
    ...
    //the same time.
    if (mem_reserved_size < resv_size)
        goto fail;              // must be first
    if (mem_free_size < (size - resv_size))
        goto fail;
    ...
    for (curr_res = kap->restriction; curr_res; curr_res = curr_res->next) {
        ...
        for (curr_size = (flags & PAA_FLAG_CONTIG) ? size_left : pa_largest_pow2(size_left);
            ...
            if (curr_size > size_left)
                continue;       /* skip over sizes that aren't applicable */
            ...
            for (;;) {
                ...
                for (;;) {      /* manually control the loop */
                    ...
                    CRASHCHECK((found_run < qp)
                               || ((found_run + LEN_TO_NQUANTUM(curr_size)) > qp + qp->run));
                    ...
                    *tail = found_run;
                    ...
                    if (size_left < curr_size)
                        break;  /* try the next size */
...
struct pa_quantum *pa_alloc(paddr_t size, paddr_t align, unsigned colour, unsigned flags,
                            unsigned *status_p, struct pa_restrict *restriction, paddr_t resv_size)
{
    ...
    for (;;) {
        if (restriction->start < sysram_end)
            break;
    ...
    if (size == QUANTUM_SIZE)
        flags |= PAA_FLAG_CONTIG;   /* because it is contig by definition */
    ...
    for (;;) {
...
#ifndef NDEBUG
        {
            ...
            for (p = data.alloc; p; p = p->u.inuse.next) {
                if (!(p->flags & PAQ_FLAG_INUSE))
                    crash();
                for (i = 0; i < p->run; ++i) {
                    if (p[i].
                        flags & (PAQ_FLAG_INITIALIZED | PAQ_FLAG_HAS_SYNC | PAQ_FLAG_LOCKED |
                                 PAQ_FLAG_MODIFIED)) {
                        crash();
                    }
                }
        ...
        if (!purger_invoke(size)) {
            if (!
                ((flags & PAA_FLAG_CONTIG) && (data.state & PAS_VPS_ALIGN)
                 && (size > QUANTUM_SIZE))) {
        ...
        for (p = data.alloc; p; p = p->u.flink.next) {
            if (p == data.alloc)
                kprintf("%s ALLOC 0x%x for %d [free=0x%x]\n", (p == data.alloc) ? "PMEM" : "    ",
                        (uintptr_t) p, p->run, (unsigned) mem_free_size);
        }
    }
#endif
...
static void kerext_pa_alloc_given(void *data)
{
...
int pa_alloc_given(struct pa_quantum *pq, unsigned num, unsigned *status_p)
{
    unsigned tmp_status;
...
static void kerext_pa_free(void *data)
{
...
void pa_free_list(struct pa_quantum *qp, paddr_t resv_size)
{
    struct pa_quantum *np;
...
void pa_free_fake(struct pa_quantum *pq)
{
    if (pq->blk != PAQ_BLK_FAKE)
        crash();
...
static void kerext_pa_free_info(void *data)
{
...
void pa_free_info(struct pa_restrict *restriction, paddr_t * totalp, paddr_t * contigp)
{
    paddr_t total;
    paddr_t contig;
    paddr_t start;
...
int pa_sysram_overlap(struct pa_restrict *res)
{
    struct block_head *bh;
    struct block_head **bhp;
...
static int pmem_add_cleanup(message_context_t * ctp, int code, unsigned flags, void *handle)
{
    ...
    for (;;) {
        ...
        if ((SIGEV_GET_TYPE(&info.event) == SIGEV_PULSE)
            ...
            && (info.event.sigev_value.sival_ptr == bhp))
            break;
        timerid = timerid + 1;
    }
...
static INLINE void load_info(memclass_info_t * info)
{
    ...
    info->size.unreserved.used = sysram_attr.size   // total system ram
        - (info->size.reserved.used + (info->size.unreserved.free + info->size.reserved.free));
}
 ...
 *
*/
static memsize_t pa_reserve(memsize_t size, memsize_t adjust, int adjust_dir,
 ...
 * FIX ME -lock
*/
static memsize_t pa_unreserve(memsize_t size, memsize_t adjust, int adjust_dir,
                              memclass_info_t * info)
{
    memsize_t r;
    CRASHCHECK(info == NULL);
...
static memclass_sizeinfo_t *pa_size_info(memclass_sizeinfo_t * s, memclass_info_t * info)
{
    ...
    if (sysram_attr.size <
        ...
        kprintf("total: %P, reserved.used: %P, total_unallocated: %P\n", sysram_attr.size,
                (paddr_t) info->size.reserved.used,
