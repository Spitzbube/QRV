...
int memmgr_map(resmgr_context_t * ctp, tProcess * prp, mem_map_t * msg)
{
  ...
  check_again:
...
#if defined(VARIANT_instr)
    {
        ...
        SETIOV(iov + iovcnt, &pid, sizeof(pid));
        iovcnt++;
        SETIOV(iov + iovcnt, &msg->i.addr, sizeof(msg->i.addr) + sizeof(msg->i.len));
        iovcnt++;
        SETIOV(iov + iovcnt, &msg->i.flags, sizeof(msg->i.flags));
        iovcnt++;
        //Wide mode only
        SETIOV(iov + iovcnt, &msg->i.prot, sizeof(msg->i.prot));
        iovcnt++;
        SETIOV(iov + iovcnt, &msg->i.fd, sizeof(msg->i.fd));
        iovcnt++;
        SETIOV(iov + iovcnt, &msg->i.align, sizeof(msg->i.align) + sizeof(msg->i.offset));
        iovcnt++;
        //@@@ This never works for imagefs files
        if (object != NULL && object->hdr.type == OBJECT_MEM_FD && object->fdmem.name) {
            SETIOV(iov + iovcnt, object->fdmem.name, strlen(object->fdmem.name));
            iovcnt++;
        }
