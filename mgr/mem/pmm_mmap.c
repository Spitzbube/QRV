    ...
    if ((flags & MAP_TYPE) == MAP_PRIVATE) {
        ...
        if (flags & MAP_NOX64K) {
            if (len > (32 << 10)) {
                ...
                if (((uintptr_t) (mem + 1) & ((64 << 10) - 1)) <=
                    (((uintptr_t) (mem + 1) + len) & ((64 << 10) - 1))) {
        ...
        case MAP_SHARED:
            if ((flags & MAP_FIXED) && addr != off) {
                if ((flags & (MAP_ELF | MAP_SYSRAM)) != (MAP_ELF | MAP_SYSRAM)
                    || prp->pid != SYSMGR_PID) {
                    return ENXIO;
                }
            }
            *vaddr = (void *) off;
            break;
        case MAP_PRIVATE:{
                if ((flags & MAP_FIXED) && addr >= user_addr && addr + len - 1 <= user_addr_end
                    && addr != off) {
