...
int vmm_fault(struct fault_info *info)
{
    ADDRESS *adp;
    struct map_set ms;
    struct mm_map *mm;
        ...
        if (map_fault_lock(&adp->map)) {
            if (!(adp->flags & MM_ASFLAG_PRIVATIZING)) {
                ...
                if (mm == NULL) {
                    ...
                    if (!(mm->mmap_flags & (PROT_READ | PROT_EXEC)))
                        r = -1;
                }
                map_coalese(&ms);
            }
...
static int fault_pulse(message_context_t * ctp, int code, unsigned _flags, void *handle)
{
    pid_t pid;
    pid_t aspace_pid;
    int tid;
    ...
    if (!(mm->mmap_flags & MAP_LAZY)) {
        ...
        case OBJECT_MEM_SHARED:
            ...
            if (obp->mem.mm.flags & SHMCTL_LAZY)
                break;
            // fall through
