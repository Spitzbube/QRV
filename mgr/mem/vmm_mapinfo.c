...
static int pmem_info(OBJECT * obp, off64_t off, struct pa_quantum *pq, unsigned num, void *d)
{
        ...
        if (data->mip != NULL) {
            for (i = 0; i < num; ++i, ++pq) {
                ...
                if (pq->flags & PAQ_FLAG_INITIALIZED)
                    curr_flags |= PG_HWMAPPED | PG_REFERENCED;
                if (pq->flags & PAQ_FLAG_MODIFIED)
                    curr_flags |= PG_MODIFIED;
#ifdef FULL_PG_FLAGS
                if (i == 0)
                    flags = curr_flags;
                if (flags != curr_flags)
...
*/
    ...
    for (;;) {
        ...
        if (or != NULL)
            break;
        if (mm->mmap_flags & MAP_STACK)
            break;              // Need to report guard pages
        map_coalese(&ms);
        chk_vaddr = mm->end + 1;
    }
    if (vaddr < mm->start) {
    ...
    if (or != NULL) {
        ...
        if ((mm->mmap_flags & MAP_ELF) && (mm->mmap_flags & PROT_WRITE)) {
            ...
            for (;;) {
                ...
                if (pms.first == prev_mm)
                    break;
                prev_mm = pms.first;
                if (prev_mm == NULL)
                    break;
                if (prev_mm->obj_ref == NULL)
                    break;
                if (!(prev_mm->mmap_flags & MAP_ELF))
                    break;
        ...
        if (mdp != NULL) {
            ...
            if ((nobp->hdr.type == OBJECT_MEM_SHARED) && (nobp->mem.mm.flags & MM_SHMEM_IFS)) {
                ...
                if ((name_mm->extra_flags & EXTRA_FLAG_LOADER) && (name_mm->mmap_flags & MAP_ELF)
                    && (prp->debug_name != NULL)) {
