...
int fdmem_create(OBJECT * obp, void *extra)
{
...
size_t fdmem_name(OBJECT * obp, size_t max, char *dest)
{
    char *name;
...
io_mmap(struct _msg_info *info, int fd, resmgr_context_t * ctp, mem_map_t * mmapp, unsigned prot)
{
    ...
    if ((msg.i.info.nd == ~0U) || (MsgSendnc(fd, &msg.i, sizeof msg.i, &msg.o, sizeof msg.o) == -1)) {
        return -1;
    }
    return msg.o.coid;
}
...
static int mem_map_fd(resmgr_context_t * ctp, PROCESS * prp, mem_map_t * mmapp, OBJECT ** object)
{
    int fd, error;
    int mmap_fd;
    OBJECT *obp;
    ...
    if ((fd = ConnectAttach(info.nd, info.pid, info.chid, 0, 0)) == -1) {
        ...
        MMAPFD_WAKEUP_CLOSER return error;
    }
    ...
    if (obp == NULL) {
        ...
        if (obp == NULL) {
            ...
            if (fd != mmap_fd) {
                pthread_mutex_unlock(&fdmem_mutex);
                atomic_sub(&fdmem_open_lock, 1);
                MMAPFD_WAKEUP_CLOSER return ENOMEM;
            }
            ...
            if ((obp = vector_lookup(&memmgr_fd_vector, mmap_fd)) == NULL) {
                ...
                atomic_add(&fdmem_close_lock, 1);
                pthread_mutex_unlock(&fdmem_mutex);
                MMAPFD_WAKEUP_CLOSER close(mmap_fd);
                atomic_sub(&fdmem_close_lock, 1);
                MMAPFD_WAKEUP_OPENER return ENOMEM;
            }
        }
    }
    ...
    MMAPFD_WAKEUP_CLOSER * object = obp;

    return (0);
}


int memmgr_open_fd(resmgr_context_t * ctp, PROCESS * prp, mem_map_t * mmapp, OBJECT ** obpp)
{
    int fd;
    struct stat64 sbuf;
    int again = 1;
...
// This code might not be needed. I leave it here in case.
    ...
    if (sbuf.st_mode & S_ISVTX)
        obp->mem.mm.flags |= MM_FDMEM_STICKY;
...
static int memmgr_fd_cleanup(message_context_t * ctp, int code, unsigned flags, void *handle)
{
    OBJECT **owner;
    OBJECT *obp;
    int force;
...
#define TIME_INFINITY	(~(time_t)0)
    ...
    if (proc_thread_pool_reserve() == 0) {
        ...
        for (;;) {
            ...
            if (obp == ZOMBIE_END)
                break;
            if (mem_free_size < MIN_MEM_FREE_SIZE)
                force = 1;
            if (force) {
    ...
    MMAPFD_WAKEUP_OPENER if (fdmem_zombies != ZOMBIE_END) {
        struct _itimer value;

        // There are fd's pending to be cleaned
        currtime = time(0);
        if (nexttime == TIME_INFINITY)
            nexttime = currtime;
        if (nexttime <= currtime)
            nexttime = currtime + STALE_CLEAN_INTERVAL;
        value.nsec = nexttime * (_Uint64t) 1000000000, value.interval_nsec = 0;
        if (TimerSettime(timerid, TIMER_ABSTIME, &value, NULL) != 0) {
            kprintf
                ("\nCannot arm timer (%d). Memory mapped file timed stale cleaning may not work.",
                 errno);
        }
    }
...
void memmgr_fd_init(void)
{
    ...
    if ((timerid = TimerCreate(CLOCK_REALTIME, &event)) == -1) {
        kprintf("\nCannot create timer (%d). Memory mapped file timed stale cleaning may not work.",
                errno);
    }
}
