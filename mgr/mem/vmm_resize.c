...
static int clear_bad_pages(struct mm_object_ref *or, struct mm_map *mm, void *d)
{
...
static int shrink_ref(struct mm_object_ref *or, struct mm_map *mm, void *d)
{
...
static int do_zero(OBJECT * obp, off64_t off, struct pa_quantum *pq, unsigned num, void *d)
{
...
static int zero_tail(OBJECT * obp, size_t size)
{
    pid_t old;
    size_t pg_off;
    int r;
...
int vmm_resize(OBJECT * obp, size_t size)
{
    uint64_t newsize;
    uint64_t oldsize;
    unsigned flags;
    ...
    if (newsize > oldsize) {
        ...
        if (vaddr != VA_INVALID) {
            ...
            if (obp->mem.mm.flags & SHMCTL_GLOBAL) {
                if (pte_unmap
                    (NULL, vaddr + (uintptr_t) newsize, vaddr + (uintptr_t) (oldsize - newsize) - 1,
                     obp) != EOK) {
                    crash();
                }
            }
    ...
    if ((newsize == 0) && (obp->mem.mm.pmem != NULL))
        crash();
    if (r != EOK)
        goto fail2;
