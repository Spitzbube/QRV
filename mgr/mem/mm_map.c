...
static struct mm_map_internal *map_alloc(void)
{
...
static void rdecl map_write_lock(struct mm_map_head *mh)
{
#ifndef NDEBUG
    unsigned count = 0;
#endif
...
int map_fault_lock(struct mm_map_head *mh)
{
#if defined(VARIANT_smp)
    unsigned l;
...
void map_fault_unlock(struct mm_map_head *mh)
{
#if defined(VARIANT_smp)
...
int map_isolate(struct map_set *ms, struct mm_map_head *mh, uintptr_t start, size_t size, int flags)
{
...
uintptr_t
map_find_va(struct mm_map_head *mh, uintptr_t va, uintptr_t size, uintptr_t mask, unsigned flags)
{
    ...
    for (;;) {
        ...
        if (hole_start < hole_end) {
            if (size < (hole_end - hole_start)) {
                if (flags & MAP_BELOW) {
                    if ((hole_start > va) && (start != VA_INVALID))
                        break;
           ...
           uintptr_t va, uintptr_t size, uintptr_t mask, unsigned flags)
{
    ...
    if ((ADDR_OFFSET(va) != 0) || (ADDR_OFFSET(size) != 0))
        crash();
...
int map_split(struct map_set *ms, size_t size)
{
    uintptr_t point;
    struct mm_map *mm;
    struct mm_map_internal *new;
    ...
    for (;;) {
        if (mm == NULL)
            crash();
        if ((point >= mm->start) && (point < mm->end))
            break;
        mm = mm->next;
    }
    new = map_alloc();
...
int map_destroy(struct map_set *ms)
{
...
static int rdecl coalese_one(struct mm_map_head *mh, struct mm_map *mm)
{
    ...
    if (next == NULL)
        return 0;
    if (mm->last_page_bss != 0)
        return 0;
    if (mm->obj_ref != next->map.obj_ref)
        return 0;
    if ((mm->end + 1) != next->map.start)
        return 0;
    if ((mm->offset + ((mm->end - mm->start) + 1)) != next->map.offset)
        return 0;
    if (mm->mmap_flags != next->map.mmap_flags)
        return 0;
    if (mm->extra_flags != next->map.extra_flags)
        return 0;
...
int map_coalese(struct map_set *ms)
{
    struct mm_map *mm;
    struct mm_map *prev;
    struct mm_map *next;
...
void map_set_update(struct map_set *ms, struct mm_map *old, struct mm_map *new)
{
    if (ms->first == old) {
...
int map_remove(struct map_set *ms)
{
    struct mm_map **owner;
    ...
    rover = ms->head->rover;
    if (rover != NULL &&
        (rover_start = rover->start) >= ms->first->start && rover_start <= ms->last->start) {
...
int map_init(struct mm_map_head *mh, uintptr_t start, uintptr_t end)
{
    mh->head = NULL;
    mh->rover = NULL;
    mh->lock = 0;
...
int map_fini(struct mm_map_head *mh)
{
    struct mm_map *first;
    struct mm_map *last;
