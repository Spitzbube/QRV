...
static int fiddle_lock(OBJECT * obp, off64_t off, struct pa_quantum *pq, unsigned num, void *d)
{
...
static int relock(struct mm_object_ref *or, struct mm_map *mm, void *d)
{
    struct lock_data *data = d;
    off64_t start;
    off64_t end;
    ...
    if (mm->extra_flags & EXTRA_FLAG_LOCK) {
        ...
        if (start < end) {
            r = memobj_pmem_walk(MPW_SYSRAM, mm->obj_ref->obp, start, end, fiddle_lock, data);
            if (r != EOK)
                return r;
        }
    }
...
int vmm_munlock(PROCESS * prp, uintptr_t vaddr, size_t len, int flags)
{
    ADDRESS *adp;
    int r;
    struct map_set ms;
