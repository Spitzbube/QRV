...
int cpu_vmm_fault(struct fault_info *info)
{
    pxe_t *ptep;
    pxe_t *pdep;
    uintptr_t vaddr;
...
int cpu_vmm_mcreate(PROCESS * prp)
{
    void *vaddr;
    unsigned pdir_size;
    ...
    if (prp->pid == PROCMGR_PID) {
        ...
        pq = pa_alloc(pdir_size, __PAGESIZE, PAQ_COLOUR_NONE, PAA_FLAG_CONTIG, NULL, restrict_4G,
                      resv);
        if (pq == NULL) {
...
#define PDIR_GLOBAL(a) (void *)((uintptr_t)(a) + pdir_user_boundry)
    memcpy(PDIR_GLOBAL(adp->cpu.pgdir), PDIR_GLOBAL(pgtbl_list->pgdir),
           pdir_size - pdir_user_boundry);
    if (pae_enabled) {
        unsigned i;
...
void cpu_vmm_mdestroy(PROCESS * prp)
{
    ADDRESS *adp;
    ...
    flushtlb();
    (void) vmm_munmap(NULL, (uintptr_t) adp->cpu.pgdir, pae_enabled ? __PAGESIZE * 4 : __PAGESIZE,
                      0, mpid);
}
...
int cpu_pte_split(uintptr_t vaddr, struct mm_pte_manipulate *data)
{
    pxe_t *pdep;
    uintptr_t pde_size;
    uint64_t pde;
...
//kprintf("splitting big page at 0x%x\n", check);
    ...
    if ((check - 1) > data->split_end)
        data->split_end = check - 1;
...
int cpu_pte_merge(struct mm_pte_manipulate *data)
{
    uintptr_t pde_size;
    uintptr_t check;
    ADDRESS *adp;
...
//kprintf("merging 0x%x into 0x%x pagesize\n", check, pde_size);
                    ...
                    if ((prev_pte & PTE_CHECK_BITS) != (curr_pte & PTE_CHECK_BITS))
                        break;
...
int cpu_pte_manipulate(struct mm_pte_manipulate *data)
{
    pxe_t *ptep;
    pxe_t *pdep;
    pxe_t *prev_pdep;
...
#define PMF_CR0_FLIPPED 0x0040
    ...
    if (op & PTE_OP_BAD) {
        ...
        bits = X86_PTE_USER1;
        if (data->prot & (PROT_READ | PROT_EXEC | PROT_WRITE))
            bits |= X86_PTE_PRESENT;
        if (data->prot & PROT_WRITE)
            bits |= X86_PTE_WRITE;
        if (data->prot & PROT_NOCACHE)
            bits |= X86_PTE_CD | X86_PTE_WT;
    ...
    if (data->shmem_flags & SHMCTL_LAZYWRITE) {
        if (op & PTE_OP_MAP) {
            ...
            // the page table.
            if (cpu_vmm_vaddrinfo
                (adp ? object_from_data(adp, address_cookie) : NULL, curr, &paddr,
                 NULL) != PROT_NONE) {
    ...
    for (;;) {
        ...
        if (pdep != prev_pdep) {
            ...
            if (PXE_GET_FLAGS(pdep) & X86_PTE_PRESENT) {
                ...
                pq = pa_alloc(__PAGESIZE, __PAGESIZE, PAQ_COLOUR_NONE, 0, &pa_status, restrict_proc,
                              resv);
                if (pq == NULL) {
...
unsigned cpu_vmm_vaddrinfo(PROCESS * prp, uintptr_t vaddr, paddr_t * paddrp, size_t *lenp)
{
    uint64_t pde;
    pxe_t *pgdir;
    pxe_t *kmap_pde;
