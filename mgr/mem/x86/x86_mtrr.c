    ...
    if (nvmtrr) {
        ...
        for (i = 0; i < nvmtrr; i++, _mtrr++) {
            ...
            if (_mtrr->phys_mask & X86_MTRR_PHYSMASK_VALID) {
                _mtrr->flags =
                    MTRR_VALID | MTRR_BIOS_SET | (_mtrr->phys_base & MTRR_DEFAULT_TYPE_MASK);
...
static int _find_free_mtrr()
{
    int i;

    for (i = 0; i < nvmtrr; i++) {
        if (!mtrr_array[i].flags)
            return i;
    }
...
static int _find_overlapping_mtrr(paddr_t start, unsigned size)
{
    int i;
    uint64_t base;
    uint64_t len;
...
static void _write_mtrr(int index, struct _mtrr *_mtrr)
{
...
int x86_set_mtrr(paddr_t start, unsigned size, unsigned flags, unsigned op)
{
    ...
    if (flags & SHMCTL_LAZYWRITE) {
        ...
        if ((old = _find_overlapping_mtrr(start, size)) == -1 && setting
            && ((index = _find_free_mtrr()) != -1)) {
            ...
            _mtrr->phys_mask =
                (~(paddr_t) (size - 1) & X86_MTRR_PHYSMASK_RANGE_MASK) | X86_MTRR_PHYSMASK_VALID;
            ...
            if ((mtrr_array[old].refcnt == 1) && mtrr_array[old].flags & MTRR_BIOS_SET) {
                mtrr_array[old].phys_base =
                    (mtrr_array[old].phys_base & X86_MTRR_PHYSBASE_BASE_MASK) | (mtrr_array[old].
                                                                                 flags &
                                                                                 MTRR_DEFAULT_TYPE_MASK);
