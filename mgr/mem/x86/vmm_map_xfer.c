...
int
vmm_map_xfer(PROCESS * actprp, PROCESS * prp, IOV ** piov, int *pparts, int *poff, IOV * pniov,
             int *pnparts, unsigned flags)
{
    ...
    *((volatile uintptr_t *) &slot->base0) = 0;
    ...
    if ((flags & MAPADDR_FLAGS_IOVKERNEL) == 0) {
        ...
        if ((uintptr_t) iov > last
            || (!(flags & MAPADDR_FLAGS_SYSPRP) && !WITHIN_BOUNDRY((uintptr_t) iov, last, bound))) {
            return -1;
        }
#endif
...
// check if the whole iov are all valid. If not, return error.
    ...
    if (addr > last
        || (!(flags & MAPADDR_FLAGS_SYSPRP) && !WITHIN_BOUNDRY((uintptr_t) addr, last, bound))) {
        if (size != 0) {
            return -1;
        }
    }
    ...
    do {
        ...
        if (len >= size) {
            ...
            last = (uintptr_t) addr + size - 1;
            if (addr > last
                || (!(flags & MAPADDR_FLAGS_SYSPRP)
                    && !WITHIN_BOUNDRY((uintptr_t) addr, last, bound))) {
                if (size != 0) {
                    return -1;
                }
            }
    ...
    *poff = GETIOVLEN(iov) - size;
    if (((flags & MAPADDR_FLAGS_IOVKERNEL) == 0) && ((unsigned) iov >= MAP_BASE)
        && ((unsigned) iov < (MAP_BASE + MAP_SIZE))) {
