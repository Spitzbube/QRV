...
void vmm_init_mem(int phase)
{
    ...
    if ((__cpu_flags & X86_CPU_PSE)
        && (PXE_GET_FLAGS(VTOPDIRP((uintptr_t) & __exc7emul)) & X86_PDE_PS)) {
        unsigned vaddr;
        paddr_t paddr;

        vaddr = ((_syspage_ptr->un.x86.pgdir[0] + pde_mask) & ~pde_mask) - __PAGESIZE;
        paddr =
            (PXE_GET(VTOPDIRP((uintptr_t) & __exc7emul)) & ~(uint64_t) pde_mask) +
            ((uintptr_t) & __exc7emul & pde_mask);
    ...
    if ((__cpu_flags & X86_CPU_PSE)
        && !(PXE_GET_FLAGS(VTOPDIRP(CPU_1TO1_VADDR_BIAS)) & X86_PTE_PRESENT)) {
    ...
    if (__cpu_flags & X86_CPU_PSE) {
        ...
        pde_map =
            (uintptr_t) ((_syspage_ptr->un.x86.pgdir[0] + pde_mask) & ~pde_mask) - 2 * __PAGESIZE;
