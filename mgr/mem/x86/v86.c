...
static void v86_map(void *on)
{
    ...
    for (addr = 0x00000000; addr < (1 << 20) + (64 << 10); addr += __PAGESIZE) {
        ptep = VTOPTEP(addr);
        if ((PXE_GET_FLAGS(ptep) & (X86_PTE_PRESENT | X86_PTE_WRITE)) ==
...
int v86_handle(resmgr_context_t * ctp, x86_cpu_v86_t * msg)
{
    ...
    SETIOV(ctp->iov + 1,
           (uint8_t *) _syspage_ptr->un.x86.real_addr + offsetof(struct _v86_memory, reg),
    ...
    if (msg->i.swi & _V86_OPTION_CALLDIRECT) {
        ...
        patch =
            (long *) ((uint8_t *) _syspage_ptr->un.x86.real_addr +
                      offsetof(struct _v86_memory, rvecs[(unsigned char) (msg->i.swi & 0xFF)]));
