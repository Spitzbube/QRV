...
static int do_manipulation(struct pte_manip *manip)
{
    int r;
...
static void ker_manipulate(void *data)
{
    KerextLock();
...
static int pte_manipulate(struct pte_manip *manip)
{
    int r;

    if (ADDR_OFFSET(manip->data.start) != 0)
        crash();
    if (ADDR_OFFSET(manip->data.end + 1) != 0)
        crash();
    if ((manip->data.op & PTE_OP_MAP) && (ADDR_OFFSET(manip->data.paddr) != 0))
        crash();
        ...
        OBJECT * obp, paddr_t paddr, unsigned extra)
{
    ...
    if ((obp != NULL)
        ...
        if (obp->shmem.special != 0)
            obp->mem.mm.flags |= SHMCTL_HAS_SPECIAL;
...
int pte_prot(ADDRESS * adp, uintptr_t start, uintptr_t end, int flags, OBJECT * obp)
{
    struct pte_manip manip;
...
int pte_unmap(ADDRESS * adp, uintptr_t start, uintptr_t end, OBJECT * obp)
{
    struct pte_manip manip;
...
int pte_prealloc(ADDRESS * adp, uintptr_t start, uintptr_t end)
{
    struct pte_manip manip;
...
int pte_bad(ADDRESS * adp, uintptr_t vaddr)
{
    struct pte_manip manip;
