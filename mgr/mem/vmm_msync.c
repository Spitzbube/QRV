...
static int pte_off(struct mm_object_ref *or, struct mm_map *mm, void *d)
{
...
static int write_page(struct sync_data *data, void *buff)
{
...
static int do_write(void *dst, size_t len, void *d)
{
    struct sync_data *data = d;
...
static int pmem_off(OBJECT * obp, off64_t off, struct pa_quantum *pq, unsigned num, void *d)
{
    struct sync_data *data = d;
    struct mm_map *mm;
    uintptr_t vaddr;
    ...
    for (i = 0; i < num; ++i) {
        ...
        if (data->adp_flags & MM_ASFLAG_ISR_LOCK)
            bits = 0;
        pq->flags &= ~bits;
        ++pq;
        off += QUANTUM_SIZE;
...
int mm_sync(OBJECT * obp, struct mm_map *mm, int flags)
{
...
static void one_cache(struct cache_data *cache, uintptr_t start, uintptr_t next)
{
...
static int check_cache(OBJECT * obp, off64_t off, struct pa_quantum *pq, unsigned num, void *d)
{
...
int vmm_msync(PROCESS * prp, uintptr_t vaddr, size_t len, int flags)
{
    ADDRESS *as;
    int r;
    struct map_set ms;
    ...
    if (cache.start != VA_INVALID) {
        CPU_CACHE_CONTROL(as, (void *) cache.start, cache.next - cache.start, cache.flags);
    }
            ...
            if (or != NULL) {
                ...
                if ((flags & MS_INVALIDATE) ||
                    ...
                    if ((obp->hdr.type == OBJECT_MEM_FD) &&
                        ...
                        int sync_flags =
                            (mm->
                             mmap_flags & MAP_NOSYNCFILE) ? flags & ~(MS_SYNC | MS_ASYNC) : flags;
