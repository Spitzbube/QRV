...
static void ext_vaddrinfo(void *data)
{
    struct args_vaddrinfo *args = data;

    KerextLock();
    KerextStatus(NULL, cpu_vmm_vaddrinfo(args->prp, args->vaddr, &args->paddr, &args->len));
}

static int get_paddr(OBJECT * obp, off64_t off, struct pa_quantum *pq, unsigned num, void *d)
{
        ...
        if (data->init) {
            ...
            for (;;) {
                if (!(pq->flags & PAQ_FLAG_INITIALIZED)) {
                    (void) memory_reference(&data->mm, vaddr, vaddr | (QUANTUM_SIZE - 1), MR_NONE,
                                            NULL);
                }
                if (pq->run < 0)
                    break;
