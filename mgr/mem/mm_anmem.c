...
int anmem_create(OBJECT * obp, void *extra)
{
    ADDRESS *adp = extra;
...
size_t anmem_name(OBJECT * obp, size_t max, char *name)
{
    return 0;
}


off64_t anmem_offset(OBJECT * obp, uintptr_t va, size_t size)
{
    ...
    if (end > obp->mem.mm.size)
        obp->mem.mm.size = end;
    return va;
}


void anmem_unoffset(OBJECT * obp, off64_t off, size_t size)
{
    VERIFY_OBJ_LOCK(obp);
