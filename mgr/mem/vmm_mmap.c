...
int gbl_vaddr_unmap(uintptr_t vaddr, size_t len)
{
    struct map_set ms;
    int r;
...
static int tymem_valid_offset(off64_t off, size_t size, struct pa_restrict *list)
{
...
static int tymem_fakes(OBJECT * obp, off64_t off, struct pa_quantum *pq, unsigned num, void *d)
{
...
static int tymem_split(OBJECT * obp, off64_t off, struct pa_quantum *pq, unsigned num, void *d)
{
    struct tymem_pmem *data = d;
    unsigned end_quanta;
                 ...
                 unsigned flags, off64_t boff, size_t size)
{
    ...
    if (flags & (MAP_LAZY | MAP_BELOW16M))
        return EINVAL;
    pg_offset = ADDR_OFFSET(boff);
    boff -= pg_offset;
    size += pg_offset;
    if (flags & (IMAP_TYMEM_ALLOCATE | IMAP_TYMEM_ALLOCATE_CONTIG)) {
        if ((boff != 0) || (pg_offset != 0))
            return EINVAL;
        pa_flags = 0;
        if (flags & IMAP_TYMEM_ALLOCATE_CONTIG)
            pa_flags = PAA_FLAG_CONTIG;
        ...
        if (obp->mem.mm.flags & MM_MEM_RDB) {
            ...
            pq = pa_alloc(size, 0, PAQ_COLOUR_NONE, pa_flags, NULL, obp->mem.mm.restriction, resv);
            if (pq == NULL) {
        ...
        // pmem entries.
        r = memobj_pmem_walk(MPW_HOLES, obp, boff, boff + size - 1, tymem_fakes, &data);
        if (r != EOK)
            return r;
        pq = data.pq;
        mm->obj_ref = (void *) pq;
...
static int rdecl setup_mappings(PROCESS * prp, struct map_set *ms, unsigned flags, unsigned preload)
{
    ADDRESS *adp;
    size_t maxsize;
    size_t todo;
         ...
         unsigned preload, int fd, void **vaddrp, size_t *sizep, part_id_t mpart_id)
{
    ...
    if (prp == NULL) {
        ...
        if (mpart_id == part_id_t_INVALID) {
            ...
            if (!((sys_mempart == NULL) && (procnto_prp == NULL)))
                crash();
        }
#endif                          /* NDEBUG */

        if (flags & MAP_STACK)
            size_requested -= guardsize;
        if (flags & MAP_ANON) {
            //
        ...
        *vaddrp = (uint8_t *) vaddr + pg_offset;
        if (pq != NULL) {
            if (vaddr < system_heap_vaddr.lo)
                system_heap_vaddr.lo = vaddr;
            vaddr += size_requested - 1;
            if (vaddr > system_heap_vaddr.hi)
                system_heap_vaddr.hi = vaddr;
        }
        return EOK;
    }
    ...
    if ((obp != NULL)
        ...
        case OBJECT_MEM_SHARED:
            ...
            vaddr = ROUNDUP(vaddr_requested + (unsigned) (obp->mem.mm.size - boff), __PAGESIZE);
            if (vaddr < end_vaddr) {
    ...
    if (flags & MAP_FIXED) {
        ...
        if (pg_offset != addr_offset) {
            ...
            if (obp == NULL) {
                ...
                if ((flags & MAP_PHYS) && !(flags & MAP_ANON))
                    goto fail1;
    ...
    create_flags = flags;
    if (extra_flags & EXTRA_FLAG_GBL_VADDR)
        create_flags |= IMAP_GLOBAL;
#ifndef STACK_GROWS_UP
    if (create_flags & MAP_STACK)
        create_flags |= MAP_BELOW;
#endif
    if ((mask == 0) && (vaddr_requested == 0) && (mm_flags & MM_FLAG_VPS)) {
        ...
        case VPS_HIGHUSAGE:
            ...
            if ((obp == NULL)
                || (obp->hdr.type != OBJECT_MEM_SHARED)
                || !(obp->mem.mm.flags & SHMCTL_HIGHUSAGE))
                break;
            // Fall through
        case VPS_AUTO:
    ...
    if (obp->hdr.type == OBJECT_MEM_TYPED) {
        ...
        if ((obp->mem.mm.flags & (MM_SHMEM_SPECIAL | SHMCTL_ANON | SHMCTL_PHYS)) !=
            (MM_SHMEM_SPECIAL | SHMCTL_PHYS)) {
            flags |= MAP_SYSRAM;
        }
    }
    ...
    if (preload == 0) {
        ...
        if (flags & MAP_STACK) {
            ...
            case OBJECT_MEM_FD:
                ...
                if (((flags & (PROT_WRITE | MAP_ELF | MAP_TYPE)) ==
                     (PROT_WRITE | MAP_ELF | MAP_PRIVATE))) {
