           ...
           const char *name, unsigned name_len)
{
    ...
    if ((p == &name[1]) && (as->owner == AS_NULL_OFF))
        return 1;
...
static void kill_restrict(struct pa_restrict *par)
{
...
static int one_restrict_create(const char *name, unsigned name_len, struct pa_restrict **rp)
{
    ...
    while (num != 0) {
        if (match_name(base_as, as, name, name_len)) {
            ...
            for (;;) {
                ...
                if (try_as->priority < as->priority)
                    break;
                owner = &try->next;
            }
...
static int multi_restrict_create(const char *name, OBJECT * obp, memclass_attr_t * attr)
{
    int is_sysram;
    struct pa_restrict *try;
    struct pa_restrict *new;
    ...
    if (!is_sysram)
        obp->mem.mm.flags |= MM_MEM_RDB;
    obp->mem.mm.restriction = par;
    return EOK;
}


int tymem_create(OBJECT * obp, void *extra)
{
    return shmem_create(obp, extra);
}


int tymem_done(OBJECT * obp)
{
    if (obp->mem.mm.refs != NULL)
        return 0;
...
*/
int tymem_register(const char *name)
{
...
int memmgr_tymem_open(const char *name, int oflags, OBJECT ** obpp, PROCESS * prp)
{
    OBJECT *obp;
    unsigned name_len;
    int r;
...
struct pa_quantum *tymem_rdb_alloc(OBJECT * obp, size_t size, unsigned flags)
{
    rsrc_request_t req;
    int r;
...
struct pa_quantum *tymem_rdb_alloc_given(OBJECT * obp, paddr_t paddr, size_t size, int *rp)
{
    rsrc_request_t req;
    int r;
    struct pa_quantum *pq;
...
void tymem_rdb_free(OBJECT * obp, paddr_t paddr, size_t size)
{
    rsrc_request_t req;
    int ret;
    ...
    if (ret == EOK) {
        ...
        /* this lock won't work if the kernel ever uses non sysram memory for any internal objects */
        CRASHCHECK(KerextAmInKernel());
        if (pthread_mutex_lock(&e->lock.mutex) != EOK)
            crash();
...
static void tymem_rdb_free_info(struct pa_restrict *rlp, paddr_t * totalp, paddr_t * contigp)
{
...
void tymem_free_info(OBJECT * obp, paddr_t * total, paddr_t * contig)
{
