...
static int mempart_proc_associate(PROCESS * prp, part_id_t mempart_id, mempart_dcmd_flags_t flags,
                                  ext_lockfncs_t * lf);
static int mempart_getlist(PROCESS * prp, part_list_t * mpart_list, int n, mempart_flags_t flags,
...
mempart_t *mempart_lookup_mpid(part_id_t mpid)
{
    ...
    if (!KerextAmInKernel()) {
        mpart =
            QueryObject(_QUERY_MEMORY_PARTITION, MPINDEX(mpid), 0 /*_QUERY_MEMORY_PARTITION*/ , 0,
                        0, 0, 0);
    } else {
...
part_id_t mempart_vec_add(mempart_t * mpart)
{
    ...
    if (data.mpid != part_id_t_INVALID)
        data.mpid |= (mpid_unique | parttype_MEM);
...
mempart_t *mempart_vec_del(part_id_t mpid)
{
    ...
    if ((data.mpid = MPINDEX(mpid)) == part_id_t_INVALID)
        return NULL;
