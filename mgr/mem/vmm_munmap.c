...
static int clean_pmem(OBJECT * obp, off64_t off, struct pa_quantum *pq, unsigned num, void *d)
{
    ...
    if (pq->blk == PAQ_BLK_FAKE) {
        ...
        do {
            ...
            if (pq_flags & PAQ_FLAG_HAS_SYNC) {
                ...
                sync_off = PADDR_TO_SYNC_OFF(paddr);
                MemobjDestroyed(PADDR_TO_SYNC_OBJ(paddr),
                                sync_off, sync_off + (QUANTUM_SIZE - 1), prp, (void *) vaddr);
            }
            vaddr += QUANTUM_SIZE;
            paddr += QUANTUM_SIZE;
            ++pq;
...
int check_last_ref(struct mm_object_ref *or, struct mm_map *mm, void *d)
{
    struct unmap_data *data = d;
    off64_t mm_start;
    off64_t mm_end;
    ...
    if (mm_start > data->end)
        return EOK;
    if (mm_end < data->start)
        return EOK;
    if (mm_start < data->end)
        data->end = mm_start;
    if (mm_end > data->start)
...
int ms_unmap(ADDRESS * adp, struct map_set *ms, int unmap_flags)
{
    struct mm_map *mm;
...
int vmm_munmap(PROCESS * prp, uintptr_t vaddr, size_t len, int flags, part_id_t mpart_id)
{
    int r;
    struct map_set ms;
    ADDRESS *adp;
