...
int memobj_pmem_del_len(OBJECT * obp, off64_t start, uint64_t len)
{
...
considered....
**************************************/
int memobj_pmem_del_end(OBJECT * obp, off64_t start, off64_t end)
{
            ...
            if (pq->blk == PAQ_BLK_FAKE) {
                ...
                if (pq->flags & PAQ_FLAG_MODIFIED) {
                    MemobjDestroyed(PADDR_TO_SYNC_OBJ(paddr),
                                    off, off + (num << QUANTUM_BITS) - 1, NULL, NULL);
                }
                ...
                for (i = 0; i < num; ++i, ++tmp) {
                    if (tmp->flags & PAQ_FLAG_HAS_SYNC) {
                        ...
                        off = PADDR_TO_SYNC_OFF(paddr);
                        MemobjDestroyed(PADDR_TO_SYNC_OBJ(paddr),
                                        off, off + (QUANTUM_SIZE - 1), NULL, NULL);
                    }
                    paddr += QUANTUM_SIZE;
                }
...
static struct pa_quantum *pmem_alloc(OBJECT * obp, unsigned numq, void *d)
{
...
int memobj_pmem_add(OBJECT * obp, off64_t off, size_t size, int flags)
{
    ...
    data.flags = 0;
    if (flags & MAP_PHYS)
        data.flags |= PAA_FLAG_CONTIG;
    if (flags & MAP_NOX64K)
        data.flags |= PAA_FLAG_NOX64K;
...
static struct pa_quantum *pmem_pq(OBJECT * obp, unsigned numq, void *d)
{
    struct add_data *data = d;
...
int memobj_pmem_add_pq(OBJECT * obp, off64_t off, struct pa_quantum *pq)
{
    struct add_data data;
...
int memobj_pmem_split(OBJECT * obp, struct pa_quantum *pq, unsigned split_quanta)
{
    struct pa_quantum *new;
    unsigned remaining;
        ...
        if (qnext > qstart) {
            ...
            if (pq->blk != PAQ_BLK_FAKE) {
                if (flags & MPW_SYSRAM) {
                    ++invoked;
                    start = max(qpos, qstart);
                    r = func(obp, NQUANTUM_TO_LEN(start), pq + (start - qpos), (end - start) + 1,
                             d);
                }
...
int memobj_offset_check(OBJECT * obp, off64_t off, uint64_t len)
{
    ...
    if (end < off)
        return ENXIO;
    if (end > ((off64_t) ((uint32_t) ~ 0) << QUANTUM_BITS))
        return ENXIO;
    return EOK;
}
