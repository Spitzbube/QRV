...
static int data_dup_run(void *src, size_t len, void *d)
{
...
static int data_dup(OBJECT * obp, off64_t off, struct pa_quantum *pq, unsigned num, void *d)
{
    struct dup_data *data = d;
    struct mm_map *mm;
    ...
    for (;;) {
        ...
        for (;;) {
            ++run;
            if (!(pq[run].flags & PAQ_FLAG_MODIFIED))
                break;
            if (run == num)
                break;
        }
        ...
        if (r != EOK)
            return r;
        r = pte_temp_map(data->adp, start, pq, data->pmm, (end - start) + 1, data_dup_run, data);
        or = mm->obj_ref;
...
int vmm_dup(PROCESS * pprp, PROCESS * prp)
{
    ADDRESS *padp;
    struct map_set pms;
    struct mm_map *pmm;
    ...
    for (;;) {
        ...
        if (!(pmm->extra_flags & EXTRA_FLAG_NOINHERIT)) {
            ...
            if (por != NULL) {
                ...
                pobp = por->obp;
                memobj_lock(pobp);
                if ((pobp->hdr.type == OBJECT_MEM_ANON)
                    && ((pmm->mmap_flags & MAP_TYPE) == MAP_PRIVATE)) {
                    anon = adp->anon;
                    memobj_lock(anon);
            ...
            if (anon != NULL) {
                ...
                r = memobj_pmem_walk(MPW_SYSRAM, pobp, pmm->offset, pmm->offset + len - 1, data_dup,
                                     &data);
                if (r != EOK)
                    goto fail2;
                memobj_unlock(anon);
