...
void memref_walk_restart(struct mm_map_head *mh)
{
    atomic_add(&mh->walk_gen, 1);
}


int memref_add(struct mm_map *mm, OBJECT * obp, ADDRESS * adp, int fd)
{
...
void memref_del(struct mm_map *del_mm)
{
    struct mm_map *mm;
    struct mm_object_ref *or;
...
int memref_walk(OBJECT * obp, int (*func)(struct mm_object_ref *, struct mm_map *, void *), void *d)
{
    struct mm_object_ref *or;
    struct mm_map *mm;
    int r;
