...
static int fill_schedpart_list(pid_t ppid, part_list_t ** schedpart_list,
...
static int get_schedpartids(const posix_spawnattr_t * _Restrict attrp, unsigned *num,
...
int procmgr_pspawn(resmgr_context_t * ctp, void *vmsg)
{
     ...
     */
    if (ctp->info.srcmsglen > ctp->info.msglen)
        return EMSGSIZE;
    ...
    case _PROC_SPAWN_START:
        {
            ...
            if ((r = parse_pspawn_msg(ctp->info.pid, msg, &spawn_msg, &partlist)) == EOK) {
                unsigned spawn_msg_len = sizeof(*spawn_msg)
                    ...
                    + spawn_msg->i.pathlen + spawn_msg->i.nfds * sizeof(int32_t)
                    + spawn_msg->i.nbytes;
                            ...
                            proc_create_attr_t * partlist)
{
    ...
    *spawn_msg = NULL;
    ...
    if (msg->i.filact_bytes > 0) {
         ...
         */
        if (file_open_actions_fixup(_factp, open_action) != EOK)
            return EINVAL;
    ...
    p = (uint8_t *) ((unsigned) (msg + 1) + msg->i.attr_bytes + msg->i.filact_bytes +
                     msg->i.filact_obytes);
    ...
    case 0:                    // MEMPART_DFLT_INHERIT_BEHVR_1
        {
            ...
            if (*part_list != NULL) {
                CRASHCHECK((*part_list)->num_entries < num_parts);
            }

            /* caller didn't supply the memory so allocate it */
            if ((*part_list == NULL)
                && ((*part_list = malloc(PART_LIST_T_SIZE(num_parts))) == NULL)) {
                return ENOMEM;
            }
    ...
    case 1:                    // MEMPART_DFLT_INHERIT_BEHVR_2
        {
            ...
            /* caller supplied the memory. Is it enough ? */
            if (*part_list != NULL) {
                CRASHCHECK((*part_list)->num_entries < 1);
            }
    ...
    if (num_entries == 0) {
        ...
        for (i = 0; i < mplist->num_entries; i++) {
            ...
            if (mempart_get_classid(mplist->i[i].id) == sys_memclass_id) {
                sysram_list_idx = i;
                if ((mplist->i[i].flags & (mempart_flags_HEAP_CREATE | mempart_flags_HEAP_SHARE)) ==
                    0)
...
static int calc_fdcnt(posix_spawn_file_actions_t * factp, unsigned *fdcnt)
{
    ...
    num_fact = 0;
    if ((r = posix_spawn_file_getactions(factp, &num_fact, NULL)) != EOK)
        return r;
    CRASHCHECK(num_fact > 0);
    num_fact = -num_fact;
    if ((fact_list = alloca(num_fact * sizeof(*fact_list))) == NULL)
        return ENOMEM;
    if ((r = posix_spawn_file_getactions(factp, &num_fact, fact_list)) != EOK)
        return r;
    CRASHCHECK(num_fact < 0);
...
*/
static int getpartids(const posix_spawnattr_t * _Restrict attrp, unsigned *num,
                      part_list_t ** partlist, parttype_t type)
{
    int r;
    ...
    if ((r = posix_spawnattr_getpartid(attrp, &num_ids, NULL)) != EOK)
        return r;
    if (num_ids == 0) {
        *num = 0;
        return EOK;
    ...
    if ((*partlist = calloc(1, PART_LIST_T_SIZE(num_ids))) == NULL)
        return ENOMEM;
    if ((all_ids = alloca(num_ids * sizeof(*all_ids))) == NULL)
        return ENOMEM;
        ...
        if ((r = posix_spawn_file_getactions(fact_p, &num_fact, fact_list)) != EOK)
            ...
            for (i = 0; i < num_fact; i++) {
                ...
                case posix_file_action_type_CLOSE:
                    ...
                    break;
                case posix_file_action_type_DUP:
                    kprintf("fd=%d, new_fd=%d\n", fact_list[i]._type.dup.fd,
                            fact_list[i]._type.dup.new_fd);
                    break;
                case posix_file_action_type_OPEN:
                    kprintf("new_fd=%d, path=%s, oflags=0x%x, omode=0x%x\n",
                            fact_list[i]._type.open->new_fd, fact_list[i]._type.open->path,
                            fact_list[i]._type.open->flags, fact_list[i]._type.open->mode);
                    break;
                default:
                    kprintf("Unknown\n");
                    break;
