...
static int get_fdinfo(pid_t pid, int nd, int base, int fdinfo_max,
                      struct _proc_spawn_fd_info *pfdinfo, int nfds, int *pfd, int flags)
{
    ...
    if (nfds != 0) {
        ...
        while (ConnectServerInfo(pid, base, &sinfo) != -1 && !(sinfo.coid & _NTO_SIDE_CHANNEL)) {
            if (!
                ((sinfo.flags & _NTO_COF_DEAD)
                 || ((sinfo.flags & _NTO_COF_CLOEXEC) && (flags & _PROC_SPAWN_FD_NOCLOEXC)))) {
...
int procmgr_spawn(resmgr_context_t * ctp, void *vmsg, proc_create_attr_t * partlist)
{
    ...
    if (partlist->mpart_list == NULL) {
        ...
        if ((num_parts < 0)
            || ((partlist->mpart_list = alloca(PART_LIST_T_SIZE(num_parts))) == NULL)) {
            proc_unlock(pprp);
    ...
    if (SCHEDPART_INSTALLED() && (partlist->spart_list == NULL)) {
        ...
        if ((num_parts < 0)
            || ((partlist->spart_list = alloca(PART_LIST_T_SIZE(num_parts))) == NULL)) {
            proc_unlock(pprp);
            /* if no partitions returned, process likely has not got proper permissions */
            return (num_parts < 0) ? EACCES : ENOMEM;
        }
    ...
    case _PROC_SPAWN_START:
        {
            ...
            off = msgsize = sizeof(struct _proc_spawn)
                ...
                + msg->spawn.i.pathlen + msg->spawn.i.nfds * sizeof(int32_t)
                + msg->spawn.i.nbytes;
            ...
            if (!
                (lcp =
                 procmgr_context_alloc(msgsize,
                                       LC_SPAWN | (((ctp->info.flags & _NTO_MI_ENDIAN_DIFF)) ?
                                                   LC_CROSS_ENDIAN : 0)))) {
                return ENOMEM;
            }
            ...
            for (sig = _SIGMIN; sig <= _SIGMAX; sig++) {
                if ((lcp->msg.spawn.i.parms.flags & SPAWN_SETSIGIGN)
                    && sigismember(&lcp->msg.spawn.i.parms.sigignore, sig)) {
    ...
    case _PROC_SPAWN_FD:{
            ...
            max =
                min(max,
                    (ctp->msg_max_size -
                     ((uintptr_t) ctp->iov[1].iov_base -
                      (uintptr_t) & msg->spawn_fd)) / sizeof(struct _proc_spawn_fd_info));
            nfds =
                get_fdinfo(msg->spawn_fd.i.ppid, ctp->info.nd, msg->spawn_fd.i.base, max,
                           ctp->iov[1].iov_base, nfds,
                           (int *) ((char *) &msg->spawn_fd + sizeof msg->spawn_fd.i),
                           msg->spawn_fd.i.flags);
            if (nfds == -1) {
                return errno;
            }
    ...
    case _PROC_SPAWN_REMOTE:{
            ...
            nfds =
                get_fdinfo(ctp->info.pid, parms->nd, 0, max, (struct _proc_spawn_fd_info *) ptr,
                           msg->spawn.i.nfds,
                           (int *) ((char *) &msg->spawn.i + sizeof msg->spawn.i), 0);
            if (nfds == -1) {
                _sfree(buff, size);
                return errno;
            }
    ...
    case _PROC_SPAWN_ARGS:
        if (ctp->info.nd == ND_LOCAL_NODE
            && (lcp = ProcessStartup(ctp->info.pid, 0)) != (void *) -1) {
