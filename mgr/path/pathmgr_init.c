...
static int pathmgr_resolve_handler(resmgr_context_t * ctp, void *msg)
{
    ...
    case PATHMGR_HANDLE:
        ...
        node =
            pathmgr_resolve_path((connect->path[0] == '/') ? prp->root : prp->cwd, connect,
                                 connect->path, prp->root);
        proc_unlock(prp);
    ...
    case _PATH_CHROOT:
        ...
        tmpnode =
            pathmgr_resolve_path((connect->path[0] == '/') ? prp->root : prp->cwd, connect,
                                 connect->path, prp->root);
        node =
            pathmgr_node_lookup(tmpnode, connect->path,
...
void pathmgr_init(void)
{
    ...
    if (resmgr_attach(dpp, &rattr, "", _FTYPE_ANY, PATHMGR_FLAG_OPAQUE | PATHMGR_FLAG_DIR, 0, 0, 0)
        != PATHMGR_HANDLE_REMOTE) {
        crash();
    }
    ...
    iofunc_func_init(_RESMGR_CONNECT_NFUNCS, &pathmgr_connect_funcs, _RESMGR_IO_NFUNCS,
                     &pathmgr_io_funcs);
    ...
    link_root_id =
        resmgr_attach(dpp, &rattr, "", _FTYPE_ALL,
                      PATHMGR_FLAG_OPAQUE | PATHMGR_FLAG_DIR | PATHMGR_FLAG_AFTER,
 ...
 */
int pathmgr_link(const char *path, uint32_t nd, pid_t pid, int chid, unsigned handle,
