...
static int mem_read(resmgr_context_t * ctp, io_read_t * msg, void *vocb)
{
    return mem_read_write(ctp, (resmgr_iomsgs_t *) msg, (struct mem_ocb *) vocb);
}


static int mem_write(resmgr_context_t * ctp, io_write_t * msg, void *vocb)
{
    return mem_read_write(ctp, (resmgr_iomsgs_t *) msg, (struct mem_ocb *) vocb);
}


static int mem_close_ocb(resmgr_context_t * ctp, void *reserved, void *vocb)
{
...
static int mem_lseek(resmgr_context_t * ctp, io_lseek_t * msg, void *vocb)
{
    struct mem_ocb *ocb = vocb;
    unsigned pos;
    unsigned size;
...
static int mem_stat(resmgr_context_t * ctp, io_stat_t * msg, void *vocb)
{
    ...
    msg->o.st_ctime = msg->o.st_mtime = msg->o.st_atime = (obp ? obp->mem.pm.mtime : time(0));
...
static int mem_devctl(resmgr_context_t * ctp, io_devctl_t * msg, void *vocb)
{
    ...
    case DCMD_ALL_GETFLAGS:
        data->oflag =
            (ocb != NULL) ? (ocb->ioflag & ~O_ACCMODE) | ((ocb->ioflag - 1) & O_ACCMODE) : O_RDWR;
...
static int mem_space(resmgr_context_t * ctp, io_space_t * msg, void *vocb)
{
    struct mem_ocb *ocb = vocb;
    unsigned start, end;
    tPathMgrObject *obj;
...
static int mem_change_attr(resmgr_context_t * ctp, resmgr_iomsgs_t * msg, void *vocb)
{
    iofunc_ocb_t nocb;
    iofunc_attr_t attr;
    struct mem_ocb *ocb = vocb;
...
static int mem_chown(resmgr_context_t * ctp, io_chown_t * msg, void *vocb)
{
    return mem_change_attr(ctp, (resmgr_iomsgs_t *) msg, vocb);
}


static int mem_chmod(resmgr_context_t * ctp, io_chmod_t * msg, void *vocb)
{
    return mem_change_attr(ctp, (resmgr_iomsgs_t *) msg, vocb);
}
...
int mem_open(resmgr_context_t * ctp, io_open_t * msg, void *handle, void *extra)
{
    ...
    if (object) {
        ...
        if (msg->connect.ioflag & O_CREAT && !object->mem.pm.mode) {
            ...
            object->mem.pm.mode =
                ((msg->connect.file_type ==
                  _FTYPE_SHMEM) ? S_IFNAM : S_IFREG) | (msg->connect.mode & ~S_IFMT);
...
static int shmem_open(resmgr_context_t * ctp, io_open_t * msg, void *handle, void *extra)
{
    ...
    msg->ftype_reply.file_type =
        (msg->connect.file_type == _FTYPE_SHMEM) ? _FTYPE_SHMEM : _FTYPE_FILE;
...
#define ALL_TMEM	(POSIX_TYPED_MEM_ALLOCATE|POSIX_TYPED_MEM_ALLOCATE_CONTIG|POSIX_TYPED_MEM_MAP_ALLOCATABLE)
    ...
    count = 0;
    if (tflags & POSIX_TYPED_MEM_ALLOCATE)
        ++count;
    if (tflags & POSIX_TYPED_MEM_ALLOCATE_CONTIG)
        ++count;
    if (tflags & POSIX_TYPED_MEM_MAP_ALLOCATABLE)
        ++count;
    ...
    errno =
        memmgr_tymem_open(msg->connect.path, ocb->tflags, &ocb->object,
                          proc_lookup_pid(ctp->info.pid));
    if (errno != EOK)
        goto fail2;
    ...
    ctp->id = mem_handle;
    if (resmgr_open_bind(ctp, ocb, &tymem_io_funcs) == -1)
        goto fail3;
...
void devmem_init(void)
{
    ...
    resmgr_attach(dpp, &rattr, "/dev/shmem", 0, _RESMGR_FLAG_DIR, &shmem_connect_funcs,
                  &mem_io_funcs, 0);
    //RUSH3: Decide if "/dev/tymem" should be a fully usable directory
    //RUSH3: structure
    resmgr_attach(dpp, NULL, "/dev/tymem", _FTYPE_TYMEM, _RESMGR_FLAG_DIR, &tymem_connect_funcs,
                  &tymem_io_funcs, 0);
...
int devmem_check_ocb_phys(resmgr_context_t * ctp, void *vocb)
{
    struct mem_ocb *ocb = vocb;
