...
static int complete_open(resmgr_context_t * ctp, struct open_entry *oep, int ret)
{
    NODE *nop;
...
int pathmgr_open(resmgr_context_t * ctp, io_open_t * msg, void *handle, void *extra)
{
    ...
    nop = pathmgr_node_lookup(0, msg->connect.path,
                              ...
                              PATHMGR_LOOKUP_NOSERVER | PATHMGR_LOOKUP_ATTACH |
                              PATHMGR_LOOKUP_NOEMPTIES |
                              ...
                              ((ctp->id == link_root_id) ? PATHMGR_LOOKUP_NOAUTO : 0), &result);
            ...
            case OBJECT_PROC_SYMLINK:
                if (result[0] == '\0') {
                    if (msg->connect.subtype == _IO_CONNECT_READLINK) {
                        return complete_open(ctp, oep,
                                             reply_symlink(ctp, 0, &msg->link_reply, &o->symlink, 0,
                                                           0));
                    }
                ...
                return complete_open(ctp, oep,
                                     reply_symlink(ctp, msg->connect.eflag, &msg->link_reply,
                                                   &o->symlink, msg->connect.path, result));
        ...
        case _IO_CONNECT_UNLINK:
            ...
            case OBJECT_SERVER:
                if (msg->connect.eflag & _IO_CONNECT_EFLAG_DIR) {
                    return complete_open(ctp, oep,
                                         (o->server.hdr.
                                          flags & PATHMGR_FLAG_DIR) ? ENOSYS : ENOTDIR);
                }
                if (!(o->server.hdr.flags & PATHMGR_FLAG_STICKY)) {
                    return complete_open(ctp, oep,
                                         (o->server.hdr.flags & PATHMGR_FLAG_DIR) ? EBUSY : ENOSYS);
                }
            ...
            case OBJECT_MEM_SHARED:
                ...
                if (o->mem.pm.mode
                    && devmem_check_perm(ctp, o->mem.pm.mode, o->mem.pm.uid, o->mem.pm.gid,
                                         S_ISUID)) {
                    return complete_open(ctp, oep, EPERM);
                }
                pathmgr_object_detach(o);
