...
static int std_stat(resmgr_context_t * ctp, io_stat_t * msg, void *ocb)
{
    struct pinfo *pp;
...
static int std_open(resmgr_context_t * ctp, io_open_t * msg, void *hdl, void *extra)
{
...
void devstd_init(void)
{
    ...
    for (i = 0; i < sizeof(pinfo) / sizeof(pinfo[0]); i++) {
        ...
        pp->path_id = resmgr_attach(dpp, NULL, pp->path,
                                    _FTYPE_ANY, 0, &std_connect_funcs, &std_io_funcs, pp);
