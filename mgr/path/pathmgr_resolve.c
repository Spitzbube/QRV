...
struct node_entry *pathmgr_resolve_path(struct node_entry *node, struct _io_connect *connect,
                                        char *path, struct node_entry *root)
{
    register char *p1, *p2;
    ...
    for (;;) {
        ...
        if (p1[0] == '.') {
            if (p1[1] == '.') {
                if (p1[2] == '/' || p1[2] == '\0') {
                    ...
                    p1 += 2;
                    eflag |=
                        _IO_CONNECT_EFLAG_DIR | _IO_CONNECT_EFLAG_DOT | _IO_CONNECT_EFLAG_DOTDOT;
                    continue;
                }
            } else if (p1[1] == '/' || p1[1] == '\0') {
        ...
        if (*p1) {
            ...
            if (!p2) {
                ...
                if ((n =
                     pathmgr_node_lookup(node, p1, PATHMGR_LOOKUP_PRUNE | PATHMGR_LOOKUP_ATTACH,
                                         0))) {
                    if (locked_nop) {
                            ...
                            struct _io_connect *connect, const char *path, struct node_entry *root)
{
    ...
    if (connect->type == _IO_CONNECT &&
        connect->subtype == _IO_CONNECT_LINK
        && connect->extra_type == _IO_CONNECT_EXTRA_RESMGR_LINK) {
    ...
    reply =
        (char *) connect +
        ((offsetof(struct _io_connect, path) + connect->path_len + connect->extra_len + len +
          _MALLOC_ALIGN - 1) & ~(_MALLOC_ALIGN - 1));

    server_max_space =
        min((char *) reply + connect->reply_max,
            (char *) connect + ctp->msg_max_size) - (char *) reply;
        ...
        for (o = n->object; o; o = o->hdr.next) {
            ...
            if (o->hdr.type == OBJECT_SERVER) {
                if ((!(local_flags & _PMFLAG_IS_LINK) || o->server.pid == PATHMGR_PID) &&
                    ...
                    if (o->hdr.flags & PATHMGR_FLAG_FTYPEONLY && o->server.file_type != file_type) {
                        continue;
                    }
