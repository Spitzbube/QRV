 ...
 */
static int proc_pathmgr_validate(resmgr_context_t * ctp, io_open_t * msg, int type, char *path,
                                 void **handle)
{
    NODE *nop;
    OBJECT *obj;
    ...
    if ((type & PROCFS_FLAG_MOUNT) && result && *result) {
        ...
        if ((obj->hdr.type == OBJECT_NAME) ||
            ...
            && msg->connect.file_type != entry.file_type) {
            ...
            ret =
                return_a_link(ctp, &msg->link_reply, msg->connect.path, NULL, msg->connect.eflag, 1,
                              1, &entry);
        }
#else
    ...
    if (!(ocb->offset & TOP_BIT)) {
        ...
        for (count = 0; obj; obj = obj->hdr.next) {
            ...
            if (count >= ocb->offset) {
                ...
                p = d->d_name;
                ultoa(obj->server.nd, p, 10);
                p += strlen(p);
                *p++ = ',';
                ultoa(obj->server.pid, p, 10);
                p += strlen(p);
                *p++ = ',';
                ultoa(obj->server.chid, p, 10);
                p += strlen(p);
                *p++ = ',';
                ultoa(obj->server.handle, p, 10);
                p += strlen(p);
                *p++ = ',';
                (void) ltoa(obj->server.file_type, p, 10);
                p += strlen(p);
                *p++ = '\0';
        ...
        for (tmpnode = nop->child; tmpnode; tmpnode = tmpnode->sibling) {
            if (tmpnode->object) {
                if ((ocb->flags & PROCFS_FLAG_MOUNT) &&
                    tmpnode->object->hdr.type != OBJECT_SERVER
                    && tmpnode->object->hdr.type != OBJECT_NAME) {
                    continue;
                }
                if ((ocb->flags & PROCFS_FLAG_LINK)
                    && tmpnode->object->hdr.type != OBJECT_PROC_SYMLINK) {
                    continue;
                }
...
#define EXTRA_ENTRIES 1
        ...
        if (*pos >= EXTRA_ENTRIES) {
            ...
            prp = QueryObject(_QUERY_PROCESS, offset3, _QUERY_PROCESS_VECTOR, 0, &offset3, 0, 0);
            if (prp == NULL)
                break;
            pid = prp->pid;
            QueryObjectDone(prp);
...
static int procfs_waitstop(PROCESS * prp, int rcvid)
{
    ...
    if (DebugProcess(NTO_DEBUG_THREAD_INFO, prp->pid, 0, (union nto_debug_data *) &msg.status) ==
        -1) {
...
static int procfs_stat(resmgr_context_t * ctp, io_stat_t * msg, void *vocb)
{
    ...
    if (ocb->pid && (ocb->flags & PROCFS_FLAG_AS)) {
        ...
        if (proc_rlock_adp(prp) != -1) {
            for (vaddr = 0; memmgr.mapinfo(prp, vaddr, &info, NULL, 0, NULL, NULL, NULL);
                 vaddr = info.vaddr + info.size) {
...
static int memmgr_shared_to_private(PROCESS * prp, uintptr_t addr, size_t size)
{
    ...
    proc_lock_owner_mark(prp);
    ProcessBind(prp->pid);
    status = memmgr.mmap(prp, 0, size, PROT_WRITE | PROT_READ, MAP_PRIVATE | MAP_ANON, NULL,    /* object */
                         0,     /* offset */
                         0,     /* align */
                         0,     /* prealloc */
                         NOFD, &tmap, &map_size, mpid = mempart_getid(prp, sys_memclass_id)
        );
    ...
    status = memmgr.mmap(prp, info.vaddr,   /* address */
                         size, (info.flags & PROT_MASK) | PROT_WRITE, (info.flags & ~(MAP_RESERVMASK | PG_MASK | PROT_MASK | MAP_SHARED)) | MAP_PRIVATE | MAP_ANON | MAP_FIXED, NULL,   /* object */
                         0,     /* offset */
                         0,     /* align */
                         0,     /* prealloc */
                         NOFD, &nmap, &map_size, mpid);
    ...
    minfo.debug.flags =
        (info.flags & ~(MAP_RESERVMASK | PG_MASK | PROT_MASK | MAP_SHARED)) | MAP_PRIVATE;
    minfo.debug.old_vaddr = dinfo.debug.vaddr;
    STRLCPY(minfo.debug.path, dinfo.debug.path,
            sizeof(minfo) - offsetof(struct _mem_debug_info, path));
...
static int procfs_devctl(resmgr_context_t * ctp, io_devctl_t * msg, void *vocb)
{
    ...
    case DCMD_ALL_GETFLAGS:
        ioctl->oflag =
            ocb ? (ocb->ioflag & ~O_ACCMODE) | ((ocb->ioflag - 1) & O_ACCMODE) : O_RDONLY;
    ...
    case DCMD_PROC_INFO:
        if (DebugProcess(NTO_DEBUG_PROCESS_INFO, ocb->pid, 0, (union nto_debug_data *) &ioctl->info)
            == -1) {
            return errno;
        }
    ...
    case DCMD_PROC_SET_FLAG:
        if (DebugProcess(NTO_DEBUG_SET_FLAG, ocb->pid, 0, (union nto_debug_data *) &ioctl->flags) ==
            -1) {
            return errno;
        }
        break;
    ...
    case DCMD_PROC_CLEAR_FLAG:
        if (DebugProcess(NTO_DEBUG_CLEAR_FLAG, ocb->pid, 0, (union nto_debug_data *) &ioctl->flags)
            == -1) {
            return errno;
        }
        break;
    ...
    case DCMD_PROC_TIDSTATUS:
        ...
        if (DebugProcess
            (NTO_DEBUG_THREAD_INFO, ocb->pid, ioctl->status.tid,
             (union nto_debug_data *) &ioctl->status) == -1) {
            return errno;
        }
        break;
    ...
    case DCMD_PROC_SIGNAL:
        if (SignalKill
            (ND_LOCAL_NODE, ocb->pid, ioctl->signal.tid, ioctl->signal.signo, ioctl->signal.code,
             ioctl->signal.value) == -1) {
            return errno;
        }
        break;
    ...
    case DCMD_PROC_GETREGSET:
        if ((ret_val =
             procfs_regset_read(ocb, prp, ioctl->regset.id, ioctl->regset.buf, &nbytes)) != EOK) {
            return ret_val;
        }
    ...
    case DCMD_PROC_SETREGSET:
        if ((ret_val =
             procfs_regset_write(ocb, prp, ioctl->regset.id, ioctl->regset.buf, &nbytes)) != EOK) {
            return ret_val;
        }
        nbytes += sizeof(ioctl->regset.id);
        ret_val = nbytes;
...
#endif
        if ((ret_val =
             procfs_regset_read(ocb, prp, REGSET_ALTREGS, &ioctl->altreg, &nbytes)) != EOK) {
            return ret_val;
        }
        ret_val = nbytes;
        break;
...
#endif
        if ((ret_val =
             procfs_regset_write(ocb, prp, REGSET_ALTREGS, &ioctl->altreg, &nbytes)) != EOK) {
            return ret_val;
        }
        ret_val = nbytes;
        break;
    ...
    default:
        ...
        case DCMD_PROC_THREADCTL:
            ...
            case _NTO_TCTL_RUNMASK_GET_AND_SET:
                if (*(unsigned *) ioctl->threadctl.data != 0 && (ocb->ioflag & _IO_FLAG_WR) == 0) {
                    /*
                     * Can't alter without
                     * write perms.
                     */
            ...
            ret_val =
                KerextThreadCtl(prp, ioctl->threadctl.tid, ioctl->threadctl.cmd,
                                ioctl->threadctl.data);
            if (ret_val == -1) {
                return proc_error(errno, prp);
            }
            break;
        ...
        case DCMD_PROC_MAPDEBUG_BASE:
            {
                ...
                if (prp->debug_name != NULL) {
                    ...
                    STRLCPY(ioctl->mapdebug.path, prp->debug_name,
                            space - offsetof(procfs_debuginfo, path));
                    msg->o.ret_val = 0;
                    SETIOV(ctp->iov + 0, &msg->o,
                           sizeof msg->o + offsetof(procfs_debuginfo,
                                                    path) + strlen(ioctl->mapdebug.path) + 1);
        ...
        case DCMD_PROC_MAPDEBUG:
            {
                ...
                CRASHCHECK(space < offsetof(procfs_debuginfo, path));
                if (memmgr.
                    mapinfo(prp, ioctl->mapdebug.vaddr, NULL, &ioctl->mapdebug,
                            space - offsetof(procfs_debuginfo, path), NULL, NULL, NULL) == 0) {
                    proc_unlock_adp(prp);
                    return proc_error(ENXIO, prp);
                }
                proc_unlock_adp(prp);
        ...
        case DCMD_PROC_MAPINFO:{
                ...
                do {
                    ...
                    while (num--
                           && (size = memmgr.mapinfo(prp, vaddr, p, NULL, 0, NULL, NULL, NULL))) {
                        ret_val++;
                        nbytes += sizeof *p;
        ...
        case DCMD_PROC_IRQS:{
                ...
                do {
                    ...
                    while (num &&
                           ((entryp =
                             QueryObject(_QUERY_INTERRUPT, id, 0, 0, &id, &entry,
                                         sizeof(entry))) != NULL)) {
        ...
        case DCMD_PROC_CHANNELS:{
                ...
                do {
                    ...
                    while (num &&
                           ((chp =
                             QueryObject(_QUERY_PROCESS, prp->pid, _QUERY_PROCESS_CHANCONS, id, &id,
                                         &chan, sizeof(chan))) != NULL)) {
        ...
        case DCMD_PROC_BREAK:{
                ...
                ProcessBind(ocb->pid);
                if (DebugProcess(NTO_DEBUG_BREAK, ocb->pid, 0, (union nto_debug_data *) &ioctl->brk)
                    == -1) {
                    ret_val = errno;
                    ProcessBind(0);
                    proc_unlock_adp(prp);
        ...
        case DCMD_PROC_GET_BREAKLIST:{
                ...
                do {
                    ...
                    if (ctp->msg_max_size - sizeof(*msg) < nbytes) {
                        breaklist.count =
                            (ctp->msg_max_size - sizeof(*msg)) / sizeof(*breaklist.breakpoints);
                    } else {
                    ...
                    ProcessBind(ocb->pid);
                    if (DebugProcess
                        (NTO_DEBUG_GET_BREAKLIST, ocb->pid, 0,
                         (union nto_debug_data *) &breaklist) == -1) {
                        ret_val = errno;
                        ProcessBind(0);
                        proc_unlock_adp(prp);
                        return proc_error(ret_val, prp);
                    ...
                    if (resmgr_msgwrite(ctp, breaklist.breakpoints,
                                        ...
                                        sizeof(msg->o) + offset * sizeof(*breaklist.breakpoints)) ==
                        -1) {
                        return proc_error(errno, prp);
                    }
        ...
        case DCMD_PROC_DEL_MEMPARTID:
            {
                ...
                if ((r = MEMPART_VALIDATE_ID(ioctl->mempart_id)) == EOK) {
                    ...
                    if ((unsigned) msg->i.dcmd == DCMD_PROC_DEL_MEMPARTID) {
                        ...
                        if ((r =
                             ProcessAssociate(prp, ioctl->mempart_id,
                                              part_flags_NONE)) == EALREADY) {
                            r = EOK;
                        }
                    }
                }
...
static int procfs_open(resmgr_context_t * ctp, io_open_t * msg, void *handle, void *extra)
{
    ...
    if (ocb->ioflag & _IO_FLAG_WR) {
        ...
        if (!(flags & PROCFS_FLAG_SELF) && prp->pid != SYSMGR_PID
            && DebugAttach(prp->pid, _DEBUG_FLAG_RLC) == -1) {
            _sfree(ocb, sizeof *ocb);
            return proc_error(errno, prp);
        }
    }
