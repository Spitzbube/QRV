...
static int resmgr_sem_open(resmgr_context_t * ctp, io_open_t * msg, RESMGR_HANDLE_T * handle,
                           void *extra);
static int resmgr_sem_unlink(resmgr_context_t * ctp, io_unlink_t * msg, RESMGR_HANDLE_T * handle,
                             void *extra);
...
static int resmgr_dir_open(resmgr_context_t * ctp, io_open_t * msg, RESMGR_HANDLE_T * handle,
                           void *extra);
static int resmgr_dir_unlink(resmgr_context_t * ctp, io_unlink_t * msg, RESMGR_HANDLE_T * handle,
                             void *extra);
...
static blocked_t *enqueue_client(semaphore_t * sem, resmgr_context_t * ctp)
{
    ...
    if ((blk = blocked_item(NULL)) != NULL) {
        ...
        for (head = &sem->blocked; *head != NULL && (*head)->priority >= blk->priority;
             head = &(*head)->link);
#else
        head = &sem->blocked;
#endif
...
static void unblock_clients(semaphore_t * sem, resmgr_context_t * ctp, int match, int error)
{
    ...
    if (!S_ISDIR(sem->attr.mode)) {
        head = &sem->blocked;
        while ((b = *head) != NULL) {
            if ((ctp == NULL) || (match && b->rcvid == ctp->rcvid)
                || (!match && b->scoid == ctp->info.scoid && b->coid == ctp->info.coid)) {
                if (error == EOK)
 ...
 */
static semaphore_t *create_sem(semaphore_t * head, const char *name, struct _client_info *cred,
                               iofunc_attr_t * attr, uint32_t * initial)
{
    semaphore_t *sem;
 ...
 */
static int list_sem(resmgr_context_t * ctp, semaphore_t * dir, off64_t * offset, struct dirent *dp,
                    int dpsize, int xtra, int *nbytes)
{
    semaphore_t *sem;
...
int resmgr_sem_open(resmgr_context_t * ctp, io_open_t * msg, RESMGR_HANDLE_T * handle, void *extra)
{
    ...
    if ((msg->connect.extra_type != _IO_CONNECT_EXTRA_NONE
         && msg->connect.extra_type != _IO_CONNECT_EXTRA_SEM)
        || msg->connect.file_type != _FTYPE_SEM)
        return (ENOSYS);
    ...
    if (msg->connect.ioflag & O_CREAT) {
        ...
        create.verification = iofunc_open(ctp, msg, create.attr =
                                          &attr, &handle->attr, create.cred = &cred);
    ...
    return ((error != EOK
             || (ctp->info.flags & _NTO_MI_ENDIAN_DIFF)) ? error : resmgr_sem_stat(ctp,
                                                                                   (io_stat_t *)
                                                                                   msg,
                                                                                   _resmgr_ocb(ctp,
                                                                                               &ctp->
 ...
 */
int resmgr_sem_unlink(resmgr_context_t * ctp, io_unlink_t * msg, RESMGR_HANDLE_T * handle,
                      void *extra)
{
 ...
 */
int resmgr_dir_unlink(resmgr_context_t * ctp, io_unlink_t * msg, RESMGR_HANDLE_T * handle,
                      void *extra)
{
    semaphore_t *sem;
    char *name;
...
int resmgr_dir_read(resmgr_context_t * ctp, io_read_t * msg, RESMGR_OCB_T * ocb)
{
    ...
    if ((msg->i.xtype & _IO_XTYPE_MASK) != _IO_XTYPE_NONE)
        return (ENOSYS);
    xtra = (msg->i.xtype & _IO_XFLAG_DIR_EXTRA_HINT) != 0
        && iofunc_client_info(ctp, ocb->ioflag, &cred) == EOK
        && iofunc_check_access(NULL, ocb->attr, S_IXUSR, &cred) == EOK;
    _mutex_lock(&SemaphoreMutex);
    error =
        list_sem(ctp, OCB2SEM(ocb), &ocb->offset, (struct dirent *) msg,
                 min(msg->i.nbytes, ctp->msg_max_size - ctp->offset), xtra, &nbytes);
    _mutex_unlock(&SemaphoreMutex);
...
void namedsem_init(void)
{
    ...
    DirIoFuncs.read = resmgr_dir_read, DirIoFuncs.close_ocb = resmgr_dir_close, DirIoFuncs.stat =
        resmgr_dir_stat;
    ...
    ResmgrAttr.nparts_max = 1, ResmgrAttr.msg_max_size =
        sizeof(struct _io_connect) + _POSIX_PATH_MAX;
    ...
    NamedSemaphores.attr.rdev =
        resmgr_attach(dpp, &ResmgrAttr, NULL, _FTYPE_SEM, _RESMGR_FLAG_DIR | _RESMGR_FLAG_FTYPEONLY,
                      &SemConnectFuncs, &SemIoFuncs, &NamedSemaphores);
    NamedSemaphores.attr.inode = (uintptr_t) & NamedSemaphores;
    DirPathname =
        resmgr_attach(dpp, &ResmgrAttr, NamedSemaphores.name, _FTYPE_ANY, _RESMGR_FLAG_DIR,
                      &DirConnectFuncs, &DirIoFuncs, &NamedSemaphores);
