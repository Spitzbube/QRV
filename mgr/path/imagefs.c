...
static char *image_get_path(union image_dirent *dir)
{
    if (!dir || !dir->attr.ino) {
        return NULL;
    }
 ...
 */
static int
image_getdir_children(union image_dirent *startdir, char *path, char ***children,
                      uint16_t * childcount, uint16_t * index)
{
    union image_dirent *dir;
    int len, ncount;
    char **names;
    ...
    for (dir = startdir; dir->attr.size;
         dir = (union image_dirent *) ((char *) dir + dir->attr.size)) {
        char *entry_name;
        int i, entry_len;
        ...
        *index = ((uintptr_t) entry_name - (uintptr_t) startdir) + len;
        ...
        if (i <= 0 && len > 0) {
            continue;           //We need at least one slash character separator, unless we are at root
        }

        entry_len = 0;
        while (entry_name[entry_len] && entry_name[entry_len] != '/') {
            entry_len++;
        }
        if (!entry_len) {
...
static int image_read(resmgr_context_t * ctp, io_read_t * msg, void *vocb)
{
    ...
    if ((dire = ocb->dir) && S_ISREG(ocb->mode)) {
        ...
        *pos += nbytes;
        ...
        for (; ocb->children && *pos < ocb->childcount; ++*pos) {
            ...
            cplen = 0;
            while (entry_name[cplen] && entry_name[cplen] != '/') {
                cplen++;
            }
            if (cplen == 0) {
                continue;
            ...
            memcpy(dirent->d_name, entry_name, cplen);
            dirent->d_name[cplen] = '\0';
...
static int image_close_ocb(resmgr_context_t * ctp, void *reserved, void *vocb)
{
    struct image_ocb *ocb = vocb;
...
static int image_stat(resmgr_context_t * ctp, io_stat_t * msg, void *vocb)
{
    struct image_ocb *ocb = vocb;
    struct image_data *image = ocb->image;
    union image_dirent *dire;
    ...
    if ((dire = ocb->dir)) {
        ...
        msg->o.st_ctime = msg->o.st_mtime = msg->o.st_atime = dire->attr.mtime;
...
static int image_pathconf(resmgr_context_t * ctp, io_pathconf_t * msg, void *vocb)
{
    struct image_ocb *ocb = vocb;
    struct image_data *image = ocb->image;
...
static int image_devctl(resmgr_context_t * ctp, io_devctl_t * msg, void *vocb)
{
    struct image_ocb *ocb = vocb;
    union {
        int oflag;
...
static int image_lseek(resmgr_context_t * ctp, io_lseek_t * msg, void *vocb)
{
    struct image_ocb *ocb = vocb;
...
static int image_mmap(resmgr_context_t * ctp, io_mmap_t * msg, void *vocb)
{
...
static union image_dirent *image_lookup(struct image_data *image, struct _io_connect *connect,
                                        int *state)
{
    ...
    len = strlen(name);
    for (dir = image->dir; dir->attr.size;
         dir = (union image_dirent *) ((unsigned) dir + dir->attr.size)) {
        if (!dir->attr.ino) {
            continue;
        }
...
static int image_open(resmgr_context_t * ctp, io_open_t * msg, void *handle, void *extra)
{
    ...
    if ((dire = image_lookup(image, &msg->connect, &notmatch))) {
        ...
        if (notmatch == LOOKUP_INT_DIR || S_ISDIR(dire->attr.mode)) {
            ...
            (void) image_getdir_children(image->dir, msg->connect.path, &ocb->children,
                                         &ocb->childcount, &ocb->index);
...
static int image_readlink(resmgr_context_t * ctp, io_readlink_t * msg, void *handle, void *extra)
{
    return image_open(ctp, (io_open_t *) msg, handle, extra);
}

static int image_unlink(resmgr_context_t * ctp, io_unlink_t * msg, void *handle, void *extra)
{
    struct image_data *image = handle;
    union image_dirent *dire;
    int notmatch = 0;
...
image_mount(resmgr_context_t * ctp, io_mount_t * msg, void *handle, io_mount_extra_t * extra)
{
    ...
    /* Perform the mount */
    ifs_image = imagefs_mount(offset, size, 0, flags, sysmgr_prp->root, msg->connect.path);
    if (ifs_image == (void *) -1) {
        return errno;
    }
...
static void kerext_validate_signature(void *p)
{
    int i;
    int j;
...
#define NATIVE_ENDIAN   IMAGE_FLAGS_BIGENDIAN
    ...
    image->dir =
        (union image_dirent *) ((char *) addr + ((struct image_header *) addr)->dir_offset);
    image->size = size;
