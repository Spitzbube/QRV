                    ...
                    unsigned response_len, void *response, int *status)
{
    ...
    ret =
        _connect_entry(base, path, mode, oflag, sflag, subtype, testcancel, accessl, file_type,
                       extra_type, extra_len, extra, response_len, response, status, &entry, 1);
...
static int pathmgr_fdinfo(resmgr_context_t * ctp, io_fdinfo_t * msg, void *vocb)
{
    ...
    if (pathmax) {
        ...
        if ((len = netmgr_ndtostr((flags & _FDINFO_FLAG_LOCALPATH) ? ND2S_LOCAL_STR | ND2S_DIR_SHOW
                                  : ND2S_DIR_SHOW, obp->server.nd, path, pathmax)) <= 1) {
            len = 1;
            *path = '\0';
        } else {
            int n;
                   ...
                   struct _io_resmgr_link_extra *extra)
{
    ...
    if (!
        (obp =
         pathmgr_object_attach(prp, 0, msg->connect.path, OBJECT_SERVER, extra->flags, extra))) {
        return ENOMEM;
    }
...
static char *add_piece(char *p, char *endp, char *src, unsigned len)
{
    int left;
    char *r = p + len;
