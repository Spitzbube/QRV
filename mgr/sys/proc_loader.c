...
static int loader_fd(struct loader_context *lcp)
{
    ...
    if ((lcp->state & LC_STATE_MASK) != LC_FORK) {
        ...
        if (ND_NODE_CMP(lcp->pnode, ND_LOCAL_NODE) == 0) {
            ...
            pfdinfo =
                 ...
                 *) (((uintptr_t) p + sizeof(*p) + p->root_len + p->cwd_len + sizeof(_Int32t) -
                      1) & ~(sizeof(_Int32t) - 1));
        }
    } else {
        nfds = i_nfds = 0;
    ...
    while (1) {
        ...
        case _FD_SPAWN_NOCLOEXC:
            if (ConnectServerInfo(lcp->ppid, ++fd, &info) == -1
                || ((fd = info.coid) & _NTO_SIDE_CHANNEL)) {
                return EOK;
            }
                ...
                /* read additional fd info */
                {
                    ...
                    if (type == _FD_SPAWN_REMOTE) {
                        ...
                        if (coid == SYSMGR_COID) {
                            ...
                            if ((coid =
                                 ConnectAttach(lcp->pnode, PROCMGR_PID, PROCMGR_CHID,
                        ...
                        pfdinfo =
                            (struct _proc_spawn_fd_info *) ((char *) &lcp->msg.spawn.i + size);
                    ...
                    if (i_nfds) {
                        ...
                        SETIOV(lcp->iov + 1,
                               (char *) &lcp->msg.spawn.i + sizeof lcp->msg.spawn.i +
                               fd * sizeof(_Int32t), spawnfd.i.nfds * sizeof(_Int32t));
...
#ifndef NDEBUG
        /* assert proper args */
        if ((buffer == NULL) || (bufsize < strlen(name)))
            crash();
#endif
                        ...
                        char *interp_name, char *interp_arg)
{
    ...
    int exelen;
    int arg_bytes;
    int ninterp = 0, interplen = 0, interp_name_len = 0, interp_arg_len = 0;
...
#error STACK_GROWS_UP not supported
    ...
    if (lcp->msgsize - (p - lcp->msg.filler) > lcp->msg.spawn.i.nbytes) {
        ...
        args.i.zero = 0;
        if (MsgSend(PROCMGR_COID, &args.i, sizeof args.i, strings + interplen, args.i.nbytes) == -1) {
            ThreadDestroy(-1, -1, (void *) errno);
        }
    }
...
int try_hash_bang(int fd, char **pname, char **arg, char *buf, int buflen)
{
    int numread;
    char *ptr = buf;
...
static int open_exe(const char *name, struct stat *statp)
{
    int fd, flags;
...
void *loader_load(void *parm)
{
    ...
    p = (const char *) &lcp->msg.spawn.i + (sizeof lcp->msg.spawn.i +
                                            lcp->msg.spawn.i.nfds * sizeof(long));
    ...
    if ((lcp->start.esp =
         (uintptr_t) ThreadTLS(0, (uintptr_t) base, size, guardpagesize,
                               (uintptr_t) base + size)) == (uintptr_t) - 1) {
