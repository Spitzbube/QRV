...
static int rdecl acquire_lock(struct proc_mux_lock **lpp)
{
...
static void rdecl release_lock(struct proc_mux_lock **lpp)
{
...
PROCESS *proc_lock_parent(PROCESS * prp)
{
    ...
    for (;;) {
        ...
        if (count > 100) {
            ...
            sleepl = 1;
            TimerTimeout(CLOCK_MONOTONIC, _NTO_TIMEOUT_NANOSLEEP, NULL, &sleepl, NULL);
        } else if (count > 50) {
