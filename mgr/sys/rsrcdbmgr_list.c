...
rsrc_root_node_t *rsrc_root_locate(const char *name, int create)
{
    ...
    for (root = g_rsrc_root; root; root = root->next_root) {
        if (strncmp(root->head_node.name, name, len) == 0 && strlen(root->head_node.name) == len) {
            break;
        }
    }
...
static int16_t find_range_priority(rsrc_root_node_t * root,
                                   uint64_t start, uint64_t end, unsigned flags)
{
    ...
    for (create = root->created; create; create = create->next) {
        ...
        if ((create->start <= start && start <= create->end) ||
            ...
            priority =
                __max(create->node->priority + (create->flags & RSRCDBMGR_FLAG_RSVP) ? 1 : 0,
                      priority);
        }
    }
...
static int find_next_priority(rsrc_root_node_t * root,
                              uint64_t start, uint64_t end, int backward, uint64_t * value)
{
    rsrc_create_t *create;
...
int rsrc_find_range(rsrc_root_node_t * root, uint64_t length,
                    ...
                    uint64_t alignment, pid_t owner, unsigned flags, int *priority)
{
        ...
        if (curblk->owner && overlaps) {
            if (newstart < curblk->start) {
                if (absorb) {
                    ...
                    absorb =
                        rsrc_insert_block(&root->active, curblk, newstart, curblk->start - 1, pid,
                                          flags, 0);
                }
            }
...
int _rsrc_block_limit_op(rsrc_root_node_t * root, uint64_t start, uint64_t end,
                         void *odata,
                         int (*overlap)(rsrc_root_node_t * root, uint64_t start, uint64_t end,
                                        void *data), void *nodata,
                         int(*nonoverlap)(rsrc_root_node_t * root, uint64_t start, uint64_t end,
                                          void *data))
{
    ...
    for (tmpinfo = root->created; tmpinfo && purgestart <= purgeend; tmpinfo = tmpinfo->next) {
        ...
        //If we start after the purge block, we should purge the range and continue
            ...
            if (nonoverlap) {
                ret =
                    nonoverlap(root, purgestart, (tmpinfo->start) ? tmpinfo->start - 1 : 0, nodata);
            }
            purgestart = (tmpinfo->end != ULONGLONG_MAX) ? tmpinfo->end + 1 : ULONGLONG_MAX;
        }
    }
...
int _rsrc_block_search_op(rsrc_request_t * rsrc, pid_t pid, unsigned flags, int *priority)
{
    ...
    if (!rsrc || !rsrc->name || ((rsrc->flags & RSRCDBMGR_FLAG_RANGE) && rsrc->start > rsrc->end)) {
        return EINVAL;
    }
    ...
    if (flags & FLAG_ATTACH_RMTOPID) {  /* Remove and assign to pid */
        ...
        if (!(flags & FLAG_ATTACH_RM)) {
            _rsrc_block_limit_op(root, lastmatch.start, lastmatch.end, NULL, _add_callout,  /* Overlap ranges */
...
int _rsrcdbmgr_create(rsrc_alloc_t * rsrc, pid_t pid)
{
    ...
    _rsrc_block_limit_op(root, create->start, create->end, NULL, NULL,  /* Overlap ranges */
...
int _rsrcdbmgr_destroy(rsrc_alloc_t * rsrc, pid_t pid)
{
    ...
    _rsrc_block_limit_op(root, rsrc->start, rsrc->end, NULL, NULL,  /* Overlap ranges */
...
{
    if (pid < 0) {
        return _rsrc_block_search_op(rsrc, -pid, FLAG_ATTACH_RMFROMPID | FLAG_ATTACH_DONTLOOK,
                                     priority);
    } else {
        return _rsrc_block_search_op(rsrc, pid, FLAG_ATTACH_RMTOPID | FLAG_ATTACH_DONTLOOK,
                                     priority);
    }
}
    ...
    for (root = g_rsrc_root; root; root = root->next_root) {
        ...
        while ((ctarget = *create)) {
            if (ctarget->creator == pid) {
                *create = ctarget->next;
                ...
                _rsrc_block_limit_op(root, ctarget->start, ctarget->end, NULL, NULL,    /* Overlap ranges */
                                     NULL, _remove_callout);    /* Non-overlap ranges */
        ...
        while ((btarget = *blockp)) {
            if (btarget->owner == pid) {
                ...
                _rsrc_block_limit_op(root, btarget->start, btarget->end, NULL, _add_callout,    /* Overlap ranges */
                                     NULL, NULL);   /* Non-overlap ranges */
...
int _rsrcdbmgr_pid_mark(pid_t pid)
{
    ...
    if (!(pidrsrc = (rsrc_list_array_t *) prp->rsrc_list)) {
        pidrsrc = prp->rsrc_list = _scalloc(sizeof(*pidrsrc));
    }
                     ...
                     rsrc_alloc_t * array, uint32_t nbytes, char **newname)
{
    ...
    if (newname) {
        *newname = (char *) &array[i - start];
        STRLCPY(*newname, root->head_node.name,
                nbytes - ((size_t) &array[i - start] - (size_t) array));
    }
