...
static void trace(tProcess * prp, siginfo_t * info)
{
    ...
    if (sig == SIGCHLD) {
        ...
        if (info->si_code > SI_USER) {
            ...
            case SIGTRAP:
            case SIGBUS:
                p += ksnprintf(p, (&buff[sizeof buff] - p) - 2, " fltno=%d ip=%p", info->si_fltno,
                               info->si_fltip);
                if (_dladdr(info->si_fltip, &sym) && sym.dli_sname) {
                    if (!sym.dli_fname) {
                        sym.dli_fname = basename(prp->debug_name);
                    }
                    p += ksnprintf(p, (&buff[sizeof buff] - p) - 2, "(%s@%s+0x%x)", sym.dli_fname,
                                   sym.dli_sname,
                                   (uintptr_t) info->si_fltip - (uintptr_t) sym.dli_saddr);
                }
            ...
            p += ksnprintf(p, (&buff[sizeof buff] - p) - 2, " by process %d value=%x.",
...
static void cleanup(tProcess * prp, siginfo_t * info)
{
    ...
    for (;;) {
        ...
        i = -1U;
        while ((chp =
                QueryObject(_QUERY_PROCESS, prp->pid, _QUERY_PROCESS_CHANCONS, ++i, &i, 0, 0))) {
