...
static void debug_info(const void *vaddr, const Elf32_Phdr * phdr, int fd, const char *name)
{
...
int
elf_load(int fd, const char *path, struct loader_startup *lsp, struct stat *statlocal,
         struct inheritance *parms)
{
    ...
    if (i != sizeof ehdr || memcmp(ehdr.e_ident, ELFMAG, SELFMAG)
    ...
    if (!(phdrs = alloca(sizeof *phdrs * ehdr.e_phnum))
        || !(seg = alloca(sizeof *seg * ehdr.e_phnum))) {
        return ENOEXEC;
    }
    ...
    for (i = 0; i < ehdr.e_phnum; i++) {
        ...
        case PT_LOAD:
            ...
            if (lsp != NULL) {
                addr =
                    mmap((void *) phdr->p_vaddr, phdr->p_filesz, prot, flags, fd, phdr->p_offset);
                if (addr == MAP_FAILED) {
                    ...
                    addr =
                        mmap((void *) phdr->p_vaddr, phdr->p_memsz, prot | PROT_WRITE,
                             MAP_PRIVATE | MAP_ANON | MAP_ELF | MAP_FIXED, NOFD, 0);
                    if (addr == MAP_FAILED) {
                        return errno;
                    }
                    ...
                    if (mem_end > fil_end) {
                        ...
                        if (mmap
                            ((void *) fil_end, mem_end - fil_end, prot,
                             (MAP_FIXED | MAP_ANON) | (flags & MAP_TYPE), NOFD, 0) == MAP_FAILED) {
                            return errno;
                        }
                    }
                }
        ...
        case PT_NOTE:
            if (0 == phdr->p_filesz)
                break;          //ignore zero-len .note headers. But return ENOEXEC if they are malformed sizes.
            if (phdr->p_filesz >= sizeof *note) {
                ...
                if (proc_read(fd, note, size, phdr->p_offset) == size) {
                    while (size > 0) {
                        ...
                        if (note->nhdr.n_namesz == sizeof QNX_NOTE_NAME
                            && !strcmp(note->strings, QNX_NOTE_NAME)) {
                        ...
                        len =
                            sizeof note->nhdr + ((note->nhdr.n_namesz + 3) & ~3) +
                            ((note->nhdr.n_descsz + 3) & ~3);
        ...
        case PT_SEGREL:
            ...
            if (proc_read(fd, buffer, size, phdr->p_offset) != size) {
                ...
                addr_num =
                    (ROUNDUP((curseg->addr - curseg->reloc) + curseg->filesz, __PAGESIZE) -
                     addrlocal) / __PAGESIZE + 1;
                ...
                while (segaddr->sr_un.sr_addr.sra_skip != (Elf32_Half) - 1) {
                    if (segaddr->sr_un.sr_addr.sra_skip == 0) {
                        ...
                        for (ent = segaddr->sr_un.sr_addr.sra_index;
                             ent < segaddr[1].sr_un.sr_addr.sra_index; ent++) {
                            int *p;
                            ...
                            if (ent >= entend) {
                                ...
                                if (proc_read(fd, segent, num, phdr->p_offset + ent * sizeof *sr) !=
                                    num) {
                                    return ENOEXEC;
                                }
                                entoff = ent;
                            ...
                            p = (int *) (addrlocal + segent[ent - entoff].sr_un.sr_ent.sre_offset +
                                         curseg->reloc);
    ...
    if (lsp && interp) {
        ...
        if (proc_read(fd2, &ehdr, sizeof ehdr, 0) != sizeof ehdr
            || memcmp(ehdr.e_ident, ELFMAG, SELFMAG) || ehdr.e_ident[EI_DATA] != ELFDATANATIVE
        ...
        if (proc_read(fd2, phdrs, sizeof *phdrs * ehdr.e_phnum, ehdr.e_phoff) !=
        ...
        for (i = 0, phdr = phdrs; i < ehdr.e_phnum; i++, phdr++) {
            if (phdr->p_type == PT_LOAD) {
                ...
                vaddr = mmap((void *) start, phdr->p_filesz, prot, flags, fd2, phdr->p_offset);
                if (vaddr == MAP_FAILED) {
    ...
    if (!debug_msg.i.name[0]) {
        ...
        STRLCPY(debug_msg.i.name, path,
                sizeof(debug_msg) - offsetof(struct _proc_spawn_debug, name));
    }
    ...
    (void) MsgSend(MEMMGR_COID, &debug_msg.i,
                   offsetof(struct _proc_spawn_debug, name) + strlen(debug_msg.i.name) + 1, 0, 0);
