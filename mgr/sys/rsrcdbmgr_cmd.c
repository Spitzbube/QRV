...
/*
 ...
 based entries.
*/
static const char *rsrc_names[] =
    { "memory", "irq", "io", "dma", /*"memory/pci" */ "pcimemory", "unknown" };
static char *rsrcdbmgr_id2name(int id)
{
...
do_create_destroy(int (*getfunc)(rsrc_alloc_t * alloc, pid_t pid),
                  ...
                  rsrc_alloc_t * alloc, unsigned count, const char *rsrcname, pid_t pid)
{
    int indx;
    int ret;
...
static int do_detach(rsrc_request_t * request, unsigned count, const char *rsrcname, pid_t pid)
{
    int indx;
    int ret;
...
static int do_attach(rsrc_request_t * request, unsigned count, const char *rsrcname, pid_t pid)
{
    int indx;
    int bestindx;
    int bestpri;
...
/*
 ...
 the request.
*/
int rsrcdbmgr_handler(message_context_t * mctp, int code, unsigned flags, void *handle)
{
...
*/
int rsrcdbmgr_proc_interface(void *list, int count, int cmd)
{
    int ret;
...
int rsrcdbmgr_proc_query(rsrc_alloc_t * list, int count, int start, int flags)
{
    int ret;
...
int rsrcdbmgr_proc_query_name(rsrc_alloc_t * list, int count, int start, char *name)
{
    int ret;

    pthread_mutex_lock(&g_rsrc_mutex);
    ret = _rsrcdbmgr_query(name, 0, 0, start, list, count * sizeof(*list), NULL);
    pthread_mutex_unlock(&g_rsrc_mutex);
    return ret;
}
