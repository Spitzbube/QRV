...
#ifndef NKDEBUG
static int kdebug_path(struct kdebug_entry *entry, char *buff, int buffsize)
{
    char *p;
...
static char *boot_waitfor_reopen(struct script_waitfor_reopen *scp, int fds[3], char *tty)
{
    unsigned check;
...
static pid_t boot_external(struct script_external *scp, int extsched, int fds[3])
{
    pid_t pid;
    int argc, envc;
...
static int map_ifs(struct asinfo_entry *as, char *name, void *d)
{
    ...
    image =
        imagefs_mount(as->start, (as->end - as->start) + 1, 0, _RESMGR_FLAG_AFTER, sysmgr_prp->root,
                      0);
...
static int bootimage_start(message_context_t * ctp, int code, unsigned flags, void *handle)
{
...
#endif
        case S_IFREG:
            if ((boot_image->flags & IMAGE_FLAGS_INO_BITS)
                && (dir->attr.ino & IFS_INO_BOOTSTRAP_EXE)) {
    ...
    if (SchedCtl(SCHED_QUERY_SCHED_EXT, &query, sizeof(query)) == -1)
        query.extsched = SCHED_EXT_NONE;

    for (pid = 0; (size = scp->hdr.size_lo | (scp->hdr.size_hi << 8));
         scp = (union script_cmd *) ((char *) scp + ((size + 3) & ~3))) {
        if (pid) {
        ...
        case SCRIPT_TYPE_EXTSCHED_APS:
            if (query.extsched == SCHED_EXT_APS) {
                ...
                aps.critical_budget_ms =
                    scp->extsched_aps.critical_hi << 8 | scp->extsched_aps.critical_lo;
