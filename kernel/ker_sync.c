...
int kdecl ker_sync_create(THREAD * act, struct kerargs_sync_create *kap)
{
            ...
            case PTHREAD_PRIO_PROTECT:
                {
                    ...
                    if ((pri < 1) || (pri >= NUM_PRI)
                        || (act->process->cred->info.euid != 0 && pri >= priv_prio)) {
                        return EINVAL;
                    }
                    lock_kernel();
    ...
    if ((syp =
         sync_create(act->process, sync, attr ? attr->__flags : PTHREAD_PROCESS_PRIVATE)) == NULL) {
        if (syncep) {
...
int kdecl ker_sync_mutex_lock(THREAD * act, struct kerargs_sync_mutex_lock *kap)
{
    ...
    mt_trace_mutex_lock(sync, act->process->pid, act->tid,
                        sync->__owner == -1 ? -1 : SYNC_TID(sync->__owner));
#endif
