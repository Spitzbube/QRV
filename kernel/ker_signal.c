...
int do_ker_signal_kill(THREAD * act, struct kerargs_signal_kill *kap)
{
    ...
    if (tgt_pid > 0) {
        ...
        if (kap->tid == 0) {
            status = signal_kill_process(prp, signo, kap->code, kap->value, src_pid, flags);
        } else {
            status = signal_kill_thread(prp, thp, signo, kap->code, kap->value, src_pid, flags);
        }
        pulse_souls.flags |= SOUL_CRITICAL;
    } else {
        lock_kernel();
        ...
        // CPU exception fault delivery
        pulse_souls.flags &= ~SOUL_CRITICAL;
        status = signal_kill_group(act, -tgt_pid, signo, kap->code, kap->value, src_pid, flags);
        pulse_souls.flags |= SOUL_CRITICAL;
    }
...
//  else
    ...
    if (ssp->mutex) {
        ...
        if (act->args.mu.saved_timeout_flags & _NTO_TIMEOUT_ACTIVE) {
            ...
            // from doing anything.
            act->timeout_flags = act->args.mu.saved_timeout_flags & ~_NTO_TIMEOUT_ACTIVE;
...
int kdecl ker_signal_action(THREAD * act, struct kerargs_signal_action *kap)
{
    ...
    if (kap->act) {
        ...
        if (new.sa_flags & (SA_MASK &
                            ~(SA_SIGINFO |
                              SA_NOCLDSTOP | SA_NOCLDWAIT | SA_RESETHAND | SA_NODEFER))) {
            return ENOTSUP;
        }
    ...
    shp = get_signal_handler(prp, kap->signo,
                             kap->act && (((uintptr_t) new.sa_handler > 15) ||
                                          (new.
                                           sa_flags & (SA_MASK & ~(SA_SIGINFO | SA_NOCLDSTOP)))));
    ...
    if (kap->act) {
        ...
        case (intptr_t) SIG_DFL:
            ...
            if (kap->signo == SIGCHLD || kap->signo == SIGURG ||
                kap->signo == SIGCONT || kap->signo == SIGPWR || kap->signo == SIGWINCH) {
                signal_ignore(prp, kap->signo);
            }
...
int kdecl ker_signal_waitinfo(THREAD * act, struct kerargs_signal_wait *kap)
{
    ...
    if (sip != NULL) {
        if (!WITHIN_BOUNDRY
            ((uintptr_t) sip, (uintptr_t) sip + sizeof(*sip), act->process->boundry_addr)) {
            return EFAULT;
        }
        ...
        for (pup = pril_first(ph); pup != NULL; pup = pril_next(pup)) {
            ...
            if (SIG_TST(sig_wait, signo)) {
                ...
                if (act->process->debugger
                    && (*debug_thread_signal) (act, signo, pup->code, pup->value, pup->id)) {
