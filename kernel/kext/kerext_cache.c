...
//
static void kerext_cache_one(void *data)
{
    ...
    unsigned flags;
    union {
        unsigned (*_32)(_Paddr32t, unsigned, int, struct cacheattr_entry *,
                        volatile struct syspage_entry *);
        unsigned (*_64)(_Paddr64t, unsigned, int, struct cacheattr_entry *,
                        volatile struct syspage_entry *);
    } control;
    ...
    while (cache_len != 0) {
        ...
        if (flags & (CACHE_FLAG_CTRL_PHYS | CACHE_FLAG_CTRL_PHYS64)) {
            if (memmgr.vaddrinfo(aspaces_prp[KERNCPU], addr, &addr, &valid_len, VI_PGTBL) ==
                PROT_NONE) {
...
static int cache_one(struct cacheattr_entry *cache, void *base, size_t len, int flags)
{
...
static int cache_iterate(int cache_idx, void *base, size_t len, int flags, unsigned term_flag)
{
    ...
    subset_cache = NULL;
    for (;;) {
        if (cache_idx == CACHE_LIST_END)
            break;
        cache = &cache_base[cache_idx];
        if (cache->flags & term_flag)
            break;
