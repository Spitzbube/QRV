...
static void kerext_page_cont(void *data)
{
...
int PageCont(pid_t pid, int tid, int code_signo)
{
...
int PageContErrno(pid_t pid, int tid, int err)
{
    struct kerargs_page_cont data;
...
int PageFaultWait(struct fault_info *info)
{
    int r;

    r = PageWait(info->vaddr, info->sigcode, info->prp->pid, memmgr.fault_pulse_code);
    if (r != EOK) {
        /*
...
static void kerext_page_wait_info(void *data)
{
    ...
    if (id < process_vector.nentries &&
        ...
        id == PINDEX(prp->pid) &&
        (thp = vector_lookup(&prp->threads, SYNC_TID(pt_info))) && (thp->state == STATE_WAITPAGE)) {
