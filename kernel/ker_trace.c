    ...
    case _TRACE_GET_FLAG(_NTO_TRACE_CLREVENTPID):  // Clearing one event process ID
        {
            ...
            case _NTO_TRACE_KERCALLEXIT:
                {
                    if ((*(++c_p)) < ker_call_entry_num && trace_masks.ker_call_tid[*c_p] == NULL) {
            ...
            case _NTO_TRACE_THREAD:
                {
                    ...
                    if (i_2 < _TRACE_MAX_TH_STATE_NUM && trace_masks.thread_tid[i_2] == NULL) {
            ...
            case _NTO_TRACE_VTHREAD:
                {
                    ...
                    if (i_2 < _TRACE_MAX_TH_STATE_NUM && trace_masks.vthread_tid[i_2] == NULL) {
            ...
            case _NTO_TRACE_SYSTEM:
                {
                    if ((*(++c_p)) < _TRACE_MAX_SYSTEM_NUM && trace_masks.system_tid[*c_p] == NULL) {
            ...
            case _NTO_TRACE_COMM:
                {
                    if ((*(++c_p)) < _TRACE_MAX_COMM_NUM && trace_masks.comm_tid[*c_p] == NULL) {
            ...
            case _NTO_TRACE_KERCALL:
                {
                    ...
                    for (i = 0; i < ker_call_entry_num; ++i) {
                        trace_masks.ker_call_masks[i] &=
                            ~(_TRACE_CALL_ARG_WIDE | _TRACE_CALL_RET_WIDE);
                    }
    ...
    case _TRACE_GET_FLAG(_NTO_TRACE_SETEVENTFAST): // Setting one event fast
        {
            ...
            case _NTO_TRACE_KERCALL:
                {
                    if ((*(++c_p)) < ker_call_entry_num) {
                        trace_masks.ker_call_masks[*c_p] &=
                            ~(_TRACE_CALL_ARG_WIDE | _TRACE_CALL_RET_WIDE);
    ...
    case _TRACE_GET_FLAG(_NTO_TRACE_SETRINGMODE):  // Setting ring mode of the kernel
        {
            ...
            if (trace_masks.main_flags || l_b_p == NULL)
                return (ECANCELED);
    ...
    case _TRACE_GET_FLAG(_NTO_TRACE_ALLOCBUFFER):  // Allocating kernel trace buffer
        {
            ...
            **((void ***) ++c_p) = NULL;
             ...
             */
            status = memmgr.mmap(NULL,  /* prp */
                                 0, /* address */
                                 b_n * (_TRACEBUFSIZE), PROT_WRITE | PROT_READ, MAP_PHYS | MAP_PRIVATE | MAP_ANON, NULL,    /* object */
                                 0, /* offset */
                                 0, /* align */
                                 0, /* prealloc */
                                 NOFD, (void *) &ptbuf, &i, mpid = mempart_getid(NULL, sys_memclass_id) // FIX ME - this ok?
                );
            InterruptLock(&trace_masks.state_spin);
            if (trace_masks.buff_0_ptr) {
                InterruptUnlock(&trace_masks.state_spin);
                (void) memmgr.munmap(NULL, (uintptr_t) ptbuf,
                                     (trace_masks.buff_num * (_TRACEBUFSIZE)), 0, mpid);
                return (EEXIST);    // buffer exist
            }
            ...
            **((_Paddr32t **) c_p) = (_Paddr32t) privateptr->tracebuf;
#else
            {
                ...
                (void) memmgr.vaddrinfo(aspaces_prp[KERNCPU], (uintptr_t) privateptr->tracebuf,
                                        &paddr, NULL, VI_NORMAL);
                **((_Paddr32t **) c_p) = paddr;
            }
#endif
    ...
    case _TRACE_GET_FLAG(_NTO_TRACE_DEALLOCBUFFER):    // Deallocating trace buffer
        {
            if (privateptr->tracebuf) {
                ...
                (void) memmgr.munmap(NULL, (uintptr_t) b_0_p,
                                     (trace_masks.buff_num * (_TRACEBUFSIZE)), 0,
