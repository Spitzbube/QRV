...
int kdecl ker_connect_detach(THREAD * act, struct kerargs_connect_detach *kap)
{
    ...
    if (cop->flags & COF_NETCON) {
        ...
        for (i = 0; i < vthread_vector.nentries; ++i) {
            ...
            if (cop1 != cop) {
                if (!(cop1->flags & COF_VCONNECT))
                    continue;
                if (cop1->un.lcl.cop != cop)
                    continue;
            }
        ...
        if ((act->process == net.prp)
            && ((cop->channel && cop->channel->process != net.prp) || (cop->scoid != coid))) {
            prio = act->priority;
            if (cop->channel) {
    ...
    for (;;) {
        while ((cop = vector_search(vec, coid, &coid))) {
            if (cop->type == TYPE_CONNECTION && !((chp = cop->channel) &&
                                                  chp->process == prp && coid == cop->scoid
                                                  && vec == &prp->chancons)) {
...
#error ENDIAN Not defined for system
        ...
        memcpy(&clp->cred, cip,
               offsetof(struct _cred_info, grouplist) + ngroups * sizeof(clp->cred.grouplist[0]));
