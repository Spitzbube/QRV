...
static
 ...
 __inline__
#endif
 CONNECT * rdecl inline_lookup_connect(PROCESS * prp, int coid)
{
    VECTOR *vec;
    CONNECT *cop;
...
int kdecl ker_msg_sendv(THREAD * act, struct kerargs_msg_sendv *kap)
{
    ...
    if (chp->flags & (_NTO_CHF_ASYNC | _NTO_CHF_GLOBAL)) {
        if (chp->flags & _NTO_CHF_GLOBAL) {
            return msgsend_gbl(act, cop, kap->smsg, -kap->sparts, (unsigned) -kap->rparts,
                               kap->coid);
        } else {
    ...
    if (cop->flags & COF_NETCON) {
        ...
        sender =
            (THREAD *) (void *) net_send1(kap->rparts, (struct _vtid_info *) (void *) kap->rmsg);
        if (sender == NULL) {
            return EINVAL;
        }
        if (sender->state != STATE_STOPPED)
            crash();
    ...
    if ((thp != NULL) && !(thp->internal_flags & _NTO_ITF_RCVPULSE)) {
        ...
        if (th_flags == 0) {
            ...
            if ((xferstat =
                 xfer_cpy_diov(thp, thp->args.ri.rmsg, sender->args.msbuff.buff,
                               thp->args.ri.rparts, sender->args.msbuff.msglen))) {
                lock_kernel();
        ...
        if ((sender->timeout_flags == 0) &&
            ...
            !(chp->flags & _NTO_CHF_FIXED_PRIORITY) && !IS_SCHED_SS(sender)) {
...
int kdecl ker_msg_sendpulse(THREAD * act, struct kerargs_msg_sendpulse *kap)
{
    ...
    if (!(src->info.euid == 0 ||
          ...
          src->info.euid == dst->info.ruid || src->info.euid == dst->info.suid)) {
        return EPERM;
    }
...
#endif
    if ((thp != NULL) && (thp->aspace_prp == aspaces_prp[KERNCPU])
        && (pril_first(&chp->send_queue) == NULL)) {
        ...
        // Yes. Transfer the data.
        status =
            xferpulse(thp, thp->args.ri.rmsg, thp->args.ri.rparts, kap->code, kap->value,
...
int kdecl ker_msg_receivev(THREAD * act, struct kerargs_msg_receivev *kap)
{
    ...
    if ((chp = vector_lookup(chvec, chid)) == NULL || chp->type != TYPE_CHANNEL) {
        lock_kernel();
        return ESRCH;
    }
    ...
    if (chp->flags & (_NTO_CHF_ASYNC | _NTO_CHF_GLOBAL)) {
        if (chp->flags & _NTO_CHF_GLOBAL) {
            ...
            return msgreceive_gbl(act, (CHANNELGBL *) chp, kap->rmsg, -kap->rparts, kap->info, cop,
                                  kap->coid);
        } else {
    ...
    if (kap->rparts >= 0) {
        ...
        if (kap->rparts != 0) {
            if (!WITHIN_BOUNDRY
                ((uintptr_t) iov, (uintptr_t) (&iov[rparts]), act->process->boundry_addr)) {
                return EFAULT;
            }
        }
  ...
  restart2:
    if (thp) {
        ...
        // By default the receiver runs with message driven priority.
        // RUSH: Fix for partition inheritance
        if ((act->priority != thp->priority || act->dpp != thp->dpp)
            && (chp->flags & _NTO_CHF_FIXED_PRIORITY) == 0) {
            AP_INHERIT_CRIT(act, thp);
            adjust_priority(act, thp->priority, thp->dpp, 1);
            if (act->real_priority != act->priority)
                act->real_priority = act->priority;
        } else {
            AP_CLEAR_CRIT(act);
        }
    ...
    if (KTYPE(act) == __KER_MSG_RECEIVEPULSEV) {
        ...
        for (;;) {
            ...
            if (thp == NULL)
                break;
            if (thp->internal_flags & _NTO_ITF_RCVPULSE)
                break;
    ...
    if (kap->sparts != 0) {
        ...
        if (kap->sparts < 0) {
            ...
            if (len <= sizeof(thp->args.msbuff.buff)) {
                if ((xferstat =
                     xfer_memcpy(thp->args.msbuff.buff, (char *) base, thp->args.msbuff.msglen =
                                 len))) {
                    goto xfer_err;
                }
            ...
            if (len <= sizeof(thp->args.msbuff.buff)) {
                if ((xferstat =
                     xfer_memcpy(thp->args.msbuff.buff, (char *) base, thp->args.ms.msglen =
                                 len))) {
                    goto xfer_err;
                }
                flags = QRV_FLG_THR_BUFF_MSG;
            ...
            if (len <= sizeof(thp->args.msbuff.buff)) {
                ...
                while (sparts) {
                    if ((xferstat =
                         xfer_memcpy(&thp->args.msbuff.buff[pos], GETIOVBASE(iov),
                                     GETIOVLEN(iov)))) {
                        goto xfer_err;
                    }
        ...
        if (flags == 0) {
            ...
            if (thp->aspace_prp == NULL) {
                xferstat =
                    xfer_cpy_diov(thp, thp->args.ri.rmsg, thp->args.msbuff.buff,
                                  thp->args.ri.rparts, thp->args.msbuff.msglen);
                lock_kernel();
            } else {
                xferstat = 0;
                lock_kernel();
...
int kdecl ker_msg_error(THREAD * act, struct kerargs_msg_error *kap)
{
    ...
    if (kap->err == ERESTART) {
        ...
        if (_TRACE_GETSYSCALL(thp->syscall) == __KER_CHANNEL_DESTROY) {
             ...
             */
            if (kap->err != EOK) {
                KerextSlogf(_SLOG_SETCODE(_SLOGC_QNET, 0), _SLOG_INFO,
                            "Qnet ChannelDestroy failed %d", kap->err);
            }
        } else {
            kererr(thp, kap->err);
        }
