...
void set_factors()
{
    ...
    for (ap = 0; ap < num_ppg; ap++) {
        if (actives_ppg[ap]->budget_in_percent)
            total_product *= actives_ppg[ap]->budget_in_percent;
    }
...
#define IS_COMPETING(ppg, min_cycles_to_schedule) (((ppg)->used_cycles + (min_cycles_to_schedule)) <= ((ppg)->std_budget_in_cycles))
    ...
    for (retry_count = 0; retry_count <= 1; retry_count++) {
        ...
        if (pcur_candidate_thread) {
            ...
            pbest_thread =
                (prio > pcur_candidate_thread->priority) ? DISPATCH_THP((DISPATCH *) pap,
                                                                        prio) :
                pcur_candidate_thread;
            ...
            if (pbest_thread->priority) {
                best_runable = MAY_SCHEDULE_MIN(pap, min_to_schedule)
                    || THREAD_IS_RUNNING_CRITICAL(pbest_thread);
        ...
        for (ap = 0; ap < num_ppg; ap++) {
            ...
            if (NUM_PROCESSORS > 1) {
                ...
                while ((ptrd->runmask & (1 << cpu)) != 0) {
                    ...
                    if (ptrd == NULL) {
                        ...
                        while ((--prio > 0)
                               && (ptrd = DISPATCH_THP((DISPATCH *) pap, prio)) == NULL) {
                            //nothing to do
                        }
                        if (!prio)
                            break;
            ...
            competing_ap_set |= (uint32_t) (IS_COMPETING(pap, min_to_schedule)
                                            || THREAD_IS_RUNNING_CRITICAL(ptrd)) << ap;
        }
...
#if aps_debugging
        {
            ...
            if (++counter > 15000) {
                counter = 0;
                disp("  rsm:");
                disp_int(runable_ap_set);
                disp("  rsa:");
                disp_int(runable_any_cpu_set);
                disp(" cs:");
                disp_int(competing_ap_set);
                disp(" sl:");
                disp_int(sleeping_ap_set);
                disp(" u:");
                disp_int(underload);
                disp(" a:");
                disp_int(all_at_limit_bmp);
                disp("-");
                disp_int(all_at_limit);
                disp(" f:");
                disp_int(free_time);
                disp(" pe:");
                disp_int(prio_enable);
            }
        }
#endif
        ...
        if (pbest_thread && THREAD_IS_RUNNING_CRITICAL(pbest_thread)
            ...
            competing_ap_set |=
                (uint32_t) IS_COMPETING((PPG *) (act->dpp), min_to_schedule) << (act->dpp->id);
        ...
        if (start_window_rotations == sched_window_rotations) {
            ...
            if (pbest_thread)
                UPD_BILL_AS_CRIT(pbest_thread, competing_ap_set != 0)
                    if (prun_as_crit)
                    *prun_as_crit = (competing_ap_set != 0);
    ...
    if (ker_verbose >= 3)
        kprintf("APS:2 clock intrs while choosing\n");
    return pbest_thread;
...
THREAD *choose_between(THREAD * a, THREAD * b)
{
    ...
    if (b->priority > a->priority)
        return b;
    if (RELATIVE_FRACTION_USED((PPG *) (a->dpp)) < RELATIVE_FRACTION_USED((PPG *) (b->dpp)))
        return a;
    return b;
}
