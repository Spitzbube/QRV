...
int configure_bmp_support()
{
    ...
    if (scheduling_policy_flags & SCHED_APS_SCHEDPOL_BMP_SAFETY)
        scheduling_policy_flags |= SCHED_APS_SCHEDPOL_FREETIME_BY_RATIO;
...
void update_zerobudget_ap_set(void)
{
    ...
    zeroap_set = 0;
    for (i = 0; i < num_ppg; i++)
        zeroap_set |= (uint32_t) (0 == actives_ppg[i]->budget_in_percent) << i;
...
int change_percent_budget(PPG * ppg, int new_budget_percent)
{
    ...
    if (ppg->parent) {
        ...
        if (percent_delta > ppg->parent->budget_in_percent)
            return -1;
        ppg->parent->budget_in_percent -= percent_delta;
        ppg->parent->bmp_budget_in_cycles =
            (bmp_support_factor * windowsize_in_cycles * ppg->parent->budget_in_percent) / 100;
        ppg->parent->std_budget_in_cycles =
            (NUM_PROCESSORS * windowsize_in_cycles * ppg->parent->budget_in_percent) / 100;
        // no need to change ppg->parent->used_cycles;
    }
    ppg->budget_in_percent = new_budget_percent;
    ppg->bmp_budget_in_cycles =
        (bmp_support_factor * windowsize_in_cycles * ppg->budget_in_percent) / 100;
    ppg->std_budget_in_cycles =
        (NUM_PROCESSORS * windowsize_in_cycles * ppg->budget_in_percent) / 100;
...
int create_ppg(sched_aps_create_parms * parms, int max_prio)
{
    ...
    if (parent->budget_in_percent < budget)
        return -EDQUOT;

    /* check if this create will result in the parent becomming zero budget */
    if (aps_security & SCHED_APS_SEC_NONZERO_BUDGETS && budget == parent->budget_in_percent)
        return -EACCES;
    ...
    if (new->critical_budget_in_cycles > NUM_PROCESSORS * windowsize_in_cycles)
        new->critical_budget_in_cycles = NUM_PROCESSORS * windowsize_in_cycles;
    ...
    trace_emit_sys_aps_budgets(new->dpp.id, new->budget_in_percent,
                               new->critical_budget_in_cycles / cycles_per_ms);
    return new->dpp.id;
}
...
int aps_name_to_id(char *name)
{
    ...
    if (name && *name) {
        for (ap = 0; ap < num_ppg; ap++)
            if (0 == strcmp(actives_ppg[ap]->name, name))
                return ap;
    }
    return -1;
...
int aps_set_parms(sched_aps_parms * parms)
{
    ...
    if (parms->scheduling_policy_flagsp) {
        ...
        if ((SCHED_APS_SCHEDPOL_BMP_SAFETY & old_spf) !=
            (SCHED_APS_SCHEDPOL_BMP_SAFETY & scheduling_policy_flags)) {
...
int aps_get_partition_info(sched_aps_partition_info * part_info)
{
    ...
    if (part_info->id < 0 || part_info->id >= num_ppg)
        return EINVAL;
    ...
    if (ppg->bankruptcy_notifier.sigev_notify != SIGEV_NONE)
        part_info->pinfo_flags |= SCHED_APS_PINFO_BANKRUPTCY_NOTIFY_ARMED;
    if (ppg->overload_notifier.sigev_notify != SIGEV_NONE)
        part_info->pinfo_flags |= SCHED_APS_PINFO_OVERLOAD_NOTIFY_ARMED;
    part_info->pid_at_last_bankruptcy = ppg->bnkr_info.last_suspect_pid;
    //convert tid to external range
    part_info->tid_at_last_bankruptcy =
        ppg->bnkr_info.last_suspect_tid == -1 ? -1 : ppg->bnkr_info.last_suspect_tid + 1;
    return EOK;
}
...
int aps_modify_partition(sched_aps_modify_parms * parms)
{
    ...
    if (parms->id < 0 || parms->id >= num_ppg)
        return EINVAL;
    ...
    if (parms->new_budget_percent != -1) {
        if (-1 == change_percent_budget(ppg, parms->new_budget_percent))
            return EINVAL;
        set_factors();
    }
    ...
    if (parms->new_critical_budget_ms != -1) {
        ...
        ppg->critical_budget_in_cycles =
            (new_crit_cycles <=
             NUM_PROCESSORS * windowsize_in_cycles) ? new_crit_cycles : NUM_PROCESSORS *
            windowsize_in_cycles;
        remaining_bankruptcy_grace = max_bankruptcy_grace;
        //@@@ APS should also delay any notification of that partition being overloaded.
    }
    trace_emit_sys_aps_budgets(ppg->dpp.id, ppg->budget_in_percent,
                               ppg->critical_budget_in_cycles / cycles_per_ms);
...
int aps_get_partition_stats(sched_aps_partition_stats * part_stats, int n_aps_to_read)
{
    ...
    if (part_stats->id < 0 || part_stats->id >= num_ppg)
        return EINVAL;
    start_ap = part_stats[0].id;
    last_ap_plus1 = start_ap + n_aps_to_read;
    if (last_ap_plus1 > num_ppg)
        last_ap_plus1 = num_ppg;
    ...
    for (retry_counter = 0; retry_counter <= 1; retry_counter++) {
        ...
        for (i = start_ap; i < last_ap_plus1; i++) {
            ...
            if (IS_BANKRUPT(ppg))
                p_stats->stats_flags |= SCHED_APS_PSTATS_IS_BANKRUPT_NOW;   //this doent yet check if notification sent
            if (ppg->state_flags & PPG_STATE_WAS_BANKRUPT)
                p_stats->stats_flags |= SCHED_APS_PSTATS_WAS_BANKRUPT;
            p_stats->id = i;
        }
    ...
    if (ker_verbose >= 3)
        kprintf("APS:2 clock intrs getting stats\n");
...
int aps_get_overall_stats(sched_aps_overall_stats * parms)
{
    ...
    parms->tid_at_last_bankruptcy =
        last_bankrupter.last_suspect_tid == -1 ? -1 : last_bankrupter.last_suspect_tid + 1;
...
int aps_get_thread_info(THREAD * thp, sched_aps_query_thread_parms * parms)
{
    ...
    parms->crit_state_flags |=
        (thp->sched_flags & AP_SCHED_RUNNING_CRIT) ? APS_QCRIT_RUNNING_CRITICAL : 0;
    parms->crit_state_flags |=
        (thp->sched_flags & AP_SCHED_BILL_AS_CRIT) ? APS_QCRIT_BILL_AS_CRITICAL : 0;
    return EOK;
}
...
int aps_attach_events(sched_aps_events_parm * parms)
{
    ...
    if (parms->bankruptcy_notification) {
        memcpy(&(ppg->bankruptcy_notifier), parms->bankruptcy_notification,
               sizeof(ppg->bankruptcy_notifier));
    }
    if (parms->overload_notification) {
        memcpy(&(ppg->overload_notifier), parms->overload_notification,
               sizeof(ppg->overload_notifier));
    };
