...
int reinit_window_parms(int new_windowsize_ms)
{
    ...
    if (new_windowsize_ms < MIN_WINDOWSIZE_MS || new_windowsize_ms > MAX_WINDOWSIZE_MS)
        return EINVAL;
    ...
    /* Calculate cycles per bucket. Read the current ticksize, in case user has changed it. */
    {
        ...
        cycles_per_bucket =
            CONDITION_CC(SYSPAGE_ENTRY(qtime)->cycles_per_sec) / ((uint64_t) 1000000000 /
                                                                  tick_nsec);
    }
    ...
    if (new_ts_buckets > MAX_TS_BUCKETS || new_ts_buckets < MIN_TS_BUCKETS)
        return EINVAL;
...
DISPATCH * create_default_dispatch(void)
{
    ...
    if (actives_lrw[APS_SYSTEM_PARTITION_ID] == NULL)
        crash();
    ...
    trace_emit_sys_aps_budgets(APS_SYSTEM_PARTITION_ID, system_ppg->budget_in_percent,
                               system_ppg->critical_budget_in_cycles / cycles_per_ms);
