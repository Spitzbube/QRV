...
void rdecl mark_running_ppg(THREAD * new)
{
    THREAD *old = NULL;
...
#endif
    if (new->priority == 0 && old->state == STATE_READY)
        crash();
}
...
int rdecl select_cpu_ppg(THREAD * thp)
{
    THREAD *act;
    int i, j, cpu;
    unsigned minprio;
...
static void rdecl yield_ppg(void)
{
...
static void rdecl resched_ppg(void)
{
    THREAD *act = actives[KERNCPU];
    THREAD *thp;
    DISPATCH *dpp;
    ...
    if (adjusted) {
        ...
        if (MAY_SCHEDULE((PPG *) (actives[KERNCPU]->dpp)))
            return;
...
static void rdecl ready_ppg(THREAD * thp)
{
    THREAD *act;
    DISPATCH *dpp;
    ...
    if ((prio > act->priority) &&
        ...
        (MAY_SCHEDULE((PPG *) thp->dpp) ||
         ...
         THREAD_IS_RUNNING_CRITICAL(thp) || act->priority == 0)) {
        mark_running(thp);
        thp = act;
        ...
        if (NUM_PROCESSORS > 1) {
            ...
            if (STATE_LAZY_RESCHED(thp) && (prio <= act->priority)
                && (thp->runmask & (1 << KERNCPU)) == 0) {
...
static void rdecl block_and_ready_ppg(THREAD * thp)
{
    THREAD *act = actives[KERNCPU];
    DISPATCH *dpp;
    uint8_t prio;
    ...
    if ((prio > act->priority) &&
        ((thp->runmask & (1 << KERNCPU)) == 0) &&
        (MAY_SCHEDULE((PPG *) thp->dpp) || THREAD_IS_RUNNING_CRITICAL(thp))) {
        mark_running(thp);
        chk_dispatch(1);
        return;
    }
...
static int rdecl may_thread_run_ppg(THREAD * thp)
{
    return MAY_SCHEDULE((PPG *) thp->dpp);
}

static void rdecl debug_moduleinfo_ppg(PROCESS * prp, THREAD * thp, void *dbg)
{
...
static DISPATCH *init_optional_scheduler()
{
    ...
    if (EOK != reinit_window_parms(DEFAULT_WINDOWSIZE_MS))
        crash();
...

    {
        ...
        info.id = id;
        if (aps_get_partition_info(&info) != EOK)
            crash();
        ...
        if (prio > act->priority) {
            ...
            if (((PPG *) thp->dpp)->remaining_cycles > 0) {
                kprintf("inversion? prio %d %d cyc %d line %d\n",
                        prio, act->priority, ((PPG *) thp->dpp)->remaining_cycles, line);
                kprintf("ret %x", __builtin_return_address(0));
                kprintf(" %x", __builtin_return_address(1));
                kprintf(" %x", __builtin_return_address(2));
                kprintf(" %x\n", __builtin_return_address(3));
