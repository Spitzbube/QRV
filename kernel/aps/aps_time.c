...
void microbill(THREAD * thp)
{
    ...
    //this tick.
    if (cycles_left_in_tick > cycles_per_bucket)
        cycles_left_in_tick = 0;
...
int rdecl ppg_tick_hook(void)
{
    ...
    if (remaining_bankruptcy_grace)
        remaining_bankruptcy_grace--;
    ...
    if (cur_hist_index + 1 >= ts_buckets) {
        ...
        if (hist_index_w2 >= MAX_W2_BUCKETS) {
            ...
            hist_index_w3++;
            if (hist_index_w3 >= MAX_W3_BUCKETS)
                hist_index_w3 = 0;
        }
    ...
    if (NUM_PROCESSORS > 1) {
        ...
        return (!MAY_SCHEDULE((PPG *) (act->dpp)) && !THREAD_IS_RUNNING_CRITICAL(act))
            || bankruptcy_detected;
    }
}
