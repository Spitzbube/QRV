 ...
 */
    ...
    if (!prp || !tid && act->process != prp)
        return NULL;

    if ((uint32_t) - 1 == tid)
        return prp->valid_thp;
    if (0 == tid)
        return act;
...
int partition_name_ok(char *name)
{
    ...
    for (i = 0; i < l; i++) {
        if (strchr(name, bad_partition_name_chars[i]))
            return 0;
    }
    return 1;
}
...
int kdecl ker_sched_ctl(THREAD * act, struct kerargs_sched_ctl *kap)
{
    ...
    case SCHED_QUERY_SCHED_EXT:{
            ...
            if (kap->length != sizeof(struct sched_query))
                return EINVAL;
    ...
    case SCHED_APS_QUERY_PARMS:{
            ...
            if (kap->length != sizeof(sched_aps_info))
                return EINVAL;
            WR_VERIFY_PTR(act, parms, sizeof(*parms));
            WR_PROBE_INT(act, parms, sizeof(*parms) / sizeof(int));
            if BAD_RESV2
    ...
    case SCHED_APS_SET_PARMS:{
            ...
            if (FAIL_ROOT0_OVERALL(act) || FAIL_PARTITIONS_LOCKED)
                return EACCES;
            parms = (sched_aps_parms *) kap->data;
            if (kap->length != sizeof(sched_aps_parms))
                return EINVAL;
            RD_VERIFY_PTR(act, parms, sizeof(*parms));
            RD_PROBE_INT(act, parms, sizeof(*parms) / sizeof(int));
            if (parms->bankruptcy_policyp) {
                RD_VERIFY_PTR(act, parms->bankruptcy_policyp, sizeof(*(parms->bankruptcy_policyp)));
                RD_PROBE_INT(act, parms->bankruptcy_policyp,
                             sizeof(*(parms->bankruptcy_policyp)) / sizeof(int));
            }
            if (parms->scheduling_policy_flagsp) {
                RD_VERIFY_PTR(act, parms->scheduling_policy_flagsp,
                              sizeof(*(parms->scheduling_policy_flagsp)));
                RD_PROBE_INT(act, parms->scheduling_policy_flagsp,
                             sizeof(*(parms->scheduling_policy_flagsp)) / sizeof(int));
            }
            if BAD_RESV3
                (parms) return EDOM;
            KerextLock();
    ...
    case SCHED_APS_CREATE_PARTITION:{
            ...
            if (kap->length != sizeof(sched_aps_create_parms))
                return EINVAL;
            WR_VERIFY_PTR(act, parms, sizeof(*parms));
            WR_PROBE_INT(act, parms, sizeof(*parms) / sizeof(int));
            if (parms->name != NULL) {
                RD_VERIFY_PTR(act, parms->name, 1);
                if (strlen(parms->name) > APS_PARTITION_NAME_LENGTH)
                    return ENAMETOOLONG;
                if (!partition_name_ok(parms->name))
                    return EINVAL;
            }
            //aps_name_to_id only works if names are untruncated
            if (FAIL_PARTITIONS_LOCKED || FAIL_ROOT_MAKES_PARTITIONS(act)
                || FAIL_SYS_MAKES_PARTITIONS(act)
                || FAIL_NONZERO_BUDGETS(parms->budget_percent)
                || FAIL_ROOT_MAKES_CRITICAL(act, parms->critical_budget_ms)
                || FAIL_SYS_MAKES_CRITICAL(act, parms->critical_budget_ms))
                return EACCES;
            //critical budget of -1, means "not set", but reject other negative values.
            if (parms->critical_budget_ms < -1)
                return EINVAL;
            if BAD_RESV3
                (parms) return EDOM;
            if ((parms->aps_create_flags & APS_CREATE_FLAGS_USE_PARENT_ID)
                && (parms->parent_id >= num_ppg))
                return EINVAL;
            KerextLock();
            if (parms->name && -1 != aps_name_to_id(parms->name))
                return EEXIST;
    ...
    case SCHED_APS_LOOKUP:{
            ...
            if (kap->length != sizeof(sched_aps_lookup_parms))
                return EINVAL;
            WR_VERIFY_PTR(act, parms, sizeof(*parms));
            WR_PROBE_INT(act, parms, sizeof(*parms) / sizeof(int));
            if (parms->name == NULL)
    ...
    case SCHED_APS_QUERY_PARTITION:{
            ...
            if (kap->length != sizeof(sched_aps_partition_info))
                return EINVAL;
            WR_VERIFY_PTR(act, parms, sizeof(*parms));
            WR_PROBE_INT(act, parms, sizeof(*parms) / sizeof(int));
            if BAD_RESV2
    ...
    case SCHED_APS_JOIN_PARTITION:{
            ...
            if (kap->length != sizeof(sched_aps_join_parms))
                return EINVAL;
            RD_VERIFY_PTR(act, parms, sizeof(*parms));
            RD_PROBE_INT(act, parms, sizeof(*parms) / sizeof(int));
            if (FAIL_ROOT_JOINS(act)
                ...
                || FAIL_JOIN_SELF_ONLY(parms->pid, parms->tid))
                return EACCES;
            if BAD_RESV2
                (parms) return EDOM;
            KerextLock();
    ...
    case SCHED_APS_MODIFY_PARTITION:{
            ...
            if (kap->length != sizeof(sched_aps_modify_parms))
                return EINVAL;
            RD_VERIFY_PTR(act, parms, sizeof(*parms));
            RD_PROBE_INT(act, parms, sizeof(*parms) / sizeof(int));
            if (FAIL_PARTITIONS_LOCKED || FAIL_PARENT_MODIFIES(act, parms->id)
                ...
                FAIL_SYS_CHANGES_CRITICAL(act, parms->new_critical_budget_ms))
                return EACCES;
            //checkthat requested budget is valid. -1 is valid, it means "no change"
            if (parms->new_budget_percent < -1 || parms->new_budget_percent > 100)
                return EINVAL;
    ...
    case SCHED_APS_PARTITION_STATS:{
            ...
            if (kap->length == 0 || (kap->length % sizeof(sched_aps_partition_stats) != 0))
                return EINVAL;
    ...
    case SCHED_APS_OVERALL_STATS:{
            ...
            if (kap->length != sizeof(sched_aps_overall_stats))
                return EINVAL;
            WR_VERIFY_PTR(act, parms, sizeof(*parms));
            WR_PROBE_INT(act, parms, sizeof(*parms) / sizeof(int));
            if BAD_RESV4
    ...
    case SCHED_APS_MARK_CRITICAL:{
            ...
            if (kap->length != sizeof(sched_aps_mark_crit_parms))
                return EINVAL;
            RD_VERIFY_PTR(act, parms, sizeof(*parms));
            RD_PROBE_INT(act, parms, sizeof(*parms) / sizeof(int));
            if BAD_RESV1
    ...
    case SCHED_APS_CLEAR_CRITICAL:{
            ...
            if (kap->length != sizeof(sched_aps_clear_crit_parms))
                return EINVAL;
            RD_VERIFY_PTR(act, parms, sizeof(*parms));
            RD_PROBE_INT(act, parms, sizeof(*parms) / sizeof(int));
            if BAD_RESV1
            ...
            if (!thp)
                return ESRCH;
            thp->sched_flags &=
                ~(AP_SCHED_PERM_CRIT | AP_SCHED_RUNNING_CRIT | AP_SCHED_BILL_AS_CRIT);
        }
        break;
    ...
    case SCHED_APS_QUERY_THREAD:{
            ...
            if (kap->length != sizeof(sched_aps_query_thread_parms))
                return EINVAL;
            WR_VERIFY_PTR(act, parms, sizeof(*parms));
            WR_PROBE_INT(act, parms, sizeof(*parms) / sizeof(int));
            if BAD_RESV2
    ...
    case SCHED_APS_ADD_SECURITY:{
            ...
            if (kap->length != sizeof(sched_aps_security_parms))
                return EINVAL;
            WR_VERIFY_PTR(act, parms, sizeof(*parms));
            WR_PROBE_INT(act, parms, sizeof(*parms) / sizeof(int));
            if BAD_RESV2
    ...
    case SCHED_APS_QUERY_PROCESS:{
            ...
            if (kap->length != sizeof(sched_aps_query_process_parms))
                return EINVAL;
            WR_VERIFY_PTR(act, parms, sizeof(*parms));
            WR_PROBE_INT(act, parms, sizeof(*parms) / sizeof(int));
            if BAD_RESV4
