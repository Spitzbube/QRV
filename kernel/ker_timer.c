...
int kdecl ker_timer_create(THREAD * act, struct kerargs_timer_create *kap)
{
    ...
    if (kap->event != NULL) {
        ...
        if (SIGEV_GET_TYPE(&tip->event) == SIGEV_PULSE) {
            ...
            if ((cop = lookup_connect(tip->event.sigev_coid)) == NULL
                || cop->type != TYPE_CONNECTION || (chp = cop->channel) == NULL) {
                timer_free(prp, tip);
                return EBADF;
            }
...
int kdecl ker_timer_info(THREAD * act, struct kerargs_timer_info *kap)
{
    ...
    // Make sure timer is valid.
    id = kap->id;
    tip = vector_search(&prp->timers, id, (kap->flags & _NTO_TIMER_SEARCH) ? &id : NULL);
    if (tip == NULL) {
        return EINVAL;
    }
