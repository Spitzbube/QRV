...
static void create_sigchld(PROCESS * prp, int status)
{
    uint64_t runtime;
    uint64_t systime;
...
int kerop_thread_ctl(THREAD * act, THREAD * op, struct kerargs_thread_ctl *kap)
{
    ...
    case _NTO_TCTL_RUNMASK_GET_AND_SET_INHERIT:
         ...
         */
        RD_VERIFY_PTR(act, kap->data, sizeof(struct _thread_runmask) + 2 * num * sizeof(unsigned));
...
#error a shift of 0.
        ...
        if (new[0] != 0)
            set_rmask = 1;
#endif
        WR_VERIFY_PTR(act, kap->data, sizeof(struct _thread_runmask) + 2 * num * sizeof(unsigned));
        WR_PROBE_INT(act, kap->data, sizeof(struct _thread_runmask) / sizeof(int) + 2 * num);
