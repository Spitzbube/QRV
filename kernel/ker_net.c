...
int kdecl ker_net_cred(THREAD * act, struct kerargs_net_cred *kap)
{
    ...
    if ((cop = lookup_connect(kap->coid)) == NULL ||
        cop->type != TYPE_CONNECTION || (cop->flags & COF_NETCON) == 0 || act->process != net.prp) {
        return EBADF;
    }
    ...
    if ((cip = kap->info)) {
        ...
        size = offsetof(CLIENT, cred.grouplist) + cip->cred.ngroups * sizeof(cip->cred.grouplist);
...
int kdecl ker_net_unblock(THREAD * act, struct kerargs_net_unblock *kap)
{
    ...
    if (act->process != net.prp || (vthp = vector_lookup(&vthread_vector, kap->vtid)) == NULL) {
        return ESRCH;
    }
...
int kdecl ker_net_infoscoid(THREAD * act, struct kerargs_net_infoscoid *kap)
{
    ...
    if (act->process != net.prp ||
        ...
        (cop->type != TYPE_CONNECTION) || (cop->process == NULL)) {
        return ESRCH;
    }
...
int kdecl ker_net_signal_kill(THREAD * act, struct kerargs_net_signal_kill *kap)
{
    ...
    if (!(src->euid == 0 ||
          src->ruid == dst->ruid ||
          src->ruid == dst->suid || src->euid == dst->ruid || src->euid == dst->suid)) {
        return EPERM;
    }
    // send the signal
