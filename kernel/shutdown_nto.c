...
static void dump_mem32(const char *name, const uint32_t * p, unsigned size)
{
    unsigned i;
...
void dump_tracering(void)
{
    ...
    if (trace_masks.main_flags != 0) {
        ...
        if ((t_b_p = privateptr->tracebuf) != NULL) {
            ...
            do {
                if (t_b_p->h.num_events > 0 && !(t_b_p->h.flags & _TRACE_FLAGS_WRITING)) {
                    for (i = 0, e_p = (traceevent_t *) t_b_p->h.begin_ptr; e_p < t_b_p->h.tail_ptr;
                         e_p++, i++) {
                        kprintf("%8x %8x %8x %8x%s", e_p->header, e_p->data[0], e_p->data[1],
                                e_p->data[2], (i % 2) ? "\n" : " ");
                    }
                    kprintf("\n");
                }
