...
ker_deliver_event (void *data)
{
  ...
  if ((prp != NULL) &&
      ((thp =
	vector_lookup (&prp->threads,
		       MTINDEX (kap->evtdest->rcvid))) != NULL))
    {
      KerextLock ();
...
int
apm_deliver_event (evtdest_t * evtdest, struct sigevent *se)
{
