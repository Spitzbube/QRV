...
static void intrevent_alloc(int num, int system_running)
{
    int i;
    INTREVENT *irp;
    INTREVENT *head;
 ...
 */
static int intrevent_error(THREAD * thp, INTERRUPT * isr)
{
    INTREVENT *curr_intr;
    INTREVENT **owner;
    PROCESS *curr_proc;
...
#define ARRAY_EL(e)
            ...
            static const char *const fmt[] = {
                ...
                ARRAY_EL(SIGEV_SIGNAL_CODE)
                    "t-%02u:  SIGEV_SIGNAL_CODE(%u) -> %u (%s)  (0x%x/0x%x/0x%x/--)\n",
                ARRAY_EL(SIGEV_SIGNAL_THREAD)
                    "t-%02u:  SIGEV_SIGNAL_THREAD(%u) -> %u (%s)  (0x%x/0x%x/0x%x/--)\n",
                ARRAY_EL(SIGEV_PULSE)
                    "t-%02u:  SIGEV_PULSE(%u) -> %u (%s)  (0x%x/0x%x/0x%x/0x%x)\n",
            ...
            for (i = 0; i < (2 * MIN_INTRS_BAD); i++, irplocal = irplocal->next)
                kprintf(fmt[SIGEV_GET_TYPE(&irplocal->event) % NUM_ELTS(fmt)],
                        ...
                        (irplocal->thread
                         && irplocal->thread->process) ? irplocal->thread->process->pid : 0,
                        (irplocal->thread
                         && irplocal->thread->process) ? irplocal->thread->process->
                        debug_name : "?", irplocal->event.sigev_signo,
                        irplocal->event.sigev_value.sival_int, irplocal->event.sigev_code,
                        irplocal->event.sigev_priority);
        }
        return 0;
    }
    ...
    if ((killer == NULL) || (high_intrs_proc == NULL))
        crash();
...
void intrevent_preemption(unsigned prio)
{
    if (!nopreempt) {
        INTR_LOCK(&intrevent_lock);
...
int intrevent_add_attr intrevent_add(const struct sigevent *evp, THREAD * thp, INTERRUPT * isr)
{
    ...
    if (--num_pev_free <= num_pev_trigger) {
        if (drain_active) {
            ...
            if (drain_last_mask < level)
                drain_last_mask = level;
        }
...
void intrevent_init(unsigned num)
{
    if (num == 0)
        num = 200;              // Set default
...
int rdecl sigevent_exe(const struct sigevent *evp, THREAD * thp, int deliver)
{
    ...
    case SIGEV_SIGNAL_THREAD:
        ...
        if (deliver) {
            ...
            (void) signal_kill_thread(prp,
                                      ...
                                      prp->pid,
                                      evp->
                                      sigev_notify & SIGEV_FLAG_CRITICAL ?
                                      SIGNAL_KILL_APS_CRITICAL_FLAG : 0);
        }
        break;
    ...
    case SIGEV_SIGNAL:
        ...
        if (deliver) {
            ...
            signal_kill_process(prp,
                                ...
                                prp->pid,
                                evp->
                                sigev_notify & SIGEV_FLAG_CRITICAL ? SIGNAL_KILL_APS_CRITICAL_FLAG :
                                0);
        }
        break;
    ...
    case SIGEV_THREAD:
        ...
        if (!deliver || (status = thread_create(thp, thp->process, evp,
                                                evp->
                                                sigev_notify & SIGEV_FLAG_CRITICAL ?
                                                THREAD_CREATE_APS_CRITICAL_FLAG : 0)) == ENOERROR) {
            status = EOK;
        }
        return status;
    ...
    case SIGEV_PULSE:
        {
            ...
            if (deliver) {
                ...
                prio =
                    (evp->sigev_priority ==
                     -1) ? chp->process->process_priority : evp->
                    sigev_priority & _PULSE_PRIO_PUBLIC_MASK;
                ...
                if ((status = pulse_deliver(chp, prio, evp->sigev_code, evp->sigev_value.sival_int,
                                            ...
                                            (evp->
                                             sigev_notify & SIGEV_FLAG_CRITICAL ?
                                             PULSE_DELIVER_APS_CRITICAL_FLAG : 0)))) {
                    return (status);
                }
            }
            break;
    ...
    case SIGEV_UNBLOCK:
        if (deliver) {
            ...
            if (evp->sigev_notify & SIGEV_FLAG_CRITICAL)
...
{
    if (evp) {
        CRASHCHECK(SIGEV_GET_TYPE(evp) == SIGEV_PULSE && evp->sigev_priority == -1);
