...
void *nano_query(int type, unsigned index1, int subtype, unsigned index2,
                 unsigned *next, void *objbuf, int objsize)
{
    ...
    if ((vecp = vector_search(&query_vector, type, 0))) {
        ...
        case _QUERY_PROCESS:
            ...
            } else if ((obj = prp = vector_search(vecp, PINDEX(index1), 0)) && (prp->pid == index1)
                       && (index1 != 0)) {
                obj =
                    vector_search((VECTOR *) ((uintptr_t) prp +
                                              vec_offset[subtype - _QUERY_PROCESS_CHANCONS]),
                                  index2, next);
            } else {
                obj = NULL;
            }
