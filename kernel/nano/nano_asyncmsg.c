...
int rdecl
msgreceive_gbl(THREAD * act, CHANNELGBL * chp, void *msg, size_t size, struct _msg_info *info,
               CONNECT * cop, int coid)
{
  ...
  restart:
    ...
    while ((cop = (CONNECT *) chp->ch.reply_queue) && (act->args.ms.sparts < parts)) {
        ...
        if (act->process == cop->process) {
            ...
            SETIOV(srciov, lcd,
                   sizeof(*lcd) + sizeof(struct _asyncmsg_put_header) * cop->sendq_size);
            ...
            nbytes = memmgr.map_xfer(act->process,
                                     cop->process,
                                     (IOV **) & srcp,
                                     &sparts, &soff, dstiov, &dparts, MAPADDR_FLAGS_IOVKERNEL);
            if (nbytes < 0) {
                return EFAULT;
            }
        ...
        while ((cd->sendq_head != cd->sendq_tail) && (act->args.ms.sparts < parts)) {
            ...
            if (!WITHIN_BOUNDRY
                ((uintptr_t) (ghp), (uintptr_t) (ghp) + (sizeof(*ghp)),
                 act->process->boundry_addr)) {
                lock_kernel();
                act->restart = NULL;
                cop->restart = NULL;
