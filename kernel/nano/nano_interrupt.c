...
static const struct sigevent *dummy_handler(void *dummy, int id)
{
    return NULL;
}
                 ...
                 void *area, int flags)
{
    ...
    *owner = itp;
    ...
    if (count == 0) {
        ...
        do {
        } while (interrupt_unmask(level, NULL) != 0);
    }
    return (id);
}

void rdecl interrupt_detach(int level, const struct sigevent *(*handler) (void *area, int id))
{
    INTERRUPT *itp;
    THREAD *act = actives[KERNCPU];
    int i;
    ...
    for (i = 0; i < interrupt_vector.nentries; ++i) {
        if (VECP(itp, &interrupt_vector, i)) {
            if (itp->level == level && itp->handler == handler && itp->thread == act) {
                break;
            }
        }
    }
...
void rdecl interrupt_detach_entry(PROCESS * prp, int index)
{
    ...
    if ((interrupt_level[itp->level].queue->next == NULL)
        && (interrupt_level[itp->level].queue == itp)) {
#ifdef VARIANT_instr
