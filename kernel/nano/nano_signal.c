...
// into a priority.
//
static void signal_discard(PRIL_HEAD * queue, PROCESS * prp, int signo)
{
    PULSE *pup;
    PULSE *next;
    ...
    for (pup = pril_find_insertion(queue, priority | _PULSE_PRIO_HEAD);
         (pup != NULL) && (pup->priority == priority); pup = next) {
        next = pril_next(pup);
...
static void signal_remove(PROCESS * prp, int signo)
{
    int i;
    THREAD *thp;
    PULSE *pup;
        ...
        if (VECP(victim, &process_vector, pid2)) {
            if (victim->pgrp == pgrp) {
                ...
                if (keriskill(thp, victim, signo)) {
                    ...
                    status |= signal_kill_process(victim, signo, code, value, pid, signal_flags);
                }
            }
        }
    }
...
//
static void sigstack_fault(THREAD * thp, CPU_REGISTERS * regs, unsigned flags)
{
    PULSE *pup;
    int signo;
    SIGHANDLER *shp;
...
// blocked mask.
//
void rdecl signal_block(THREAD * thp, sigset_t * sig_blocked)
{
    PULSE *pup;
    PULSE *next;
    PROCESS *prp = thp->process;
...
//
void rdecl usr_fault(int code_signo, THREAD * thp, uintptr_t addr)
{
    PROCESS *prp;
    siginfo_t info;
