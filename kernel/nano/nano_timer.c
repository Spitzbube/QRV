...
static struct timer_queue *new_queue(unsigned entries, unsigned downshift)
{
...
static void kill_queue(struct timer_queue *queue)
{
...
static void timer_insert(TIMER * tip, enum fire_source src)
{
    TIMER *fore;
    TIMER *back;
  ...
  retry:
    ...
    if (fore->link.next != back) {
        ...
        for (;;) {
            ...
            if (tip->itime.nsec >= back->itime.nsec)
                goto insert_after;
            if (tip->itime.nsec < fore->itime.nsec)
                goto insert_before;
        }
    }
...
static void timer_rearm(TIMER * tip, enum fire_source src)
{
    uint64_t interval;
    QTIME *qtp;
...
static TIMER *timer_fire(TIMER * tip)
{
    THREAD *thp;
    TIMER *next;
...
void rdecl timer_expiry(QTIME * qtp)
{
    ...
    for (;;) {
        ...
        if (queue->num_active != 0) {
            ...
            for (;;) {
                ...
                if (queue->fire_idx == curr_tick_idx)
                    break;
    ...
    TICKER_STOP();
    if (!timer_ops_delayed && ((pending_head != PENDING_TIMER_END) || (queue_dead != NULL))) {
...
void rdecl timer_pending(TIMER * skip)
{
    TIMER *tip;
    struct timer_queue *queue;
    struct timer_queue *next;
...
static void new_period(struct timer_queue **head, unsigned downshift)
{
    struct timer_queue *new;
...
TIMER *rdecl timer_alloc(PROCESS * prp)
{
    return object_alloc(prp, &timer_souls);
}


void rdecl timer_free(PROCESS * prp, TIMER * tip)
{
...
static void timer_next_preempt(THREAD * thp, CPU_REGISTERS * regs)
{
    KEROP_STOP();
}
...
void rdecl timer_next(uint64_t * np)
{
    struct timer_queue *queue;
    TIMER *tip;
    TIMER *first;
        ...
        if (queue->num_active != 0) {
            for (i = 0; i <= queue->mask; ++i) {
                ...
                for (;;) {
                    ...
                    if (tip == first)
                        goto next;
                    if ((tip->clockid != CLOCK_SOFTTIME) && (tip->pending == NULL))
                        break;
                }
