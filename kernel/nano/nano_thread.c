...
static int procmgr_stack_alloc(THREAD * thp)
{
...
static void procmgr_stack_free(THREAD * thp)
{
    void **stk;
...
static void kerext_stack_purge(void *p)
{
    int r;
...
// thread_create_flags are an or-ing of kermacros.h : THREAD_CREATE_*_FLAG
    ...
    thp->sched_flags = 0;
    if (thread_create_flags & THREAD_CREATE_APS_CRITICAL_FLAG)
...
void rdecl thread_destroy(THREAD * thp)
{
    ...
    if (thp->state != STATE_DEAD) {
        ...
        if (prp->death_chp && (prp->death_chp->flags & _NTO_CHF_THREAD_DEATH)) {
            pulse_deliver(prp->death_chp, thp->real_priority, _PULSE_CODE_THREADDEATH, thp->tid + 1,
                          -1, 0);
        }
    }
...
static void threadstack_fault(THREAD * thp, CPU_REGISTERS * reg, unsigned flags)
{
...
void rdecl thread_specret(THREAD * thp)
{
    ...
    if ((attr = thp->args.wa.attr)) {
        ...
        if (attr->__flags & PTHREAD_SCOPE_PROCESS) {
            verify = ENOTSUP;
        } else if ((attr->__stackaddr || attr->__stacksize)
    ...
    if (thp->process->pid == PROCMGR_PID) {
        ...
        tsp->__stackaddr =
            (uint8_t *) thp->un.lcl.stackaddr + ((attr == NULL) ? 0 : attr->__guardsize);
    }
