...
int em_event(tracebuf_t * t_b_p)
{
    ...
    if (dropped_events != 0) {
        if (ker_verbose > 2)
            kprintf("INSTR: dropped %d events!\n", dropped_events);
        dropped_events = 0;
    }
    if (dropped_buffers != 0) {
        if (ker_verbose > 2)
            kprintf("INSTR: dropped %d buffers!\n", dropped_buffers);
        dropped_buffers = 0;
    }
    if ((itp = interrupt_level[HOOK_TO_LEVEL(_NTO_HOOK_TRACE)].queue)) {
        ...
        if ((r_vlocal = _TRACE_EH_INVOKE((uint32_t(*)(int)) itp->handler,
...
uint32_t exe_pt_event_h
    (ehandler_data_t * stor_ptr, uint32_t header, pid_t pid, int tid, uint32_t d1, uint32_t d2) {
...
uint32_t exe_event_h(ehandler_data_t * stor_ptr, uint32_t header, uint32_t d1, uint32_t d2) {
    event_data_t *area = stor_ptr->area;
    int l = _TRACE_GET_LOCK();
    int cpu = RUNCPU;
    THREAD *thp = stor_ptr->thp;
...
uint32_t exe_pt_event_h_buff
    (ehandler_data_t * stor_ptr, uint32_t header, pid_t pid, int tid, void *buff, uint32_t size) {
    event_data_t *area = stor_ptr->area;
    int l = _TRACE_GET_LOCK();
    int cpu = RUNCPU;
    THREAD *thp = stor_ptr->thp;
...
uint32_t exe_event_h_buff
    (int locked, ehandler_data_t * stor_ptr, uint32_t header, void *buff, uint32_t size) {
    event_data_t *area = stor_ptr->area;
    int l = 0;
    int cpu = RUNCPU;
    THREAD *thp = stor_ptr->thp;
...
static uint32_t exe_event_h_no_lock
    (ehandler_data_t * stor_ptr, uint32_t header, uint32_t d1, uint32_t d2) {
    event_data_t *area = stor_ptr->area;
    int cpu = RUNCPU;
    THREAD *thp = stor_ptr->thp;
    PROCESS *pp = aspaces_prp[cpu];
...
void aps_name_em(uint32_t partition_id, char *name)
{
    ...
    if (_TRACE_CKH_EXE_EHB(trace_masks.class_system_ehd_p,
                           trace_masks.system_ehd_p[_NTO_TRACE_SYS_APS_NAME],
                           header, &buf, sizeof(buf.partition_id) + strlen(buf.name) + 1)) {
        add_trace_d1_string(header, buf.partition_id, "%s", buf.name);
    }
...
void aps_bankruptcy_em(uint32_t partition_id, pid_t pid, int tid)
{
    ...
    buf.tid = tid + 1;
    buf.partition_id = partition_id;
    if (_TRACE_CKH_EXE_PT_EHB(trace_masks.class_system_ehd_p,
                              trace_masks.system_ehd_p[_NTO_TRACE_SYS_APS_BNKR],
                              header, buf.pid, buf.tid, (void *) &buf, sizeof(buf)
        )) {
        add_trace_buffer(header, (void *) &buf, _TRACE_ROUND_UP(sizeof(buf)));
    }
    return;
...
void time_em(uint32_t msb, uint32_t lsb)
{
    uint32_t header = _TRACE_MAKE_CODE(RUNCPU,
                                       ...
                                       _TRACE_CONTROL_C,
                                       _TRACE_CONTROL_TIME);
    if (_TRACE_CKH_EXE_EH(trace_masks.class_control_ehd_p,
                          trace_masks.control_time_ehd_p, header, msb, lsb)) {
...
#endif
        _TRACE_EMIT_CONTROL_TIME(ts[cpu].bits[_TRACE_CLOCK_MSB], ts[cpu].bits[_TRACE_CLOCK_LSB]
            );
    }
...
void th_em_st(THREAD * thp, uint32_t s, int cpu)
{
    ...
    if (_TRACE_THPTID(thp, s)) {
        ...
        uint32_t header = _TRACE_MAKE_CODE(cpu,
                                           _TRACE_STRUCT_S,
                                           _TRACE_PR_TH_C,
                                           s);
        ...
        if (_TRACE_THREAD_ARG_WIDE & trace_masks.th_mask) {
            ...
            if (_TRACE_CKH_EXE_PT_EHB(trace_masks.class_thread_ehd_p,
                                      trace_masks.thread_ehd_p[s],
                                      header, buf.pid, buf.tid, (void *) &buf, bufsize)) {
                add_trace_buffer(header, (void *) &buf, _TRACE_ROUND_UP(bufsize));
            }
        } else {
            if (_TRACE_CKH_EXE_PT_EH(trace_masks.class_thread_ehd_p,
                                     trace_masks.thread_ehd_p[s],
                                     header, buf.pid, buf.tid, buf.pid, buf.tid)) {
...
void th_em_cd(THREAD * thp, uint32_t s, uint32_t c)
{
    ...
    if (_TRACE_THPTID(thp, s)) {
        ...
        if (_TRACE_CKH_EXE_PT_EH(trace_masks.class_thread_ehd_p,
                                 trace_masks.thread_ehd_p[s], header, pid, tid, pid, tid)) {
...
void pr_em_create(PROCESS * prp, pid_t pid)
{
    ...
    if (_TRACE_CKH_EXE_EH(trace_masks.class_process_ehd_p,
                          trace_masks.pr_create_ehd_p, header, ppid, pid)) {
...
void pr_em_destroy(PROCESS * prp, pid_t pid)
{
    ...
    if (_TRACE_CKH_EXE_EH(trace_masks.class_process_ehd_p,
                          trace_masks.pr_destroy_ehd_p, header, ppid, pid)) {
        (void) add_trace_event(header, NULL, ppid, (uint32_t) pid);
    }
...
void pr_em_create_name(PROCESS * prp)
{
    ...
    if (_TRACE_CKH_EXE_EHB(trace_masks.class_process_ehd_p,
                           trace_masks.pr_create_name_ehd_p,
                           header,
                           &buf, sizeof(buf.ppid) + sizeof(buf.pid) + strlen(buf.name) + 1)) {
        add_trace_d2_string
            (header,
             prp->parent ? (uint32_t) prp->parent->pid : NULL, (uint32_t) prp->pid, "%s", str_p);
    }
...
void th_em_name(THREAD * thp)
{
    ...
    if (_TRACE_CKH_EXE_EHB(trace_masks.class_process_ehd_p,
                           ...
                           header,
                           &buf, sizeof(buf.pid) + sizeof(buf.tid) + strlen(buf.name) + 1)) {
        add_trace_d2_string(header, buf.pid, buf.tid, "%s", buf.name);
    }
}
...
void vth_em_cd(THREAD * vthp, uint32_t s, uint32_t c)
{
    ...
    if (_TRACE_VTHPTID(vthp, s)) {
        ...
        if (_TRACE_CKH_EXE_PT_EH(trace_masks.class_vthread_ehd_p,
                                 trace_masks.vthread_ehd_p[s], header, pid, tid, pid, tid)) {
            (void) add_trace_event(header, NULL, pid, tid);
        }
    }
...
void comm_exe_em(CONNECT * cop, uint32_t srcvid, uint32_t ev)
{
    ...
    if ((!trace_masks.comm_pid[ev] || trace_masks.comm_pid[ev] == pid) &&
        _TRACE_CKH_EXE_EH(trace_masks.class_comm_ehd_p,
                          trace_masks.comm_ehd_p[ev], header, srcvid, pid)) {
...
void comm_em(THREAD * thp, CONNECT * cop, uint32_t srcvid, uint32_t ev)
{
    if (_TRACE_IPCPTID(thp, ev)) {
        ...
        header = _TRACE_MAKE_CODE(RUNCPU, _TRACE_STRUCT_S, _TRACE_COMM_C, ev);
        if (_TRACE_CKH_EXE_PT_EH(trace_masks.class_comm_ehd_p,
                                 trace_masks.comm_ehd_p[ev],
                                 header, thp->process->pid, thp->tid + 1, srcvid, pid)) {
            (void) add_trace_event(header, NULL, srcvid, pid);
        }
    }
...
void comm_em_signal(THREAD * thp, siginfo_t * s_i)
{
    ...
    if (_TRACE_IPCPTID(thp, _NTO_TRACE_COMM_SIGNAL)) {
        if (_TRACE_COMM_ARG_WIDE & trace_masks.comm_mask[_NTO_TRACE_COMM_SIGNAL]) {
            ...
            if (_TRACE_CKH_EXE_PT_EHB(trace_masks.class_comm_ehd_p,
                                      trace_masks.comm_ehd_p[_NTO_TRACE_COMM_SIGNAL],
                                      header,
                                      thp->process->pid,
                                      thp->tid + 1,
                                      (void *) arg_arr, sizeof(uint32_t) * NUM_ELTS(arg_arr)
                )) {
            ...
            if (_TRACE_CKH_EXE_PT_EH(trace_masks.class_comm_ehd_p,
                                     ...
                                     header,
                                     thp->process->pid,
                                     thp->tid + 1, s_i->si_signo, s_i->si_code)) {
...
void add_ktrace_int_handler_enter(INTRLEVEL * lp, INTERRUPT * isr)
{
    ...
    if (_TRACE_ENTER_INT & trace_masks.int_handler_masks[l]) {
        ...
        if (_TRACE_CKH_EXE_EHBNL(trace_masks.class_int_handler_enter_ehd_p,
                                 trace_masks.int_handler_enter_ehd_p[l], header, &buf, sizeof(buf)
            )) {
...
void add_ktrace_int_handler_exit(INTRLEVEL * lp, const struct sigevent *ev)
{
    ...
    if (_TRACE_EXIT_INT & trace_masks.int_handler_masks[l]) {
        ...
        if (_TRACE_CKH_EXE_EHBNL(trace_masks.class_int_handler_exit_ehd_p,
                                 trace_masks.int_handler_exit_ehd_p[l], header, &buf, sizeof(buf)
            )) {
            add_trace_buffer(header, (uint32_t *) & buf, sizeof(buf) / sizeof(uint32_t));
        }
    }
...
void add_ktrace_int_enter(INTRLEVEL * lp)
{
    ...
    if (_TRACE_ENTER_INT & trace_masks.int_masks[l]) {
        ...
        if (_TRACE_CKH_EXE_EHNL(trace_masks.class_int_enter_ehd_p,
                                trace_masks.int_enter_ehd_p[l], header, i_n, ip)) {
...
void add_ktrace_int_exit(INTRLEVEL * lp)
{
    ...
    if (_TRACE_EXIT_INT & trace_masks.int_masks[l]) {
        ...
        if (_TRACE_CKH_EXE_EHNL(trace_masks.class_int_exit_ehd_p,
                                trace_masks.int_exit_ehd_p[l], header, i_n, ink)) {
...
int add_trace_event(uint32_t header, uint32_t d_0, uint32_t d_1, uint32_t d_2)
{
  ...
  begin:
    ...
    if (t_b_p->h.num_events == 0) {
        ...
        t_b_p->h.tail_ptr++;
        e_p->header =
            _TRACE_MAKE_CODE(RUNCPU, _TRACE_STRUCT_S, _TRACE_CONTROL_C, _TRACE_CONTROL_BUFFER);
        e_p->data[0] = d_0;
...
void add_trace_b_add(trace_state_t * st_p, uint32_t header, char *b_p, unsigned len)
{
    ...
    while (bi < len) {
        ...
        if (st_p->di == sizeof(st_p->data)) {
            ...
            (void) add_trace_event(header, st_p->time_off, st_p->data.idata[0],
                                   st_p->data.idata[1]);
                ...
                if (VECP(prp, &process_vector, i_1)) {  // process names and task IDs
                    ...
                    for (i_2 = 0; i_2 < prp->threads.nentries; ++i_2) {
                        if (VECP(thp, &prp->threads, i_2)) {
                            ...
                            _TRACE_TH_EMIT_STATE_ONCPU(thp,
                                                       thp->state,
...
void trace_emit_address(THREAD * thp, uint32_t vaddr)
{
    ...
    header = _TRACE_MAKE_CODE(RUNCPU, _TRACE_STRUCT_S, _TRACE_SYSTEM_C, _NTO_TRACE_SYS_ADDRESS);
    if (_TRACE_CKH_EXE_EH(trace_masks.class_system_ehd_p,
                          trace_masks.system_ehd_p[_NTO_TRACE_SYS_ADDRESS], header, vaddr, 0)) {
