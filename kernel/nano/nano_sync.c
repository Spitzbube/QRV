                ...
                if (prp) {
                    ...
                    if (SYNC_PINDEX(sync->__owner) == SYNC_PINDEX(SYNC_OWNER_BITS(prp->pid, 0))) {
                        ...
                        if (thp != NULL) {
                            ...
                            if ((type == TYPE_THREAD && thp->state == STATE_MUTEX)
                                || (type == TYPE_SYNCEVENT
                    ...
                    if (event_state == EVENT_DELIVER) {
                        ...
                        while ((thp = pril_first(&syp->waiting))) {
                            ...
                            if ((thp->timeout_flags & (_NTO_TIMEOUT_ACTIVE | (1 << STATE_MUTEX))) !=
                                ...
                                (void) signal_kill_thread(thp->process, thp, SIGDEADLK, SI_NOINFO,
                                                          (intptr_t) sync, prp->pid, 0);
...
static void ker_synchash_rem(void *data)
{
...
void MemobjDestroyed(OBJECT * obp, off_t start, off_t end, PROCESS * prp, void *vaddr)
{
    ...
    if (am_inkernel()) {
        ...
        for (;;) {
            ...
            if ((data.addr & ~(HASH_PAGESIZE - 1)) == (data.end & ~(HASH_PAGESIZE - 1)))
                break;
...
void mutex_holdlist_add(THREAD * thp, SYNC * syp)
{
    SYNC **owner;
    SYNC *curr;
    THREAD *new;
...
void mutex_holdlist_rem(SYNC * syp)
{
    THREAD *thp;
    THREAD *next_thp;
    SYNC *next_syp;
