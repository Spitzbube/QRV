...
// in state reply-blocked when we (the receiver) start to run.
                if (thp != thp2 && thp2->state != STATE_REPLY && thp2->state != STATE_NET_REPLY)
                    crash();
#endif

                status =
                    xfer_cpy_diov(thp, thp->args.ri.rmsg, thp2->args.msbuff.buff,
        ...
        case QRV_FLG_THR_ASYNC_RECEIVE:{
                ...
                err =
                    msgreceive_gbl(thp, (CHANNELGBL *) chp, thp->args.ri.rmsg, thp->args.ri.rparts,
                                   thp->args.ri.info, cop, coid);
                if (err) {
                    kererr(thp, err);
                }
                break;
