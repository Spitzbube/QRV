...
#error ENDIAN Not defined for system
        ...
        if (TYPE_MASK(thp->type) == TYPE_THREAD) {
            ...
            if (actives[KERNCPU]->process == net.prp &&
                rep->chid == net.chp->chid && (cop->flags & COF_NETCON) == 0) {
    ...
    int viov_to_piov(PROCESS * prp, IOV * piop, int pparts, IOV * viop, int vparts, int voffset) {
        char *vaddr;
        paddr_t paddr;
        int vlen, nparts, n;
        ...
        for (nparts = 0; pparts;) {
            ...
            for (; pparts && vlen; vlen -= n, vaddr += n) {
                ...
                if (nparts
                    && paddr == (paddr_t) (uintptr_t) GETIOVBASE(piop - 1) + GETIOVLEN(piop - 1)) {
...
#ifndef NDEBUG
        if (thp == NULL || thp->un.net.vtid != vtid)
            crash();
#endif
