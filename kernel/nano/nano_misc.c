...
int rdecl kerisusr(THREAD * srcthp, PROCESS * dstprp)
{
...
int rdecl kerschedok(THREAD * thp, int policy, const struct sched_param *param)
{
    ...
    if (thp->process->cred->info.euid != 0 && param->sched_priority >= priv_prio)
        return EPERM;
    if (policy < SCHED_NOCHANGE || policy > SCHED_SPORADIC)
        return EINVAL;
    if (policy != SCHED_SPORADIC)
        return EOK;
    if (param->sched_ss_max_repl < 1 || param->sched_ss_max_repl > SS_REPL_MAX)
        return EINVAL;
    if (param->sched_ss_low_priority <= 0 || param->sched_ss_low_priority > param->sched_priority)
        return EINVAL;
    if (timespec2nsec(&param->sched_ss_repl_period) < timespec2nsec(&param->sched_ss_init_budget))
        return EINVAL;
    return EOK;
}
...
int purger_register(int (*purger)(size_t), unsigned prio)
{
    struct purger *new;
    struct purger **owner;
    struct purger *chk;
...
void pril_validate(PRIL_HEAD * ph, PRIL * present, PRIL * not_present)
{
    PRIL **owner;
    PRIL *p;
    PRIL *n;
    ...
    for (;;) {
        ...
        if (p->type & FLAG_HEAD) {
            n = p->prev.prio_tail;
            if (PRIL_PRIORITY(p) != PRIL_PRIORITY(n))
                crash();
        } else {
            if (p->prev.pril != owner)
                crash();
        }
        n = p->next.pril;
        if (p->type & FLAG_TAIL) {
            if ((n != NULL) && PRIL_PRIORITY(p) <= PRIL_PRIORITY(n))
                crash();
        } else {
            if (PRIL_PRIORITY(p) != PRIL_PRIORITY(n))
                crash();
...
void *rdecl pril_first(PRIL_HEAD * ph)
{
    return ph->data;
}
#endif
...
void *rdecl pril_next_prio(void *d)
{
    PRIL *p = d;
...
void *rdecl pril_find_insertion(PRIL_HEAD * ph, unsigned priority)
{
    PRIL *head;
    PRIL *tail;
    PRIL **owner;
...
void rdecl pril_add(PRIL_HEAD * ph, void *d)
{
    PRIL *new = d;
    PRIL *head;
    PRIL *tail;
...
void rdecl pril_rem(PRIL_HEAD * ph, void *d)
{
    PRIL *p = d;
    PRIL *next;
    PRIL *t;
...
void rdecl pril_update_register(PRIL_HEAD * ph, struct pril_update *upp)
{
    ...
    for (;;) {
        ...
        if ((uintptr_t) chk->pril >= (uintptr_t) pril)
            break;
        prev = chk;
        chk = chk->next;
    }
...
void rdecl pril_update_unregister(PRIL_HEAD * ph, struct pril_update *upp)
{
