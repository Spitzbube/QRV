...
static void chk_dispatch(int chkstate)
{
    unsigned int i;
    THREAD *thp;
    THREAD **owner;
...
static int rdecl select_cpu(THREAD * thp)
{
    THREAD *act;
    int i, j, cpu;
    unsigned minprio;
 ...
 * off any queue.
 */
static THREAD *rdecl select_thread_default(THREAD * act, int cpu, int prio)
{
    THREAD *thp;
    uint32_t runmask;
    int hipri;
...
*/
static void rdecl priority_chain_drop(THREAD * thp, int why)
{
    SYNC *syp;
    THREAD *next_owner;
    ...
    case STATE_MUTEX:
        {
            ...
            if (pril_first(&syp->waiting) == thp) {
                ...
                if (thp2 && (thp != thp2) && (thp->priority == thp2->priority)
                    && (thp2->priority > thp2->real_priority)) {
    ...
    if (thp->state != STATE_READY && thp->state != STATE_RUNNING) {
        ...
        if (TYPE_MASK(thp->type) == TYPE_VTHREAD) {
            ...
            if (!(thp->internal_flags & _NTO_ITF_NET_UNBLOCK) && net.chp) {
                ...
                pulse_deliver(net.chp, thp->priority, _PULSE_CODE_NET_UNBLOCK, thp->un.net.vtid, -1,
                              0);
            }
#endif
        } else {
...
static void rdecl block_and_ready_default(THREAD * thp)
{
...
static void rdecl yield_default(void)
{
    THREAD *act = actives[KERNCPU];
    THREAD *thp;
 ...
 */
static void rdecl resched_default(void)
{
    THREAD *act = actives[KERNCPU];
    THREAD *thp;
...
static int rdecl may_thread_run_default(THREAD * thp)
{
    return 1;
}
 ...
 */
void sched_trace_initial_parms_default()
{
}


static void rdecl debug_moduleinfo_default(PROCESS * prp, THREAD * thp, void *dbg)
{
...
DISPATCH *init_scheduler_default(void)
{
    DISPATCH *dpp;
