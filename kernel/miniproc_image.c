...
enter_func(Elf32_Addr start, char **argv, char **envp, volatile struct syspage_entry *sysp)
{
  asm("subi 	%%r1,%%r1, 4;" "li		%%r11,-16;" "and	%%r1,%%r1,%%r11;" "li		%%r11,0;" "stw	%%r11,0(%%r1);" "mtlr	%0;" "lwz	%%r3,-4(%1);" "mr		%%r4,%1;" "mr		%%r5,%2;" "li		%%r6,0;" "li		%%r7,0;" "blr":
...
static void *find_file(char *cmd, char *path)
{
    //NYI: How to match up file name with dir entry?
    return NULL;
}

static void *startup(void *in)
{
...
static int map_ifs(struct asinfo_entry *as, char *name, void *data)
{
    struct image_header **ihpp = data;

    *ihpp =
        mmap(0, (as->end - as->start) + 1, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_PHYS, NOFD,
             as->start);
    return 1;
}

void miniproc_image_start(void)
{
    ...
    if (walk_asinfo("imagefs", map_ifs, &ihp))
        crash();
    ...
    for (;;) {
        if (dir->attr.size == 0)
            return;
        if (dir->attr.ino == ihp->script_ino)
            break;
...
//NYI: check scp->hdr.type
        if (strcmp(cp, "reopen") == 0) {
            ...
            if (tid)
                (void) ThreadJoin(tid, NULL);
            tid = ThreadCreate(0, startup, scp, 0);
            if (scp->external.flags & SCRIPT_FLAGS_BACKGROUND)
                tid = 0;
        }
