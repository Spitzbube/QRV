...
static int path_link(char *path, struct _io_resmgr_link_extra *lep)
{
...
static int path_resolve(char *path, struct _io_connect_entry *enp)
{
...
static void do_miniproc(void)
{
    ...
    for (;;) {
        ...
        case _IO_WRITE:
            ...
            while (nbytes) {
                if ((n =
                     MsgRead(rcvid, cp, min(nbytes, sizeof(*msp) - sizeof(msp->write)), off)) <= 0)
                    break;
                scrn_display(cp, n);
                off += n;
                nbytes -= n;
        ...
        case _MEM_MAP:
            err = memmgr.mmap(0, msp->memmap.i.addr, msp->memmap.i.len, msp->memmap.i.prot, msp->memmap.i.flags, 0, (unsigned) msp->memmap.i.offset, msp->memmap.i.align, msp->memmap.i.preload, msp->memmap.i.fd, (void **) &msp->memmap.o.real_addr, (size_t *) &msp->memmap.o.real_size, mempart_getid(NULL, sys_memclass_id));  // FIX ME - this ok for miniproc ?
        ...
        case _MEM_CTRL:
            ...
            case _MEM_CTRL_UNMAP:
                err =
                    memmgr.munmap(0, msp->memctrl.i.addr, msp->memctrl.i.len, msp->memctrl.i.flags,
                                  mempart_getid(NULL, sys_memclass_id));
                break;

            case _MEM_CTRL_SYNC:
                err =
                    memmgr.msync(0, msp->memctrl.i.addr, msp->memctrl.i.len, msp->memctrl.i.flags);
                break;
...
static void *_miniproc(void *dummy)
{
    do_miniproc();
    return NULL;
}
