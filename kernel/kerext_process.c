...
static void kerext_process_create(void *data)
{
               ...
               ((kap->extra->spart_list == NULL) && SCHEDPART_INSTALLED()));
    {
        ...
        for (i = 0; i < mpart_list->num_entries; i++) {
            if ((status =
                 MEMPART_ASSOCIATE(prp, mpart_list->i[i].id, mpart_list->i[i].flags)) != EOK) {
        ...
        if (SCHEDPART_INSTALLED()) {
            for (i = 0; i < spart_list->num_entries; i++) {
                if ((status =
                     SCHEDPART_ASSOCIATE(prp, spart_list->i[i].id,
                                         spart_list->i[i].flags)) != EOK) {
                    (void) MEMPART_DISASSOCIATE(prp, part_id_t_INVALID);
                    (void) MUTEX_DESTROY(prp, &prp->mpartlist_lock);
                    (void) MUTEX_DESTROY(prp, &prp->spartlist_lock);
                    object_free(NULL, &process_souls, prp);
    ...
    if ((prp->limits = lookup_limits(parent->cred->info.euid)) || (prp->limits = parent->limits)) {
        prp->limits->links++;
    }
...
static void kerext_process_destroyall(void *data)
{
...
static void kerext_process_destroy(void *data)
{
    ...
    if (prp->limits && prp->limits->links == ~0U)
        crash();
    if (prp->pid)
        crash();
    if (prp->cred)
    ...
    if (prp->alarm)
        crash();
    if (pril_first(&prp->sig_pending))
        crash();
    if (prp->sig_table)
        crash();
    if (prp->nfds)
        crash();
    if (prp->chancons.vector)
        crash();
    if (prp->fdcons.vector)
        crash();
    if (prp->threads.vector)
        crash();
    if (prp->timers.vector)
        crash();
    if (prp->memory)
        crash();
    if (prp->join_queue)
...
static void kerext_process_restore(void *data)
{
...
void
ProcessRestore(int rcvid, struct _thread_local_storage *tsp, void *frame, void *frame_base,
               unsigned frame_size)
{
...
static void kerext_process_startup(void *data)
{
...
static void kerext_process_exec(void *data)
{
    ...
    if (!(cop = lookup_rcvid(NULL, kap->rcvid, &thp)) || thp->state != STATE_REPLY
        || thp->blocked_on != cop) {
        kererr(act, ESRCH);
        return;
    }
...
static void process_swap(PROCESS * child)
{
    ...
    *pprp = child->child;
    ...
    parent->child = 0;
    child->num_processes = parent->num_processes;
    parent->num_processes = 0;
}
...
static void kerext_process_swap(void *data)
{
...
#undef FLAGS
    ...
    if (!proc_cop || !proc_cop->channel || proc_cop->links != 2)
        crash();
    if (proc_cop->un.lcl.pid != SYSMGR_PID || proc_cop->un.lcl.chid != SYSMGR_CHID)
        crash();
    if (parent->chancons.nentries != parent->chancons.nfree)
        crash();
    if (child->chancons.nentries != child->chancons.nfree + 1)
        crash();
    if (child->fdcons.nentries != child->fdcons.nfree)
        crash();
#endif
...
void rdecl set_safe_aspace(int cpu)
{
...
static void kerext_process_shutdown(void *data)
{
...
void kerext_thread_tls(void *data)
{
    ...
    if (kap->esp) {
        ...
        tsp =
            (struct _thread_local_storage *) ((kap->esp & ~(STACK_ALIGNMENT - 1)) -
                                              sizeof *act->un.lcl.tls);
