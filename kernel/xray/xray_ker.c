 ...
 */
void rdecl xray_profile_cswitch(THREAD * thp)
{
 ...
 */
void rdecl xray_trace_cswitch(THREAD * thp)
{
    static THREAD *prev_thp = 0;
 ...
 * XRAY initialization
 */
void xray_initialize(unsigned version, unsigned pass)
{
    ...
    if ((base = (xray_entry *) xray->xray_base)) {
        xray_base = xray->xray_flags & XRAY_FLAGS_CACHEABLE ?
            (xray_entry *) PHYS_TO_CACHED_ADDR(base) : (xray_entry *) PHYS_TO_UNCACHED_ADDR(base);
    ...
    if (xray->xray_flags & XRAY_FLAGS_CSWITCH) {
        ...
        mark_running = xray->xray_flags & XRAY_FLAGS_TRACE ?
            xray_trace_cswitch : xray_profile_cswitch;
    }
}
