...
static int _trace_emit_in_f(tThread * act, void *kap, uint32_t num, uint32_t arg_1, uint32_t arg_2)
{
    ...
    if (_TRACE_GETSTATE(act->syscall) == 2 && _TRACE_CHK_ENTRY(act, _TRACE_ENTER_CALL, num)) {
        ...
        if (trace_masks.class_ker_call_enter_ehd_p) {
            h_r_v = exe_pt_event_h(trace_masks.class_ker_call_enter_ehd_p,
                                   header,
                                   _TRACE_DER_PTR(act->process, pid), act->tid + 1, arg_1, arg_2);
        }
        if (trace_masks.ker_call_enter_ehd_p[num] && h_r_v) {
            h_r_v = h_r_v && exe_pt_event_h(trace_masks.ker_call_enter_ehd_p[num],
                                            header,
                                            _TRACE_DER_PTR(act->process, pid),
                                            act->tid + 1, arg_1, arg_2);
        }
        if (h_r_v) {
...
static void _trace_emit_out_f(tThread * act, uint32_t num, uint32_t arg_2)
{
    ...
    if (trace_masks.class_ker_call_exit_ehd_p && !_TRACE_GETEXIT(act->syscall)) {
        h_r_v = exe_pt_event_h(trace_masks.class_ker_call_exit_ehd_p,
                               header,
                               _TRACE_DER_PTR(act->process, pid), act->tid + 1, arg_1, arg_2);
    }
    if (trace_masks.ker_call_exit_ehd_p[num] && !_TRACE_GETEXIT(act->syscall) && h_r_v) {
        h_r_v = h_r_v && exe_pt_event_h(trace_masks.ker_call_exit_ehd_p[num],
                                        header,
                                        _TRACE_DER_PTR(act->process, pid),
                                        act->tid + 1, arg_1, arg_2);
    }
...
static int _trace_emit_in_w(tThread * act, void *kap, uint32_t num, uint32_t * arg_arr, uint32_t len)
{
    ...
    if (_TRACE_GETSTATE(act->syscall) == 2 && _TRACE_CHK_ENTRY(act, _TRACE_ENTER_CALL, num)) {
        ...
        if (trace_masks.class_ker_call_enter_ehd_p) {
            h_r_v = exe_pt_event_h_buff(trace_masks.class_ker_call_enter_ehd_p,
                                        header,
                                        _TRACE_DER_PTR(act->process, pid),
                                        act->tid + 1, (void *) arg_arr, sizeof(uint32_t) * len);
        }
        if (trace_masks.ker_call_enter_ehd_p[num] && h_r_v) {
            h_r_v = h_r_v && exe_pt_event_h_buff(trace_masks.ker_call_enter_ehd_p[num],
                                                 header,
                                                 _TRACE_DER_PTR(act->process, pid),
                                                 act->tid + 1,
                                                 (void *) arg_arr, sizeof(uint32_t) * len);
        }
        if (h_r_v) {
...
static void _trace_emit_out_w(tThread * act, int num, uint32_t * arg_arr, uint32_t len)
{
    ...
    if (trace_masks.class_ker_call_exit_ehd_p && !_TRACE_GETEXIT(act->syscall)) {
        h_r_v = exe_pt_event_h_buff(trace_masks.class_ker_call_exit_ehd_p,
                                    header,
                                    _TRACE_DER_PTR(act->process, pid),
                                    act->tid + 1, (void *) arg_arr, sizeof(uint32_t) * len);
    }
    if (trace_masks.ker_call_exit_ehd_p[num] && !_TRACE_GETEXIT(act->syscall) && h_r_v) {
        h_r_v = h_r_v && exe_pt_event_h_buff(trace_masks.ker_call_exit_ehd_p[num],
                                             header,
                                             _TRACE_DER_PTR(act->process, pid),
                                             act->tid + 1,
                                             (void *) arg_arr, sizeof(uint32_t) * len);
    ...
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_MSG_DELIVER_EVENT]) {
        ...
        _TRACE_IN_F_0PTR
            (__KER_MSG_DELIVER_EVENT,
             (uint32_t) kap->rcvid, _TRACE_DER_PTR(kap->event, sigev_notify)
            );
    }
}
...
int kdecl _trace_ker_msg_verify_event(tThread * act, struct kerargs_msg_verify_event *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_MSG_VERIFY_EVENT]) {
        ...
        _TRACE_IN_F_0PTR
            (__KER_MSG_VERIFY_EVENT, (uint32_t) kap->rcvid, _TRACE_DER_PTR(kap->event, sigev_notify)
            );
    }
}
...
int kdecl _trace_ker_signal_action(tThread * act, struct kerargs_signal_action *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_SIGNAL_ACTION]) {
        ...
        _TRACE_IN_F_1PTR
            (__KER_SIGNAL_ACTION,
             kap->signo, (uint32_t) _TRACE_DER_PTR(kap->act, sa_handler), kap->oact);
    }
}
...
int kdecl _trace_ker_signal_suspend(tThread * act, struct kerargs_signal_suspend *kap)
{
    if (kap->sig_blocked) {
        _TRACE_IN_F_0PTR
            (__KER_SIGNAL_SUSPEND, kap->sig_blocked->__bits[0], kap->sig_blocked->__bits[1]
            );
    } else {
    ...
    if (kap->sig_wait) {
        _TRACE_IN_F_1PTR
            (__KER_SIGNAL_WAITINFO,
             ...
             (uint32_t) kap->sig_wait->__bits[1], kap->sig_info);
    } else {
...
int kdecl _trace_ker_channel_connect_attrs(tThread * act, struct kerargs_channel_connect_attr *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_CHANCON_ATTR]) {
        ...
        if (kap->new_attrs) {
            fill_mem(act, (void *) (arg_arr + 2), (void *) kap->new_attrs,
                     sizeof(union _channel_connect_attr));
        } else {
...
int kdecl _trace_ker_interrupt_wait(tThread * act, struct kerargs_interrupt_wait *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_INTERRUPT_WAIT]) {
        ...
        _TRACE_IN_F_0PTR
            (__KER_INTERRUPT_WAIT, kap->flags, kap->timeout ? _TRACE_SEC(*kap->timeout) : NULL);
    }
}
...
int kdecl _trace_ker_clock_time(tThread * act, struct kerargs_clock_time *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_CLOCK_TIME]) {
        ...
        _TRACE_IN_F_1PTR
            (__KER_CLOCK_TIME, kap->id, kap->new ? _TRACE_SEC(*kap->new) : NULL, kap->old);
    }
}
...
int kdecl _trace_ker_clock_adjust(tThread * act, struct kerargs_clock_adjust *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_CLOCK_ADJUST]) {
        ...
        _TRACE_IN_F_1PTR
            (__KER_CLOCK_ADJUST, kap->id, _TRACE_DER_PTR(kap->new, tick_count), kap->old);
    }
}
...
int kdecl _trace_ker_clock_period(tThread * act, struct kerargs_clock_period *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_CLOCK_PERIOD]) {
        ...
        _TRACE_IN_F_1PTR(__KER_CLOCK_PERIOD, kap->id, _TRACE_DER_PTR(kap->new, nsec), kap->old);
    }
}
...
int kdecl _trace_ker_timer_create(tThread * act, struct kerargs_timer_create *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_TIMER_CREATE]) {
        ...
        _TRACE_IN_F_0PTR
            (__KER_TIMER_CREATE,
             (uint32_t) kap->id, (uint32_t) _TRACE_DER_PTR(kap->event, sigev_notify)
            );
    }
}
...
int kdecl _trace_ker_timer_settime(tThread * act, struct kerargs_timer_settime *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_TIMER_SETTIME]) {
        ...
        _TRACE_IN_F_1PTR
            (__KER_TIMER_SETTIME,
             kap->id, kap->itime ? _TRACE_SEC(kap->itime->nsec) : NULL, kap->oitime);
    }
}
...
int kdecl _trace_ker_timer_alarm(tThread * act, struct kerargs_timer_alarm *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_TIMER_ALARM]) {
        ...
        _TRACE_IN_F_1PTR
            (__KER_TIMER_ALARM,
             kap->id, kap->itime ? _TRACE_SEC(kap->itime->nsec) : NULL, kap->otime);
    }
}
...
int kdecl _trace_ker_timer_timeout(tThread * act, struct kerargs_timer_timeout *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_TIMER_TIMEOUT]) {
        ...
        _TRACE_IN_F_1PTR
            (__KER_TIMER_TIMEOUT,
             kap->timeout_flags, kap->ntime ? _TRACE_SEC(*kap->ntime) : NULL, kap->otime);
    }
}
...
int kdecl _trace_ker_sync_destroy(tThread * act, struct kerargs_sync_destroy *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_SYNC_DESTROY]) {
        ...
        _TRACE_IN_F_0PTR
            (__KER_SYNC_DESTROY, (uint32_t) kap->sync, _TRACE_DER_PTR(kap->sync, __owner)
            );
    }
}
...
int kdecl _trace_ker_sync_mutex_lock(tThread * act, struct kerargs_sync_mutex_lock *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_SYNC_MUTEX_LOCK]) {
        ...
        _TRACE_IN_F_0PTR
            (__KER_SYNC_MUTEX_LOCK, (uint32_t) kap->sync, _TRACE_DER_PTR(kap->sync, __owner)
            );
    }
}
...
int kdecl _trace_ker_sync_mutex_unlock(tThread * act, struct kerargs_sync_mutex_unlock *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_SYNC_MUTEX_UNLOCK]) {
        ...
        _TRACE_IN_F_0PTR
            (__KER_SYNC_MUTEX_UNLOCK, (uint32_t) kap->sync, _TRACE_DER_PTR(kap->sync, __owner)
            );
    }
}
...
int kdecl _trace_ker_sync_condvar_wait(tThread * act, struct kerargs_sync_condvar_wait *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_SYNC_CONDVAR_WAIT]) {
        ...
        _TRACE_IN_F_0PTR(__KER_SYNC_CONDVAR_WAIT, (uint32_t) kap->sync, (uint32_t) kap->mutex);
    }
}
...
int kdecl _trace_ker_sync_sem_post(tThread * act, struct kerargs_sync_sem_post *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_SYNC_SEM_POST]) {
        ...
        _TRACE_IN_F_0PTR
            (__KER_SYNC_SEM_POST, (uint32_t) kap->sync, _TRACE_DER_PTR(kap->sync, __count)
            );
    }
}
...
int kdecl _trace_ker_sync_sem_wait(tThread * act, struct kerargs_sync_sem_wait *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_SYNC_SEM_WAIT]) {
        ...
        _TRACE_IN_F_0PTR
            (__KER_SYNC_SEM_WAIT, (uint32_t) kap->sync, _TRACE_DER_PTR(kap->sync, __count)
            );
    }
}
...
int kdecl _trace_ker_sync_mutex_revive(tThread * act, struct kerargs_sync_mutex_revive *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_SYNC_MUTEX_REVIVE]) {
        ...
        _TRACE_IN_F_0PTR
            (__KER_SYNC_MUTEX_REVIVE, (uint32_t) kap->sync, _TRACE_DER_PTR(kap->sync, __owner)
            );
    }
}
...
int kdecl _trace_ker_sched_set(tThread * act, struct kerargs_sched_set *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_SCHED_SET]) {
        ...
        _TRACE_IN_F_0PTR(__KER_SCHED_SET, kap->pid, _TRACE_DER_PTR(kap->param, sched_priority)
            );
    }
}
                                                ...
                                                struct kerargs_channel_connect_attr *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_CHANCON_ATTR]) {
        ...
        if (kap->old_attrs && _TRACE_TESTARGERR(act)) {
            fill_mem(act, (void *) (arg_arr + 2), (void *) kap->old_attrs,
                     sizeof(union _channel_connect_attr));
        } else {
            memset((void *) (arg_arr + 2), 0, sizeof(union _channel_connect_attr));
        }
...
static void _trace_ex_ker_timer_info(tThread * act, struct kerargs_timer_info *kap)
{
    if (_TRACE_CALL_ARG_WIDE & trace_masks.ker_call_masks[__KER_TIMER_INFO]) {
        ...
        if (kap->info && _TRACE_TESTARGERR(act)) {
            ...
            (void) memset((void *) (arg_arr + 1), 0, sizeof(struct _timer_info)
                );
        }
...
void _trace_ker_exit(tThread * act)
{
    ...
    if (_TRACE_GETSTATE(act->syscall) > 1) {
        if (_TRACE_CHK_ENTRY(act, _TRACE_EXIT_CALL, n)) {
            if (_TRACE_GETARGSFLAG(act)) {
                ...
                (void) add_trace_event
                    (_TRACE_MAKE_CODE
                     (RUNCPU, _TRACE_STRUCT_S, _TRACE_KER_CALL_C,
                      (2 * _TRACE_MAX_KER_CALL_NUM) + n), NULL, NULL, NULL);
            }
        }
        _TRACE_EXITCALL(act->syscall);
        ...
        if ((trace_masks.comm_mask[_NTO_TRACE_COMM_RMSG] & _TRACE_ENTER_COMM ||
             trace_masks.comm_mask[_NTO_TRACE_COMM_RPULSE] & _TRACE_ENTER_COMM) &&
            !(act->syscall & _TRACE_COMM_CALL_EXIT) && act->syscall & _TRACE_COMM_IPC_EXIT) {
            comm_em(act, NULL, 0U, ~_NTO_TRACE_COMM_RMSG);
        }
    }
