...
struct callout_rtn {
    unsigned *rw_size;
    void (*patcher)(paddr32_t paddr, uintptr_t vaddr, unsigned rtn_offset, unsigned rw_offset,
                    void *data, const struct callout_rtn * src);
    unsigned rtn_size;
    uint8_t rtn_code[1];
};
...
#endif
typedef unsigned (cache_rtn) (paddr32_t, unsigned, int, struct cacheattr_entry *,
...
int add_cache(int next, unsigned flags, unsigned line_size, unsigned num_lines,
              const struct callout_rtn *rtn);
int add_cache_ways(int next, unsigned flags, unsigned line_size, unsigned num_lines, unsigned ways,
                   const struct callout_rtn *rtn);
...
void hwi_add_rtc(const char *name, paddr_t base, unsigned reg_shift, unsigned len, int mmap,
                 int cent_reg);
...
int mdriver_add(char *name, int intr, int (*handler)(int state, void *data), paddr32_t data_paddr,
                unsigned data_size);
