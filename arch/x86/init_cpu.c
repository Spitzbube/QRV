...
//
void set_trap(unsigned trapnum, void (*func)())
{
...
unsigned vmm_fault_shim(PROCESS * prp, void *vaddr, unsigned flags)
{
...
static void calculate_combines(struct level_label *labels)
{
    unsigned i;
    int level;
    int primary;
...
static void add_code(const void *start, unsigned size)
{
    unsigned new;
...
static void gen_load(unsigned reg, uintptr_t value)
{
    uint8_t buff[5];
...
static void gen_burst_loads(unsigned genflags, unsigned level, int cascaded_eoi)
{
...
static void gen_queue_load(unsigned level, struct intrinfo_entry *iip)
{
    if (iip->id.size == 0 || iip->num_vectors == 1 || iip->cpu_intr_stride != 0) {
        ...
        gen_modrm(0x8d, REGNO_ESI, REGNO_NONE, REGNO_ESI, 1, (uintptr_t) & interrupt_level[level]);
    }
}

static void gen_cond_branch(int value, unsigned off)
{
    uint8_t buff[6];
    intptr_t disp;
...
static void gen_transfer(int call, uintptr_t dest)
{
    uint8_t buff[5];
    intptr_t disp;
...
static void gen_burst(struct __intrgen_data *burst, unsigned level, int cascaded_eoi)
{
...
static void gen_intr(struct level_label *labels, unsigned level, struct intrinfo_entry *iip)
{
    struct level_label *combine;
...
void cpu_interrupt_init(unsigned num_levels)
{
