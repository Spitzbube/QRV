 ...
 */
void wr_probe_1(void *p)
{
    volatile int *ptr = p;
    int tmp;

    if (actives[KERNCPU]->process->boundry_addr == VM_KERN_SPACE_BOUNDRY) {
        __asm__ __volatile__("ldr	%0, [%1];" "str	%0, [%1];":"=&r"(tmp)
                             :"r"(ptr)
            );
    } else {
        __asm__ __volatile__("ldrt	%0, [%1];" "strt	%0, [%1];":"=&r"(tmp)
                             :"r"(ptr)
            );
    }
}
 ...
 */
void rd_probe_num(const void *p, int num)
{
    const volatile int *ptr = p;
    int tmp;

    if (actives[KERNCPU]->process->boundry_addr == VM_KERN_SPACE_BOUNDRY) {
        __asm__ __volatile__("0:	ldr		%0, [%1], #4;"
                             "	subs	%2, %2, #1;" "	bne		0b;":"=&r"(tmp)
                             :"r"(ptr), "r"(num)
            );
    } else {
        __asm__ __volatile__("0:	ldrt	%0, [%1], #4;"
                             "	subs	%2, %2, #1;" "	bne		0b;":"=&r"(tmp)
                             :"r"(ptr), "r"(num)
            );
    }
 ...
 */
void wr_probe_num(void *p, int num)
{
    volatile int *ptr = p;
    int tmp;

    if (actives[KERNCPU]->process->boundry_addr == VM_KERN_SPACE_BOUNDRY) {
        __asm__ __volatile__("0:	ldr		%0, [%1];"
                             "	str		%0, [%1], #4;"
                             "	subs	%2, %2, #1;" "	bne		0b;":"=&r"(tmp)
                             :"r"(ptr), "r"(num)
            );
    } else {
        __asm__ __volatile__("0:	ldrt	%0, [%1];"
                             "	strt	%0, [%1], #4;"
                             "	subs	%2, %2, #1;" "	bne		0b;":"=&r"(tmp)
                             :"r"(ptr), "r"(num)
            );
