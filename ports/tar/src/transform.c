...
static struct replace_segm *add_segment(void)
{
...
static void add_literal_segment(char *str, char *end)
{
    size_t len = end - str;
    if (len) {
...
static void add_char_segment(int chr)
{
    struct replace_segm *segm = add_segment();
    segm->type = segm_literal;
...
static void add_backref_segment(size_t ref)
{
    struct replace_segm *segm = add_segment();
...
static void add_case_ctl_segment(enum case_ctl_type ctl)
{
    struct replace_segm *segm = add_segment();
    ...
    for (cur = beg = str; *cur;) {
        if (*cur == '\\') {
            ...
            case '9':
                ...
                if (n > regex.re_nsub)
                    USAGE_ERROR((0, 0,
                                 _("Invalid transform replacement: back reference out of range")));
...
bool _transform_name_to_obstack(char *input)
{
    regmatch_t *rmp;
    int rc;
    size_t nmatches = 0;
        ...
        if (rc == 0) {
            ...
            for (segm = repl_head; segm; segm = segm->next) {
                ...
                case segm_backref: /* Back-reference segment */
                    if (rmp[segm->v.ref].rm_so != -1 && rmp[segm->v.ref].rm_eo != -1) {
                        size_t size = rmp[segm->v.ref].rm_eo - rmp[segm->v.ref].rm_so;
...
bool transform_name_fp(char **pinput, char *(*fun)(char *, void *), void *dat)
{
    char *str;
