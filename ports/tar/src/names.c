...
int uname_to_uid(char const *uname, uid_t * uidp)
{
    ...
    if (cached_no_such_uname && strcmp(uname, cached_no_such_uname) == 0)
        return 0;

    if (!cached_uname || uname[0] != cached_uname[0]
...
static void check_name_alloc()
{
    if (names == allocated_names) {
        if (allocated_names == 0)
            allocated_names = 10;   /* Set initial allocation */
        name_array = x2nrealloc(name_array, &allocated_names, sizeof(name_array[0]));
    }
}
...
/* Get the next NELT_NAME element from name_array.  Result is in
   ...
   value. */
struct name_elt *name_next_elt(int change_dirs)
{
...
static void regex_usage_warning(const char *name)
{
    ...
    if (warn_regex_usage && is_pattern(name)) {
        ...
        WARN((0, 0, _("use --wildcards to enable pattern matching, or --no-wildcards to")));
        WARN((0, 0, _("suppress this warning.")));
    }
}
...
void names_notfound(void)
{
    ...
    for (cursor = namelist; cursor; cursor = cursor->next)
        if (!WASFOUND(cursor) && cursor->name[0]) {
            ...
            if (cursor->found_count == 0)
                ERROR((0, 0, _("%s: Not found in archive"), quotearg_colon(cursor->name)));
            else
    ...
    if (same_order_option) {
        ...
        while ((name = name_next(1)) != NULL) {
            ...
            ERROR((0, 0, _("%s: Not found in archive"), quotearg_colon(name)));
        }
    }
}

...
#define SUCCESSOR(name) ((name)->next)
    ...
    for (cursor = list, counter = first_length - 1; counter; cursor = SUCCESSOR(cursor), counter--)
        continue;
...
static void add_hierarchy_to_namelist(struct name *name, dev_t device)
{
    ...
    if (!buffer)
        ...
        size_t allocated_length = (name_length >= NAME_FIELD_SIZE
                                   ? name_length + NAME_FIELD_SIZE : NAME_FIELD_SIZE);
...
/* This is like name_match, except that
    ...
    returns TRUE. */
struct name *name_scan(const char *file_name)
{
    size_t length = strlen(file_name);
...
char *name_from_list(void)
{
    ...
    while (gnu_list_name && (gnu_list_name->found_count || gnu_list_name->name[0] == 0))
...
char *new_name(const char *file_name, const char *name)
{
