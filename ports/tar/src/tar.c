...
static void assert_format(unsigned fmt_mask)
{
    if ((FORMAT_MASK(archive_format) & fmt_mask) == 0)
        USAGE_ERROR((0, 0, _("GNU features wanted on incompatible archive format")));
}
...
void tar_list_quoting_styles(argp_fmtstream_t fs, char *prefix)
{
    int i;
...
void tar_set_quoting_style(char *arg)
{
    int i;
...
#define GRID 90
    ...
    { "exclude-tag", EXCLUDE_TAG_OPTION, N_("FILE"), 0,
     N_("exclude contents of directories containing FILE, except" " for FILE itself"), GRID + 1 },
...
#define GRID 100
    ...
    { "show-omitted-dirs", SHOW_OMITTED_DIRS_OPTION, 0, 0,
     N_("when listing or extracting, list each directory that does not match search criteria"),
     GRID + 1 },
...
#define GRID 110
    ...
    { NULL, 'o', 0, 0,
     N_("when creating, same as --old-archive; when extracting, same as --no-same-owner"),
     GRID + 1 },
#undef GRID
...
static void show_default_settings(FILE * fp) DECL_SHOW_DEFAULT_SETTINGS(fp, fprintf)
static void
show_default_settings_fs(argp_fmtstream_t fs) DECL_SHOW_DEFAULT_SETTINGS(fs, argp_fmtstream_printf)
static void set_subcommand_option(enum subcommand subcommand)
{
    if (subcommand_option != UNKNOWN_SUBCOMMAND && subcommand_option != subcommand)
        USAGE_ERROR((0, 0, _("You may not specify more than one `-Acdtrux' option")));
...

static RETSIGTYPE sigstat(int signo)
{
    compute_duration();
    print_total_stats();
...
void set_stat_signal(const char *name)
{
...
static void
get_date_or_file(struct tar_args *args, const char *option, const char *str, struct timespec *ts)
{
    if (FILE_SYSTEM_PREFIX_LEN(str) != 0 || ISSLASH(*str)
        || *str == '.') {
        struct stat st;
...
static void report_textual_dates(struct tar_args *args)
{
    ...
    for (p = args->textual_date; p;) {
        ...
        if (strcmp(p->date, treated_as) != 0)
            WARN((0, 0, _("Option %s: Treating date `%s' as %s"), p->option, p->date, treated_as));
        free(p);
        p = next;
    }
}
...
enum read_file_list_state { /* Result of reading file name from the list file */ file_list_success, /* OK, name read successfully */
...
/* Read from FP a sequence of characters up to FILENAME_TERMINATOR and put them
   into STK.
 */
static enum read_file_list_state read_name_from_file(FILE * fp, struct obstack *stk)
{
    int c;
    size_t counter = 0;
...
static void add_file_id(const char *filename)
{
    ...
    for (p = file_id_list; p; p = p->next)
        if (p->ino == st.st_ino && p->dev == st.st_dev) {
            FATAL_ERROR((0, 0, _("%s: file list already read"), quotearg_colon(filename)));
        }
    p = xmalloc(sizeof *p);
    p->next = file_id_list;
    p->ino = st.st_ino;
...
static void update_argv(const char *filename, struct argp_state *state)
{
    FILE *fp;
    size_t count = 0, i;
    char *start, *p;
...
static void tar_help(struct argp_state *state)
{
    argp_fmtstream_t fs;
    state->flags |= ARGP_NO_EXIT;
    argp_state_help(state, state->out_stream, ARGP_HELP_STD_HELP & ~ARGP_HELP_BUG_ADDR);
...
static error_t parse_opt(int key, char *arg, struct argp_state *state)
{
    ...
    case 'b':
        {
            uintmax_t u;
            if (!(xstrtoumax(arg, 0, 10, &u, "") == LONGINT_OK && u == (blocking_factor = u)
                  && 0 < blocking_factor && u == (record_size = u * BLOCKSIZE) / BLOCKSIZE))
                USAGE_ERROR((0, 0, "%s: %s", quotearg_colon(arg), _("Invalid blocking factor")));
        }
        break;
    ...
    case 'I':
        USAGE_ERROR((0, 0,
                     _("Warning: the -I option is not supported;" " perhaps you meant -j or -T?")));
        break;
    ...
    case 'L':
        {
            ...
            if (xstrtoumax(arg, 0, 10, &u, "") != LONGINT_OK)
                USAGE_ERROR((0, 0, "%s: %s", quotearg_colon(arg), _("Invalid tape length")));
    ...
    case 'X':
        if (add_exclude_file(add_exclude, excluded, arg, MAKE_EXCL_OPTIONS(args), '\n')
            != 0) {
            int e = errno;
    ...
    case ATIME_PRESERVE_OPTION:
        atime_preserve_option =
            (arg ? XARGMATCH("--atime-preserve", arg, atime_preserve_args, atime_preserve_types)
             : replace_atime_preserve);
        if (!O_NOATIME && atime_preserve_option == system_atime_preserve)
            FATAL_ERROR((0, 0,
                         _("--atime-preserve='system' is not supported" " on this platform")));
        break;
    ...
    case CHECKPOINT_OPTION:
        if (arg) {
            ...
            if (*p)
                FATAL_ERROR((0, 0, _("--checkpoint value is not an integer")));
        } else
    ...
    case EXCLUDE_CACHES_UNDER_OPTION:
        add_exclusion_tag("CACHEDIR.TAG", exclusion_tag_under, cachedir_file_p);
        break;

    case EXCLUDE_CACHES_ALL_OPTION:
        add_exclusion_tag("CACHEDIR.TAG", exclusion_tag_all, cachedir_file_p);
        break;
    ...
    case GROUP_OPTION:
        if (!(strlen(arg) < GNAME_FIELD_SIZE && gname_to_gid(arg, &group_option))) {
            uintmax_t g;
            if (xstrtoumax(arg, 0, 10, &g, "") == LONGINT_OK && g == (gid_t) g)
                group_option = g;
            else
                FATAL_ERROR((0, 0, "%s: %s", quotearg_colon(arg), _("%s: Invalid group")));
        }
        break;
    ...
    case OCCURRENCE_OPTION:
        if (!arg)
            ...
            if (xstrtoumax(arg, 0, 10, &u, "") == LONGINT_OK)
                ...
                FATAL_ERROR((0, 0, "%s: %s", quotearg_colon(arg), _("Invalid number")));
        }
        break;
    ...
    case OWNER_OPTION:
        if (!(strlen(arg) < UNAME_FIELD_SIZE && uname_to_uid(arg, &owner_option))) {
            uintmax_t u;
            if (xstrtoumax(arg, 0, 10, &u, "") == LONGINT_OK && u == (uid_t) u)
                owner_option = u;
            else
                FATAL_ERROR((0, 0, "%s: %s", quotearg_colon(arg), _("Invalid owner")));
        }
        break;
    ...
    case RECORD_SIZE_OPTION:
        {
            ...
            if (!(xstrtoumax(arg, 0, 10, &u, "") == LONGINT_OK && u == (size_t) u))
                USAGE_ERROR((0, 0, "%s: %s", quotearg_colon(arg), _("Invalid record size")));
            record_size = u;
            if (record_size % BLOCKSIZE != 0)
                USAGE_ERROR((0, 0, _("Record size must be a multiple of %d."), BLOCKSIZE));
    ...
    case STRIP_COMPONENTS_OPTION:
        {
            uintmax_t u;
            if (!(xstrtoumax(arg, 0, 10, &u, "") == LONGINT_OK && u == (size_t) u))
                USAGE_ERROR((0, 0, "%s: %s", quotearg_colon(arg), _("Invalid number of elements")));
    ...
    case '7':
        ...
        argp_error(state, _("Options `-[0-7][lmh]' not supported by *this* tar"));
...
static struct argp_option *find_argp_option(struct argp_option *options, int letter)
{
    for (;
         !(options->name == NULL
           && options->key == 0
           && options->arg == 0 && options->flags == 0 && options->doc == NULL); options++)
...
static void decode_options(int argc, char **argv)
{
            ...
            if (opt && opt->arg) {
                if (in < argv + argc)
                    *out++ = *in++;
                    ...
                    USAGE_ERROR((0, 0, _("Old option `%c' requires an argument."), *letter));
            }
        }
    ...
    if (argp_parse(&argp, argc, argv, ARGP_IN_ORDER | ARGP_NO_HELP, &index, &args))
    ...
    if (occurrence_option) {
        if (!args.input_files)
            USAGE_ERROR((0, 0, _("--occurrence is meaningless without a file list")));
        if (subcommand_option != DELETE_SUBCOMMAND
            && subcommand_option != DIFF_SUBCOMMAND
            && subcommand_option != EXTRACT_SUBCOMMAND && subcommand_option != LIST_SUBCOMMAND)
            USAGE_ERROR((0, 0, _("--occurrence cannot be used in the requested operation mode")));
    }
    ...
    if (archive_names > 1 && !multi_volume_option)
        USAGE_ERROR((0, 0, _("Multiple archive files require `-M' option")));

    if (listed_incremental_option && NEWER_OPTION_INITIALIZED(newer_mtime_option))
        USAGE_ERROR((0, 0, _("Cannot combine --listed-incremental with --newer")));

    if (volume_label_option) {
        if (archive_format == GNU_FORMAT || archive_format == OLDGNU_FORMAT) {
            size_t volume_label_max_len = (sizeof current_header->header.name - 1   /* for trailing '\0' */
                                           - (multi_volume_option ? (sizeof " Volume " - 1  /* for null at end of " Volume " */
    ...
    if (use_compress_program_option) {
        ...
        if (subcommand_option == UPDATE_SUBCOMMAND
            || subcommand_option == APPEND_SUBCOMMAND || subcommand_option == DELETE_SUBCOMMAND)
    ...
    if (args.pax_option
        ...
        && (subcommand_option != EXTRACT_SUBCOMMAND
            || subcommand_option != DIFF_SUBCOMMAND || subcommand_option != LIST_SUBCOMMAND))
    ...
    case CREATE_SUBCOMMAND:
        if (!args.input_files && !files_from_option)
            USAGE_ERROR((0, 0, _("Cowardly refusing to create an empty archive")));
        break;
    ...
    case DIFF_SUBCOMMAND:
        for (archive_name_cursor = archive_name_array;
             archive_name_cursor < archive_name_array + archive_names; archive_name_cursor++)
    ...
    case APPEND_SUBCOMMAND:
        for (archive_name_cursor = archive_name_array;
             archive_name_cursor < archive_name_array + archive_names; archive_name_cursor++)
            if (!strcmp(*archive_name_cursor, "-"))
                USAGE_ERROR((0, 0, _("Options `-Aru' are incompatible with `-f -'")));
...
int main(int argc, char **argv)
{
    ...
    switch (subcommand_option) {
    case UNKNOWN_SUBCOMMAND:
        USAGE_ERROR((0, 0, _("You must specify one of the `-Acdtrux' options")));
