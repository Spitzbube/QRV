...
void buffer_write_global_xheader()
{
...
enum compress_type check_compressed_archive()
{
...
{
    archive = rmtopen(archive_name_array[0], O_RDONLY | O_BINARY, MODE_RW, rsh_command_option);
    if (archive == -1)
        return archive;
...
static void print_stats(FILE * fp, const char *text, tarlong numbytes)
{
...
void print_total_stats()
{
    ...
    case APPEND_SUBCOMMAND:
        ...
        print_stats(stderr, _("Total bytes written"), prev_written + bytes_written);
        break;

    case DELETE_SUBCOMMAND:
        {
            char buf[UINTMAX_STRSIZE_BOUND];
            print_stats(stderr, _("Total bytes read"), records_read * record_size);
            print_stats(stderr, _("Total bytes written"), prev_written + bytes_written);
            fprintf(stderr, _("Total bytes deleted: %s\n"),
                    STRINGIFY_BIGINT((records_read - records_skipped)
                                     * record_size - (prev_written + bytes_written), buf));
        }
        break;
...
static void _open_archive(enum access_mode wanted_access)
{
    ...
    if (use_compress_program_option) {
        ...
        if (!index_file_name
            && wanted_access == ACCESS_WRITE && strcmp(archive_name_array[0], "-") == 0)
            stdlis = stderr;
        ...
        case ACCESS_READ:
            {
                ...
                if (type != ct_none)
                    FATAL_ERROR((0, 0,
                                 _("Archive is compressed. Use %s option"), compress_option(type)));
            }
            break;
        ...
        case ACCESS_UPDATE:
            archive = rmtopen(archive_name_array[0],
                              O_RDWR | O_CREAT | O_BINARY, MODE_RW, rsh_command_option);
...
static void do_checkpoint(bool write)
{
...
static void short_read(size_t status)
{
        ...
        if (!read_full_records) {
            ...
            FATAL_ERROR((0, 0,
                         ngettext("Unaligned block (%lu byte) in archive",
                                  "Unaligned block (%lu bytes) in archive", rest), rest));
        }
    ...
    if (!read_full_records && verbose_option > 1 && record_start_block == 0 && status != 0) {
        unsigned long rsize = (record_size - left) / BLOCKSIZE;
        WARN((0, 0, ngettext("Record size = %lu block", "Record size = %lu blocks", rsize), rsize));
    }
...
static void backspace_output(void)
{
...

    {
        ...
        if (rmtlseek(archive, position, SEEK_SET) != position) {
            ...
            WARN((0, 0, _("Cannot backspace archive file; it may be unreadable without -i")));

            /* Replace the first part of the record with NULs.  */

            if (record_start->buffer != output_start)
                memset(record_start->buffer, 0, output_start - record_start->buffer);
        }
    }
}
...
static void increase_volume_number()
{
    global_volno++;
    if (global_volno < 0)
...
void change_tape_menu(FILE * read_file)
{
    ...
    while (!stop) {
        ...
        if (getline(&input_buffer, &size, read_file) <= 0) {
            ...
            if (subcommand_option != EXTRACT_SUBCOMMAND
                && subcommand_option != LIST_SUBCOMMAND && subcommand_option != DIFF_SUBCOMMAND)
 ...
 y or newline  Continue operation\n"));
        ...
        case 'n':
...

            {
                ...
                for (name = input_buffer + 1; *name == ' ' || *name == '\t'; name++);
                ...
                *cursor = '\0';
                ...
                if (name[0]) {
                    ...
                    *archive_name_cursor = xstrdup(name);
                    ...
                    fprintf(stderr, "%s", _("File name not specified. Try again.\n"));
            }
            break;
...
*/
static bool new_volume(enum access_mode mode)
{
  ...
  tryagain:
    if (prompt) {
        ...
        if (info_script_option) {
            ...
            if (sys_exec_info_script(archive_name_cursor, global_volno + 1))
                FATAL_ERROR((0, 0, _("%s command failed"), quote(info_script_option)));
        } else
    ...
    if (strcmp(archive_name_cursor[0], "-") == 0) {
        ...
        archive = rmtopen(*archive_name_cursor, O_RDWR | O_CREAT, MODE_RW, rsh_command_option);
    else
        switch (mode) {
        case ACCESS_READ:
            archive = rmtopen(*archive_name_cursor, O_RDONLY, MODE_RW, rsh_command_option);
            break;

        case ACCESS_WRITE:
            if (backup_option)
                maybe_backup_file(*archive_name_cursor, 1);
            archive = rmtcreat(*archive_name_cursor, MODE_RW, rsh_command_option);
            break;
...
static bool read_header0(struct tar_stat_info *info)
{
    enum read_header rc;
...
bool try_new_volume()
{
    ...
    case GNUTYPE_MULTIVOL:
        ...
        continued_file_size = UINTMAX_FROM_HEADER(current_header->header.size);
        continued_file_offset = UINTMAX_FROM_HEADER(current_header->oldgnu_header.offset);
        break;
    ...
    if (real_s_name) {
        uintmax_t s;
        if (!continued_file_name || strcmp(continued_file_name, real_s_name)) {
            if ((archive_format == GNU_FORMAT || archive_format == OLDGNU_FORMAT)
                && strlen(real_s_name) >= NAME_FIELD_SIZE
                && strncmp(continued_file_name, real_s_name, NAME_FIELD_SIZE) == 0)
                WARN((0, 0,
                      _("%s is possibly continued on this volume: header contains truncated name"),
                      quote(real_s_name)));
            else {
                WARN((0, 0, _("%s is not continued on this volume"), quote(real_s_name)));
                return false;
            }
        }
...
/* Check the LABEL block against the volume label, seen as a globbing
   ...
   multi-volume mode.  */
static bool check_label_pattern(union block *label)
{
    char *string;
    bool result;
...
static void match_volume_label(void)
{
    ...
    if (!label)
        FATAL_ERROR((0, 0, _("Archive not labeled to match %s"), quote(volume_label_option)));
    if (!check_label_pattern(label))
        FATAL_ERROR((0, 0, _("Volume %s does not match %s"),
                     quote_n(0, label->header.name), quote_n(1, volume_label_option)));
}

/* Mark the archive with volume label STR. */
static void _write_volume_label(const char *str)
{
...
static void add_volume_label(void)
{
    ...
    char *s = xmalloc(strlen(volume_label_option) + sizeof VOL_SUFFIX + strlen(p) + 2);
    sprintf(s, "%s %s %s", volume_label_option, VOL_SUFFIX, p);
    _write_volume_label(s);
    free(s);
}

static void add_chunk_header()
{
    if (archive_format == POSIX_FORMAT) {
        off_t block_ordinal;
        union block *blk;
        ...
        st.orig_file_name = xheader_format_name(&st, "%d/GNUFileParts.%p/%f.%n", real_s_part_no);
...
static void gnu_add_multi_volume_header(void)
{
    int tmp;
    ...
    OFF_TO_CHARS(real_s_totsize - real_s_sizeleft, block->oldgnu_header.offset);
...
static void add_multi_volume_header(void)
{
    if (archive_format == POSIX_FORMAT) {
...
static void multi_volume_sync()
{
    if (multi_volume_option) {
        if (save_name) {
            assign_string(&real_s_name, safer_name_suffix(save_name, false, absolute_names_option));
...
static void _gnu_flush_read(void)
{
    ...
    for (;;) {
        ...
        if ((status == 0 || (status == SAFE_READ_ERROR && errno == ENOSPC))
            && multi_volume_option) {
