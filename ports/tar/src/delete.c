...
static void move_archive(off_t count)
{
...
#ifdef MTIOCTOP
    {
        ...
        if (count < 0
            ? (operation.mt_op = MTBSR, operation.mt_count = -count, operation.mt_count == -count)
            : (operation.mt_op = MTFSR, operation.mt_count = count, operation.mt_count == count)) {
            if (0 <= rmtioctl(archive, MTIOCTOP, (char *) &operation))
                return;

            if (errno == EIO && 0 <= rmtioctl(archive, MTIOCTOP, (char *) &operation))
                return;
        }
    }
#endif                          /* MTIOCTOP */

    {
        ...
        if (increment / count != record_size || (position < position0) != (increment < 0)
...
void delete_archive_members(void)
{
    ...
    if (logical_status == HEADER_SUCCESS || logical_status == HEADER_SUCCESS_EXTENDED) {
        ...
        while (1) {
            ...
            new_blocks++;
            blocks_to_keep = (current_stat_info.stat.st_size + BLOCKSIZE - 1) / BLOCKSIZE;
            set_next_block_after(current_header);
            if (new_blocks == blocking_factor)
                write_record(1);
