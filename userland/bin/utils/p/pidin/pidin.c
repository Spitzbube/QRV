...
void piddspinfo(int fd, char *pidname, pid_t pid, int flags, const char *fmt,
                const char *mem_format);
...
static struct shorthand {
    ...
    {
     "users", "%a %N %U %V %W %X %Y %Z" },  // users
    {
     "ttimes", "%a %b %N %J %t %y %z" },    // ttimes
    {
     "times", "%a %N %L %t %u %v %w %x" },  // times
    {
     "timers", "%a %b %N %R" }, // pidin timers
    {
     "threads", "%a %N %h %J %B" }, // pidin threads
    {
     "syspage", 0, "Page" },    // syspage
    {
     "signals", "%a %b %N %S %s" }, // signals
    {
     "session", "%L %a %P %e %N" }, // session
    {
     "sched", "%a %b %N %p %l %H %J",
      },                        // scheduling
    {
     "rmasks", "%a %b %N %i" }, // runmasks
    {
     "regs", "%a %b %N %r" },   // registers
    {
     "rc", 0, "rc" },           // remote connections
    {
     "pmem", "%a %b %N %p %J %c %d %m",
      },                        // memory (process only)
    {
     "net", 0, "net" },         // node info on network
    {
     ...
     "            %M @%> %? %< %=" },   // memory
    {
     ...
     "             %: @%> %; %< %=  %@" },  // mapinfo
    {
     "irqs", "%a %b %N %Q" },   // interrupts
    {
     "channels", "%a %b %N %[" },   // channels
    {
     "info", 0, "system" },     // flags
    {
     "flags", "%a %N %f" },     // flags
    {
     "fds", "%a %N %o" },       // fd
    {
     "family", "%a %N %L %P %e %G %C" },    // family
    {
     "extsched", NULL, "ExtSchedulers" },   // ext_schedulers
    {
     "environment", "%a %N %E" },   // environment
#ifndef NO_BACKTRACE_LIB
    {
     "backtrace", "%I %q" },
#endif
    {
     "arguments", "%a %A" },    // arguments
    {
     0, 0 }
};
...
int main(int argc, char *argv[])
{
    ...
    do {
        do {
            ...
            if (optind < argc && !(s = lookup_shorthand(argv[optind]))) {
...
int find_netdir(char *node, char *buf, size_t bufsize, int flag)
{
    ...
    if (*node == '/') {
        ...
        if ((len =
             netmgr_ndtostr(ND2S_DIR_SHOW | ND2S_NAME_HIDE, ND_LOCAL_NODE, buf, bufsize)) == -1)
            return -1;
        len--;
...
void dspsched(void)
{
    ...
    if (!SchedCtl(SCHED_QUERY_SCHED_EXT, &query, sizeof(query))) {
        ...
        case SCHED_EXT_APS:{
                ...
                if ((aps_pstats =
                     alloca(aps_info.num_partitions * sizeof(sched_aps_partition_stats))) == NULL)
                    error_exit(!0, "Unable to query APS statistics - %s\n", strerror(ENOMEM));

                aps_pstats =
                    (sched_aps_partition_stats *) calloc(aps_info.num_partitions,
                                                         sizeof(sched_aps_partition_stats));
                if (!aps_pstats)
                    error_exit(!0, "Unable to allocate memory\n", strerror(ENOMEM));

                APS_INIT_DATA(&aps_pstats[0]);
                aps_pstats->id = APS_SYSTEM_PARTITION_ID;
                if (SchedCtl
                    (SCHED_APS_PARTITION_STATS, aps_pstats,
                     aps_info.num_partitions * sizeof(sched_aps_partition_stats)))
                    error_exit(!0, "Unable to query APS statistics - %s\n", strerror(errno));
                round[0] = aps_info.windowsize_cycles / 2 / 100, round[1] =
                    aps_info.cycles_per_ms / 2;
                printf("APS scheduler");
                printf(", %d partitions", aps_info.num_partitions);
                printf(", window %d ms",
                       (int) (aps_info.windowsize_cycles / aps_info.cycles_per_ms));
                ...
                for (n = 0; n < aps_info.num_partitions; ++n) {
                    if (SchedCtl
                        (SCHED_APS_QUERY_PARTITION, (aps_pinfo.id = n, &aps_pinfo),
                         sizeof(aps_pinfo)))
                        error_exit(!0, "Unable to query APS partition - %s\n", strerror(errno));
                    sprintf(notify, (aps_pinfo.notify_pid != -1) ? "%d/%d" : "",
                            aps_pinfo.notify_pid, aps_pinfo.notify_tid);
                    printf("%-*.*s   %3d%% (%3d%%)   %3dms (%3dms)%c   %s\n",
                           APS_PARTITION_NAME_LENGTH, APS_PARTITION_NAME_LENGTH, aps_pinfo.name,
                           aps_pinfo.budget_percent,
                           (int) ((aps_pstats[n].run_time_cycles +
                                   round[0]) * 100 / aps_info.windowsize_cycles),
                           (int) (aps_pinfo.critical_budget_cycles / aps_info.cycles_per_ms),
                           (int) ((aps_pstats[n].critical_time_cycles +
                                   round[1]) / aps_info.cycles_per_ms),
                           (aps_pstats[n].
                            stats_flags & SCHED_APS_PSTATS_IS_BANKRUPT_NOW) ? '!' : ' ', notify);
                }
            } break;
        }
    }
    ...
    do {
        ...
        if (dent) {
            ...
            for (i = 1, cpu = _SYSPAGE_ENTRY(sysinfo, cpuinfo); i <= sysinfo->num_cpu; i++, cpu++) {
                ...
                snprintf(cpubuf, sizeof(cpubuf), "%s",
                         &_SYSPAGE_ENTRY(sysinfo, strings)->data[cpu->name]);
...
static int sortkey(const void *a, const void *b)
{
...
static int sortpid(const void *a, const void *b)
{
    const sortentry_t *as = (const sortentry_t *) a;
    const sortentry_t *bs = (const sortentry_t *) b;
...
static int getinfo(char *procname, struct shared_info *info_p)
{
    int ret, fd;
...
static int *getpidlist(char *nodepath, const char *fmt)
{
...
void dspinfo(pid_t pid, char *pidname, int flags, const char *fmt, const char *mi_fmt)
{
...
void piddspinfo(int fd, char *pidname, pid_t pid, int flags, const char *fmt, const char *mf)
{
    int tid, lasttid;
    const char *f;
    struct shared_info info;
...
static struct format *get_format(FILE * fp, const char **f)
{
    ...
    if (formats[c].letter != c) {
        error_exit(1, "Format letter '%c'(%#x) <=> '%c'(%#x) is invalid.\n", c, c,
                   formats[c].letter, formats[c].letter);
    }
    if (!formats[c].print)
...
static void validate_formats()
{
    ...
    if (formats['z'].letter != 'z') {
        error_exit(1, "Format table invalid: '%c'(%#x) <=> '%c'(%#x)\n", 'z', 'z',
                   formats['z'].letter, formats['z'].letter);
    }
    return;
}
...
void error_exit(int printmsg, const char *fmt, ...)
{
...
void warning_exit(int printmsg, int expectwarn, const char *fmt, ...)
{
    if (printmsg) {
        va_list arglist;
        va_start(arglist, fmt);
