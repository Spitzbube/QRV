...
static void real_print_char(int c)
{
    putchar(c);
}
...
void dump_asinfo(const char *parent_name, uint16_t owner, int level)
{
    ...
    for (i = 0; i < num; ++i) {
        if (owner == as->owner) {
            ksprintf(name, "%s%s%s", parent_name, parent_name[0] ? "/" : "",
                     __hwi_find_string(as->name));
            printf(" %4hx) %16llx-%-16llx o:%04hx a:%04hx p:%d n:%s\n", i * sizeof(*as), as->start,
                   as->end, as->owner, as->attr, as->priority, name);
...
void print_hwinfo()
{
    ...
    while (tag->prefix.size != 0) {
        ...
        kprintf("  %d) size:%d tag:%d(%s)", hwi_tag2off(tag), tag->prefix.size, tag->prefix.name,
                name);
        if (*name >= 'A' && *name <= 'Z') {
...
void print_qtime()
{
    ...
    kprintf("  boot:%l CPS:%l%l rate/scale:%d/-%d intr:%d\n",
            ...
            qtime->timer_rate, -(int) qtime->timer_scale, (int) qtime->intr);
}
...
void print_cpuinfo()
{
    ...
    for (i = 0; i < mysyspage->num_cpu; ++i) {
        kprintf("  %d) cpu:%l flags:%l speed:%l cache i/d:%d/%d name:%d\n",
                i,
                cpu[i].cpu,
                cpu[i].flags, cpu[i].speed, cpu[i].ins_cache, cpu[i].data_cache, cpu[i].name);
    }
}

void print_cacheattr()
{
    ...
    for (i = 0; i < num; ++i) {
        kprintf("  %d) flags:%b size:%w #lines:%w control:%l next:%d\n",
                i,
                cache[i].flags,
                cache[i].line_size, cache[i].num_lines, cache[i].control, cache[i].next);
    }
}
...
static int enable_print_syspage(const char *name)
{
    unsigned i;
    unsigned on_bit;
    unsigned off_mask;
...
void print_syspage(char *enables)
{
    unsigned i;
    unsigned flags;
    int have_enables = 0;
            ...
            if ((err = devctl(fd, DCMD_PROC_SYSINFO, ptr, len, NULL)) == EOK) {
                if ((err = devctl(fd, -1, NULL, 0, NULL)) != EENDIAN || !full) {
                    if (err == EENDIAN) {
                        ...
                        for (n = ptr->asinfo.entry_size / sizeof(struct asinfo_entry), i = 0; i < n;
                             ++i, ++as) {
