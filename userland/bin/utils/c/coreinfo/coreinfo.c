...
static void print_sysinfo(procfs_sysinfo * p, int xlat)
{
    ...
    if (verbose) {
        ...
        printf(" system_private:off/size=%d,%d", p->system_private.entry_off,
               p->system_private.entry_size);
        printf("\n");
        ...
        printf("\n");
        printf(" typed_strings:off/size=%d,%d", p->typed_strings.entry_off,
               p->typed_strings.entry_size);
    ...
    if (p->cpuinfo.entry_off + p->cpuinfo.entry_size <= p->total_size) {
        ...
        for (i = 0; strings && i < p->num_cpu; i++) {
            ...
            printf("  cpu %d cpu=%d name=%s speed=%d\n", i + 1, c->cpu,
                   strings ? strings + c->name : "", c->speed);
        ...
        case _DEBUG_WHY_TERMINATED:
          ...
          dsp_siginfo:
            ...
            if (SI_FROMKERNEL(&p->info)) {
                ...
                case SIGCHLD:
                    dsp_index(sigcld_code, sizeof sigcld_code / sizeof *sigcld_code,
                              p->info.si_code);
                    ...
                    break;
                case SIGSEGV:
                    dsp_index(sigsegv_code, sizeof sigsegv_code / sizeof *sigsegv_code,
                              p->info.si_code);
                    goto dsp_ref;
                case SIGBUS:
                    dsp_index(sigbus_code, sizeof sigbus_code / sizeof *sigbus_code,
                              p->info.si_code);
                    goto dsp_ref;
                case SIGTRAP:
                  dsp_ref:
                    ...
                    break;
                case SIGILL:
                    dsp_index(sigill_code, sizeof sigill_code / sizeof *sigill_code,
                              p->info.si_code);
                    goto dsp_adr;
                case SIGFPE:
                    dsp_index(sigfpe_code, sizeof sigfpe_code / sizeof *sigfpe_code,
                              p->info.si_code);
                  dsp_adr:
                ...
                printf(" from pid=%d uid=%d value=%d(%p)",
                       p->info.si_pid, p->info.si_uid, p->info.si_value.sival_int,
                       p->info.si_value.sival_ptr);
            }
            break;
