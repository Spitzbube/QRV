...
-c "cmdline" Take next argument as a command to execute.
    ...
    - v Verbose.Echo commands before executing. % -fesh % C - "Fat" Embedded shell(QNX)
    % C[-irv][script_file]
Options:
-c "cmdline" Take next argument as a command to execute.
    - i Enter interactive mode after running any script file.
    - r Run in restricted mode.
    - v Verbose.Echo commands before executing. % -uesh % C - micro - Embedded shell(QNX)
    % C
#endif
#include <dirent.h>
        ...
        // Filename expansion.
            ...
            while (dp = readdir(dirp)) {
                ...
                if (fnmatch(pat, dp->d_name, 1) == 0) {
                    ...
                    if ((n < MAX_TOKENS) && (s2 + tlen <= (tokenbuf + sizeof(tokenbuf)))) {
...
char *recall_line(int c, char *buf)
{

    if ((c == UP_KEY && recall_lineno < MAX_LINES - 1) || (c == DN_KEY && recall_lineno > 1))
...
int aliascmd(char *cmd, int query)
{
    int i;
    char *ap, *cp, *dp;
...
void print_els(char *fname, struct stat *sbuf)
{
...
#endif
               sbuf->st_mode & 0xfff, sbuf->st_uid, sbuf->st_gid, sbuf->st_size, fname);
    } else {
        char format[8];
...
int ecp(char *sname, char *dname, int verbose)
{
    FILE *src, *dst;
    int c;
    struct stat sbuf;
...
int localcmd(void)
{
    ...
    if (strcmp(argv[0], "kill") == 0)
        return argv[1] ? (kill(atoi(argv[1]), atoi(argv[1]) ? SIGTERM : 0) ==
                          -1 ? errno : EOK) : ESRCH;
...
#endif
                   argv[1], buff.f_blocks, used, buff.f_bfree, (used * 100) / buff.f_blocks,
                   buff.f_basetype);
            return (EOK);
        }
...
#ifndef EMBEDDED
    // Check for do_restricted executables
    if (do_restrict && strchr(argv[0], '/'))
        return (EPERM);
#endif
...
int main(int argc, char *argv[])
{
    ...
    if (login && (opt = stack("/etc/esh")) == 0)
        do_restrict = 0;
    else if (login && opt == -1 && do_restrict) {    /* stack() failed, and we're restricted */
...
#define ECHOCTL	0
            ...
            if (tcgetattr(fileno(infp), &tios) != -1) {
                tios.c_lflag |=
                    ECHO | ECHOE | ECHOK | ECHOKE | ECHONL | ICANON | IEXTEN | ISIG | ECHOCTL;
