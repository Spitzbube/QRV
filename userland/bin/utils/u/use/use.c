...
% C[-a][-d < directory >][-e][-f < filelist >][-i] files Options:
-a Extracts all usage information from the load module in its source form, suitable for piping
    into usemsg. - d Recursively display info for all
        files under < directory >.
            - e Only consider ELF files.
            - f Read list of files, one per line, from file < filelist >, displaying
            usage info for each
          . - i Displays information from load module(ELF only). - s Displays.ident strings(ELF only). - r Displays information about running process and libraries(native only).Where:
            files A list of executable load modules or shell scripts that contain
                usage messages(see printed documentation for use and usemsg for details)
                .
#endif
...
break_out(int sig_number)
{
    printf("break??\n");
    if (more_pid != -1)
        kill(more_pid, SIGQUIT);    /* kill child */
    exit(EXIT_FAILURE);
}
#endif
...
int map_fn(const char *filename, const struct stat *buf, int flags)
{
...
int add_solibs(procfs_info * i, int fd)
{
    int num, j, retval = 0;
...
#define TMPBUF 1024
    if (filelist) {
        ...
        if (!fp) {
            fprintf(stderr, "Unable to open %s for reading: %s", filelist, strerror(errno));
            return 1;
        }
...
void setup_paging()
{
    ...
    if (numlines && (getenv("TERM") != NULL)) {
        ...
        if (pipe(pipfd) != -1) {
            ...
            if ((more_pid =
                 qnx_spawn(0, NULL, 0, -1, -1, 0, "/bin/more", args, arge, iov, -1)) != -1) {
                fflush(stdout);
                if (dup2(pipfd[1], 1) != -1)
                    numlines = 0;
                if (close(0) == -1)
                    perror("close0");   /* we don't need stdin */
                if (close(pipfd[0]) == -1)
                    perror("close1");   /* we don't need the read end */
                if (close(pipfd[1]) == -1)
                    perror("close2");   /* we've dup2ed the write end to stdout */
            } else {
...
FILE *open_help(char *cmd, int filedes)
{
    int fds[2];
    FILE *fp;
    char *argv[4];
...
void print_data(FILE * fp, char *full_fname, long nbytes)
{
    ...
    for (i = IS_LANG(lines[0]) ? 1 : 0; i < nlines; ++i) {
        ...
        if (lines[i][0] == '\t')
            ...
            else if ((lines[i][1] == '1' || lines[i][1] == '2') && lines[i][2] == '>') {
                char buf[90];
                int pid;
                char *p;
                int pfiled[2];
...
void bad_lmf(char *fname, FILE * fp, int numlines)
{
    ...
    if (b) {
        ...
        while ((b = fgets(buf, sizeof(buf), fp))) {
            ...
            if (*b != '#')
                break;          /* end if we run out of comment */
            b++;                /* skip past '#'                */
            if (!strncmp(b, "endif", 5))
                break;          /* end on #endif                */
