...
#ifdef __USAGE
% C - POSIX pipe daemon % C[-1][-a < size >][-P][-s < size >][-d] & Options:
-1 Unblock select for writing
    when _PC_PIPE_BUF available(default 1 byte)
...
void update_pipe(pipe_t * p, int force)
{
    ...
    if (p->fd != -1 && (StrictUpdate || force)) {
        ...
        p->attr.flags &=
            ~(IOFUNC_ATTR_DIRTY_OWNER | IOFUNC_ATTR_DIRTY_MODE | IOFUNC_ATTR_DIRTY_TIME);
    }
}
    ...
    if ((p = malloc(sizeof(pipe_t))) != NULL) {
        if ((p->name = name) == NULL || (p->name = strdup(name)) != NULL) {
            if ((p->buffer = malloc(BufferSize)) != NULL) {
                ...
                p->fd = -1;
                p->waiting[IOFUNC_NOTIFY_INPUT] = p->waiting[IOFUNC_NOTIFY_OUTPUT] =
                    p->waiting[IOFUNC_NOTIFY_OBAND] = NULL;
                p->rd = p->wr = 0;
                iofunc_attr_lock(&p->attr);
                LOCK(head);
...
int acquire_pipe(pipe_t * head, char *name, pipe_t ** pipe)
{
    ...
    if ((fd =
         _connect(0, name, 0, O_ACCMODE, SH_DENYNO, _IO_CONNECT_COMBINE, !0, 0, _FTYPE_ANY,
                  _IO_CONNECT_EXTRA_NONE, sizeof(st.i), &st.i, sizeof(st.o), &st.o, NULL)) == -1) {
        error = errno;
    } else {
        LOCK(head);
        if ((p = lookup_pipe(head, st.o.st_dev, st.o.st_ino, 0)) != NULL) {
            ...
            *pipe = p, error = EOK;
            ...
            p->attr.mtime = st.o.st_mtime, p->attr.atime = st.o.st_atime, p->attr.ctime =
                st.o.st_ctime;
            p->attr.nlink = st.o.st_nlink, p->attr.rdev = st.o.st_rdev;
            p->attr.flags &=
                ~(IOFUNC_ATTR_DIRTY_MASK | IOFUNC_ATTR_MTIME | IOFUNC_ATTR_ATIME |
                  IOFUNC_ATTR_CTIME);
            p->fd = fd;
            *pipe = p, error = EOK;
        } else {
...
void check_writers(pipe_t * pipe)
{
    ...
    while ((room = BufferSize - pipe->attr.nbytes) != 0 && (b = *bb) != NULL) {
        if (b->size > AtomicSize || b->nbytes <= room) {
            if ((chunk = write_to_pipe(pipe, b->rcvid, b->nbytes, b->offset, NULL)) < 0
                && b->size != b->nbytes)
 ...
 */
blocked_t *block_client(blocked_t ** blocked, resmgr_context_t * ctp, int offset, size_t size,
                        size_t nbytes, int tail)
{
    blocked_t *b;
 ...
 */
int resmgr_pipe_unlink(resmgr_context_t * ctp, io_unlink_t * msg, RESMGR_HANDLE_T * handle,
                       void *extra)
{
    return (EACCES);
}
    ...
    if (!count) {
        ...
        if (!pipe->attr.wcount)
            ...
            error = EAGAIN;
                  ...
                  block_client(&pipe->waiting[IOFUNC_NOTIFY_INPUT], ctp, 0, nbytes, nbytes,
                               !0)) == NULL)
            error = ENOMEM;
        else
            error = _RESMGR_NOREPLY;
...
int resmgr_pipe_write(resmgr_context_t * ctp, io_write_t * msg, RESMGR_OCB_T * ocb)
{
    ...
    if (!count && nonblock) {
        ...
        if ((b =
             block_client(&pipe->waiting[IOFUNC_NOTIFY_OUTPUT], ctp, offset + count, nbytes,
                          nbytes - count, !0)) == NULL)
            error = ENOMEM;
        else
            error = _RESMGR_NOREPLY;
...
int resmgr_pipe_notify(resmgr_context_t * ctp, io_notify_t * msg, RESMGR_OCB_T * ocb)
{
    ...
    if (!S_ISFIFO(pipe->attr.mode)) {
        ...
        if (BufferSize - pipe->attr.nbytes >= NotifyCounts[IOFUNC_NOTIFY_OUTPUT]
            || !pipe->attr.rcount)
...
int resmgr_pipe_devctl(resmgr_context_t * ctp, io_devctl_t * msg, RESMGR_OCB_T * ocb)
{
    ...
    if (S_ISFIFO(pipe->attr.mode) && msg->i.dcmd == DCMD_CHR_ISCHARS) {
        ...
        msg->o.ret_val = EOK;
        *fionread =
            !(ocb->ioflag & _IO_FLAG_RD) ? 0 : (ctp->info.
                                                flags & _NTO_MI_ENDIAN_DIFF) ? ENDIAN_RET32(pipe->
                                                                                            attr.
                                                                                            nbytes)
            : pipe->attr.nbytes;
        ...
        msg->o.ret_val = EOK;
        error =
            (fstatvfs(pipe->fd, stvfs) != -1) ? _RESMGR_PTR(ctp, &msg->o,
                                                            sizeof(msg->o) +
                                                            sizeof(*stvfs)) : errno;
    } else {
...
int resmgr_fifo_open(resmgr_context_t * ctp, io_open_t * msg, RESMGR_HANDLE_T * handle, void *extra)
{
    ...
    if (msg->connect.file_type != _FTYPE_PIPE) {
        ...
        if ((error = iofunc_open(ctp, msg, &pipe->attr, NULL, NULL)) != EOK)
            release_pipe(pipe, error);
        else if ((msg->connect.ioflag & (_IO_FLAG_RD | _IO_FLAG_WR)) == _IO_FLAG_WR
                 && !pipe->attr.rcount && msg->connect.ioflag & O_NONBLOCK)
            release_pipe(pipe, error = ENXIO);
        else if ((error = iofunc_ocb_attach(ctp, msg, NULL, &pipe->attr, NULL)) != EOK)
            release_pipe(pipe, error);
        else if ((error = open_semantics(pipe, ctp, msg->connect.ioflag)) != EOK
                 && error != _RESMGR_NOREPLY)
...
int main(int argc, char *argv[])
{
    ...
    iofunc_attr_init(&Pipes.attr,
                     S_IFNAM | S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH, NULL,
                     NULL);
    Pipes.attr.mount = &Mount;
    Pipes.attr.flags |= IOFUNC_ATTR_SYNTHETIC;
    if ((Pipes.attr.rdev =
         resmgr_attach(Dispatch, &ResmgrAttr, Pipes.name =
                       PIPE_NAME, _FTYPE_PIPE, 0, &PipeConnectFuncs, &PipeIoFuncs, &Pipes)) == -1)
    ...
    if ((Fifos.attr.rdev =
         resmgr_attach(Dispatch, &ResmgrAttr, Fifos.name =
                       NULL, _FTYPE_PIPE,
                       _RESMGR_FLAG_DIR | _RESMGR_FLAG_AFTER | _RESMGR_FLAG_FTYPEONLY,
                       &FifoConnectFuncs, &PipeIoFuncs, &Fifos)) == -1)
