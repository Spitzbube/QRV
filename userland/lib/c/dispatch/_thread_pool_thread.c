...
static void _thread_pool_update(thread_pool_t * pool, int w_adj, int n_adj)
{
    ...
    // number of threads.
    while (add--) {
        if (pthread_create(0, pool->pool_attr.attr, _thread_pool_context_thread, pool) != EOK) {
            _mutex_lock(&(props->inline_lock));
            pool->created--;
            props->newthreads--;
            _mutex_unlock(&(props->inline_lock));
...
int _thread_pool_reserve(thread_pool_t * pool, int wait_count)
{
    ...
    pool->waiting++;
    _mutex_unlock(&(props->inline_lock));
    ret = pthread_create(&tid, pool->pool_attr.attr, _thread_pool_reserve_thread, pcp);
    if (ret != 0) {
        errno = ret;
        ret = -1;
        // free storage for pcp
