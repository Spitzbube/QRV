    ...
    for (i = 0; i < num; i++, phdr++) {
        if (phdr->p_type == PT_LOAD) {
            if (phdr->p_flags & PF_W) {
                ...
                obj->text_rel = obj->text_addr ? (obj->text_addr - phdr->p_vaddr) : 0;
                obj->text_addr = obj->text_addr ? obj->text_addr : phdr->p_vaddr;
 ...
 */
static const Elf32_Sym *lookup_global(const char *name, const list_head_t * this,
                                      const struct object *omit, int jumpslot, struct object **obj)
{
    struct objlist *ol;
...
static int load_elf32(struct object *obj, int fd)
{
    ...
    if ((read(fd, &ehdr, sizeof ehdr) != sizeof ehdr) || memcmp(ehdr.e_ident, ELFMAG, SELFMAG)
        || ehdr.e_type != ET_DYN) {
        ERROR_BAD_LIB;
        return -1;
    }
    ...
    // get a new base
    if ((new_base =
         mmap(base, size, PROT_NONE, MAP_ELF | MAP_ANON | MAP_LAZY | MAP_PRIVATE, NOFD,
              0)) == MAP_FAILED) {
        ERROR_NO_MEMORY;
        return -1;
    }
        ...
        for (n = 0, lseg = 0; n < ehdr.e_phnum; n++, phdr++) {
            if (phdr->p_type == PT_LOAD) {
                ...
                vaddr = mmap((void *) ((phdr->p_vaddr - (uintptr_t) base) + (uintptr_t) new_base),
                             ...
                             lseg
                             || obj->
                             flags & OBJFLAG_TEXTREL ? MAP_ELF | MAP_PRIVATE | MAP_NOSYNCFILE |
                             MAP_FIXED : MAP_ELF | MAP_SHARED | MAP_NOSYNCFILE | MAP_FIXED, fd,
                             phdr->p_offset);
                ...
                if (vaddr == MAP_FAILED) {  // OK, let's do a MAP_ANON...
                    vaddr = mmap((void *) ((phdr->p_vaddr - (uintptr_t) base) +
                                 ...
                                 MAP_ANON | MAP_ELF | MAP_PRIVATE, NOFD,
                                 (phdr->p_vaddr - (off_t) base) + (off_t) new_base);
                    ...
                    if ((vaddr == MAP_FAILED)
                        || (lseg
                            && (uintptr_t) vaddr !=
                            ((phdr->p_vaddr - (uintptr_t) base) + (uintptr_t) new_base))) {
                        if (lseg)
                    ...
                    if (phdr->p_filesz > 0x3ff000) {
                        ...
                        for (nbytes = 0; nbytes < phdr->p_filesz; nbytes += bytes) {
                            if ((bytes = readblock(fd, 1, phdr->p_offset + nbytes,
                                                   min(phdr->p_filesz - nbytes, 0x3ff000),
                                                   (void *) ((unsigned) vaddr + nbytes))) == -1
                                || bytes == 0) {
                    ...
                    if (mem_end > fil_end) {
                        ...
                        if (mmap((void *) fil_end, mem_end - fil_end, prot,
                                 MAP_ELF | MAP_FIXED | MAP_ANON | MAP_PRIVATE, NOFD,
                                 0) == MAP_FAILED) {
                ...
                if (!lseg) {
                    ...
                    new_base =
                        (void *) (((uintptr_t) vaddr - (uintptr_t) phdr->p_vaddr) +
                                  (uintptr_t) base);
...
static int find_file(const char *name, const char *libpath, const char *rpath,
                     const char **fullpath)
{
...
*/
static struct object *find_object_stat(const list_head_t * list, const char *name, struct stat *st,
                                       struct objlist **listentry)
{
    struct objlist *l;
    ...
    list_forward(list, l) {
        if (l->object->name &&
            ...
            (!st
             || (st->st_ino == l->object->sinfo.st_ino && st->st_dev == l->object->sinfo.st_dev))) {
            if (listentry) {
                *listentry = l;
            }
            return l->object;
...
static struct object *find_object(const list_head_t * list, const char *name,
                                  struct objlist **listentry)
{
    return find_object_stat(list, name, NULL, listentry);
}

static const Elf32_Sym *lookup_linkmap_scope(const char *name, struct object *target_obj,
                                             struct object *omit, struct object **robj)
{
    Link_map *cur_lm;
    struct objlist *ol;
    struct objlist *to;
...
static int init_array(const list_head_t * objs)
{
    ...
    list_backward(objs, o) {
        vector_decode(vec, sizeof vec / sizeof *vec, o->object->dynamic, 0);
        if (o->object->flags & (OBJFLAG_BEGANINITARRAY | OBJFLAG_INITARRAY)) {
            continue;
        }
...
static uintptr_t LDD_C_USEAGE_ATTRIBUTE ldd(void *frame)
{
    ...
    o->object->flags |=
        OBJFLAG_INIT | OBJFLAG_FINI | OBJFLAG_EXECUTABLE | OBJFLAG_INITARRAY | OBJFLAG_FINIARRAY;
    o->root = &objects;
    ...
    if (((unsigned) other.dynamic >= (other.text_addr + other.data_offset))
        && ((unsigned) other.dynamic < (other.text_addr + other.data_offset + other.data_size))) {
    ...
    if (ld_preload) {
        ...
        while (needed && *needed) {
            ...
            if (!(obj = find_object(&objects, name_only(needed), NULL))) {
                ...
                if (load_object(obj, needed, libpath, NULL, NULL, NULL)) {
...
#pragma disable_message(201);
                ...
                if (dyn->d_tag == DT_NEEDED) {
                    ...
                    if (!(obj = find_object(&_dl_all_objects, needed, NULL))) {
                        ...
                        if (load_object(obj, olist->object->strings +
                                        dyn->d_un.d_val, libpath, olist->object->rpath, NULL,
                                        NULL)) {
    ...
    list_forward(handle, olist) {
        if (olist->object->flags & OBJFLAG_TEXTREL) {
            /* previously mapped in as writeable, now make it read only again */
            mprotect((void *) olist->object->text_addr, olist->object->text_size,
                     PROT_READ | PROT_EXEC);
...
static struct object *find_calling_dso(void *calling_pc)
{
    struct objlist *o;
...
static Ldd_Eh_Node_t *ldd_add_event_handler(Ldd_Eh_t eh, void *eh_d_handle, unsigned flags)
{
    Ldd_Eh_Node_t *node;
    if (eh == NULL) {
        errno = EINVAL;
...
static void ldd_run_one_handler(struct object *obj, Ldd_Eh_t eh, void *eh_d_handle, unsigned flags)
{
    Ldd_Eh_Data_t ehd;
