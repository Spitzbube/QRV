...
static const struct _io_connect_entry _connect_proc_entry = {
    ND_LOCAL_NODE,
    PATHMGR_PID,
    PATHMGR_CHID,
    PATHMGR_HANDLE,
...
static int _connect_io(struct _connect_ctrl const *ctrl, int fd, const char *prefix,
                       unsigned prefix_len, const char *path, void *buffer,
                       const struct _io_connect_entry *entry)
{
    ...
    if (msg->extra_len) {
        ...
        if ((align =
             (_QNX_MSG_ALIGN -
              (offsetof(struct _io_connect, path) + msg->path_len)) & (_QNX_MSG_ALIGN - 1))) {
...
static int _connect_request(struct _connect_ctrl *ctrl, const char *prefix, unsigned prefix_len,
                            const char *path, unsigned path_skip,
                            const struct _io_connect_entry *entry, void *buffer, int chrootlen)
{
    register struct _io_connect *msg = ctrl->msg;
    register struct _io_connect_link_reply *reply = &(ctrl->reply->link);
    register int fd;
    ...
    if ((fd =
         ConnectAttach(entry->nd, entry->pid, entry->chid, ctrl->base, _NTO_COF_CLOEXEC)) != -1) {
        ...
        if ((ctrl->status =
             _connect_io(ctrl, fd, prefix, prefix_len, path + path_skip, buffer, entry)) == -1) {
            ConnectDetach_r(fd);
            return (-1);
        }
        ...
        if ((ctrl->status & (_IO_CONNECT_RET_FLAG | _IO_CONNECT_RET_TYPE_MASK)) ==
            ...
            if (sizeof(*reply) + reply->nentries * sizeof(*entry) + reply->path_len >=
                msg->reply_max) {
                errno = ENAMETOOLONG;
                return -1;
            }
            ...
            if (reply->nentries) {
                ...
                for (; cnt > 0; entry++, cnt--) {
                    ...
                    if ((msg->file_type == _FTYPE_ANY) ||
                        ...
                        fd = _connect_request(ctrl, 0, 0, save, entry->prefix_len + path_skip,
            ...
            if ((msg->subtype == _IO_CONNECT_OPEN &&
                ...
                ((msg->subtype == _IO_CONNECT_COMBINE ||
                  msg->subtype == _IO_CONNECT_COMBINE_CLOSE) && reply->path_len)) {
            ...
            if (ctrl->fds) {
                if (ctrl->fds_index >= ctrl->fds_len - 1) {
                    ...
                    if (!(ctrl->flags & FLAG_MALLOC_FDS) ||
                        !(tmp =
                          realloc(ctrl->fds,
                                  (ctrl->fds_len + FD_BUF_INCREMENT) * sizeof(*ctrl->fds)))) {
                        errno = ENOMEM;
                        return (-1);
                    }
                    ctrl->fds = tmp;
...
int _connect_ctrl(struct _connect_ctrl *ctrl, const char *path, unsigned response_len,
                  void *response)
{
...
#define MIN_FILLER_SIZE (sizeof(struct _io_connect_entry) * 3 * SYMLOOP_MAX + PATH_MAX + 1)
  ...
  onemoretime:
    ...
    if ((fd = _connect_request(ctrl, 0, 0, path, 0,
                               (ctrl->entry
                                && (ctrl->flags & FLAG_NO_RETRY)) ? ctrl->
                               entry : &_connect_proc_entry, buffer->filler, 0)) != -1) {
        errno = save_errno;
