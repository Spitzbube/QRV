...
static int find_historical(char *buffer, int buflen, int oflags, int mode, int flags, char *namebuf,
                           int nblen);
static char *append_path(char *buffer, int buflen, int copylen, const char *format,
                         const char *name, ...);
...
int cfgopen(const char *name, unsigned flags, const char *historical, char *namebuf, int nblen)
{
    ...
    if (flags & CFGFILE_CREAT || (flags & 0x3)) {
        ...
        bits = ((flags & CFGFILE_USER_NODE) ? 1 : 0) +
            ...
            ((flags & CFGFILE_NODE) ? 1 : 0) + ((flags & CFGFILE_GLOBAL) ? 1 : 0);
        if (bits > 1) {
            errno = EINVAL;
            return -1;
        }
    ...
    if (flags & CFGFILE_USER_NODE && cname && home) {
        if (append_path
            (buffer, sizeof(buffer), (namebuf != NULL) ? nblen : 0, "%s/%s/%s", name, home,
             _cfg_locations[DIR_USER], cname) != NULL)
    ...
    if (flags & CFGFILE_USER && fd == -1 && home) {
        if (append_path
            (buffer, sizeof(buffer), (namebuf != NULL) ? nblen : 0, "%s/%s", name, home,
             _cfg_locations[DIR_USER]) != NULL)
            fd = getfd(buffer, oflags, mode, flags, nblen ? namebuf : NULL);
        else
            errno = ENAMETOOLONG;
    }
    ...
    if (flags & CFGFILE_NODE && fd == -1 && cname) {
        if (append_path
            (buffer, sizeof(buffer), (namebuf != NULL) ? nblen : 0, "%s/%s", name,
             _cfg_locations[DIR_HOST], cname) != NULL)
            fd = getfd(buffer, oflags, mode, flags, nblen ? namebuf : NULL);
        else
            errno = ENAMETOOLONG;
    }
    ...
    if (historical && *historical && fd == -1) {
        if (append_path(buffer, sizeof(buffer), 0, NULL, historical) != NULL)
            fd = find_historical(buffer, sizeof(buffer), oflags, mode, flags & ~CFGFILE_CREAT,
                                 namebuf, nblen);
        else
            errno = ENAMETOOLONG;
    }
...
*/
FILE *fcfgopen(const char *filename, const char *mode, int location, const char *historical,
               char *namebuf, int nblen)
{
    int fd;
    int oflag;
    FILE *fp;
    ...
    for (p = strtok_r(buffer, ":", &last); p; p = strtok_r(NULL, ":", &last)) {
        if (namebuf != NULL && nblen != 0 && nblen <= strlen(p))
            errno = ENAMETOOLONG;
                  ...
                  getfd(p, oflags, mode, flags & ~CFGFILE_CREAT, nblen ? namebuf : NULL)) != -1)
            break;
    }
...
int main(int argc, char **argv)
{
    ...
    fd = cfgopen("/etc/testcfg", CFGFILE_USER_NODE | CFGFILE_USER | CFGFILE_NODE | CFGFILE_GLOBAL,
                 NULL, NULL, 0);
    if (fd != -1) {
    ...
    fd = cfgopen("/etc/testcfg", CFGFILE_USER_NODE | CFGFILE_USER | CFGFILE_NODE | CFGFILE_GLOBAL,
                 "/etc/testcfg.%H:/etc/host_cfg/%N/etc/testcfg", NULL, 0);
    if (fd != -1) {
        printf("USER_HOST/USER/HOST/GLOBAL/HIST+ passed \n");
    } else {
        printf("USER_HOST/USER/HOST/GLOBAL/HIST+ failed \n");
    ...
    fd = cfgopen("/etc/testcfg",
                 CFGFILE_USER | CFGFILE_NODE | CFGFILE_RDWR | CFGFILE_CREAT | CFGFILE_TRUNC, NULL,
                 NULL, 0);
    if (fd != -1) {
        printf("BAD \n");
    } else {
    ...
    fd = cfgopen("/etc/testcfg", CFGFILE_USER | CFGFILE_RDWR | CFGFILE_CREAT | CFGFILE_TRUNC, NULL,
                 NULL, 0);
    if (fd != -1) {
