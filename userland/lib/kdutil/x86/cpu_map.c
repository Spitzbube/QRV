...
static uintptr_t find_va(unsigned size)
{
    uintptr_t rover;
    uintptr_t next;
    unsigned pg_size;
    ...
    for (;;) {
        ...
        rover = next;
        if (wrapped && (rover >= va_rover))
            return 0;
    }
}


void *cpu_map(paddr64_t paddr, unsigned size, unsigned prot, int colour, unsigned *mapped_len)
{
    unsigned off;
    uintptr_t vaddr;
    unsigned cr0;
    ...
    if (mapped_len != NULL)
        *mapped_len = min(size, (__PAGESIZE * 2) - off);
...
unsigned cpu_vaddrinfo(void *p, paddr64_t * paddrp, unsigned *lenp)
{
