...
static int dev_random_read(resmgr_context_t * ctp, io_read_t * msg, RESMGR_OCB_T * ocb)
{
    int status;
    int bytes_used;
...
static void *resmgr_thread(void *p)
{
    ...
    if (dispatch == NULL) {
        slogf(_SLOGC_CHAR, _SLOG_CRITICAL,
              "random: Unable to create dispatch context: %s.", strerror(errno));
        return NULL;
    }
    ...
    id2 = resmgr_attach(dispatch, &res_attr, "/dev/urandom", _FTYPE_ANY,
                        0, &connect_funcs, &io_funcs, &io_attr);
    if (id1 == -1 || id2 == -1) {
        slogf(_SLOGC_CHAR, _SLOG_CRITICAL, "random: Unable to attach resmgr: %s.", strerror(errno));
        return NULL;
    }
    ...
    if (ctp == NULL) {
        slogf(_SLOGC_CHAR, _SLOG_CRITICAL,
              "random: Unable to allocate resmgr context: %s.", strerror(errno));
        return NULL;
    }
