...
static void *timer_thread(void *p)
{
    ...
    if (ret != 0) {
        slogf(_SLOGC_CHAR, _SLOG_CRITICAL, "random: Unable to get pool_id for timer thread.");
        return NULL;
    }
...
static void *syspoll_thread(void *p)
{
    ...
    ret = yarrow_add_source(Yarrow, &pool_id);
    if (ret != 0) {
        slogf(_SLOGC_CHAR, _SLOG_CRITICAL, "random: Unable to get pool_id for syspoll thread.");
        return NULL;
    }
    ...
    while (1) {
        ...
        while (dir_entry != NULL) {
            ...
            while (fd != -1) {
                ...
                ret = devctl(fd, DCMD_PROC_INFO, &debug_pid, sizeof(debug_pid), NULL);
                if (ret == -1)
                    break;

                SHA1Update(&context, (uint8_t *) & debug_pid, sizeof(debug_pid));

                for (i = 0; i < debug_pid.num_threads; i++) {
                    memset(&debug_tid, 0, sizeof(debug_tid));
                    debug_tid.tid = i + 1;
                    ret = devctl(fd, DCMD_PROC_TIDSTATUS, &debug_tid, sizeof(debug_tid), NULL);
                    if (ret == -1)
                        continue;

                    SHA1Update(&context, (uint8_t *) & debug_tid, sizeof(debug_tid));
                }
        ...
        if (Yarrow) {
            ...
            yarrow_input(Yarrow, digest, sizeof(digest), pool_id, sizeof(digest));
        }
    }
...
static void *interrupt_thread(void *p)
{
    ...
    if (ret != 0) {
        slogf(_SLOGC_CHAR, _SLOG_CRITICAL, "random: Unable to gain IO privs: %s", strerror(errno));
        return NULL;
    }

    chid = ChannelCreate(0);
    if (chid == -1) {
        slogf(_SLOGC_CHAR, _SLOG_CRITICAL, "random: ChannelCreate() failed: %s", strerror(errno));
        return NULL;
    }

    coid = ConnectAttach(0, 0, chid, _NTO_SIDE_CHANNEL, 0);
    if (coid == -1) {
        slogf(_SLOGC_CHAR, _SLOG_CRITICAL, "random: ConnectAttach() failed: %s", strerror(errno));
        return NULL;
    }
    ...
    if (intr_id == -1) {
        slogf(_SLOGC_CHAR, _SLOG_CRITICAL,
              "random: Unable to attach event to intr %d: %s", intr, strerror(errno));
        return NULL;
    }
