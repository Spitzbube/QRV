...
static int dummy_timer_reload(struct syspage_entry *sysp, struct qtime_entry *qtime)
{
    return 1;
}

static void dummy_display_char(struct syspage_entry *sysp, char c)
{
}

static int dummy_poll_key(struct syspage_entry *sysp)
{
    return -1;
}

static int dummy_break_detect(struct syspage_entry *sysp)
{
    return 0;
}
...
//      _exit(1);
    ...
    if (async_check) {
        ...
        if (old_timer_reload == NULL)
            old_timer_reload = dummy_timer_reload;
    ...
    if (debug_flag)
        kprintf("%s: starting next program at 0x%x\n", progname, start_vaddr);
...
char *find_typed_string(int index)
{
    ...
    for (;;) {
        ...
        if (type == index)
            return (&names[i + sizeof(uint32_t)]);
        if (type == _CS_NONE)
            return (NULL);
        i += sizeof(uint32_t);
        i += strlen(&names[i]) + 1;
...
int kdebug_timer_reload(struct syspage_entry *sysp, struct qtime_entry *qtime)
{
    int ret;
