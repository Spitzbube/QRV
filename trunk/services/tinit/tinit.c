...
#ifdef __USAGE
% C[-pt][env_var = value] ... Options:
-p Start Photon.If it fails or terminates start a console login.
    - t Do not mask suspend signal(SIGTSTP)
in spawned process. - f file Specify a ttys file(default /etc / config / ttys)
You can specify commands to run by placing them in / etc / config / ttys
    in the following format:device command terminal ie ser1 "/bin/ksh" qansi - m
#endif
