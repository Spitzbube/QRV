...
static int _elf_fill(Elf * elf, off_t offset, size_t bytes)
{
    ...
    while ((nbytes = write(elf->e_fd, &_elf_fill_char, sizeof _elf_fill_char)) > 0
           && nbytes < towrite) {
        towrite -= nbytes;
    }
...
#define BSWAP32( buf )		( * ( unsigned * )buf)
    ...
    if (_elf_read(elf, &arhdr, pos, sizeof arhdr) == sizeof arhdr) {
        pos += sizeof arhdr;
        if (!memcmp(arhdr.ar_fmag, ARFMAG, sizeof arhdr.ar_fmag) && arhdr.ar_name[0] == '/'
            && arhdr.ar_name[1] == ' ') {
            int size;
/* --- */
...
//printf( "\nelf_update()" ); flushall();
    ...
    if (cmd == ELF_C_NULL || cmd == ELF_C_WRITE) {
        ...
        if (do_layout) {
            ...
            align_offset = offset =
                elf->e_ehdrp->e_phoff + elf->e_ehdrp->e_phnum * sizeof *elf->e_phdrp;
...
/* --- */
        ...
        for (i = 0; i < elf->e_ehdrp->e_shnum; i++) {
            ...
            for (j = 0; j < scn->s_fragments; j++) {
                ...
                if ((dirty || (scn->s_data[j].d_flags & ELF_F_DIRTY))
                    && shdr->sh_type != SHT_NOBITS) {
                    if (data->d_off > d_off) {
                        _elf_fill(elf, offset + d_off, data->d_off - d_off);
                    }

                    _elf_write(elf, data->d_buf, elf->e_shdrp[i].sh_offset + data->d_off,
                               data->d_size);
                }
        ...
        if (dirty || (elf->e_shdr_flags & ELF_F_DIRTY)) {
            ...
            _elf_write(elf, (char *) elf->e_shdrp, offset,
                       elf->e_ehdrp->e_shnum * sizeof *elf->e_shdrp);
        }
...
//printf( "3" ); flushall();
        ...
        if (!skip
            && (memcmp(ar_hdr.ar_name, "#1/", 3)
                || (skip = i = strtol(&ar_hdr.ar_name[3], 0, 10)) == 0)) {
...
Elf_Scn *elf_nextscn(Elf * elf, Elf_Scn * scn)
{
    ...
    if (!scn) {
         ...
         */
        if (elf->e_shdrp == NULL) {
            if (elf->e_ehdrp->e_shnum == 0 || _elf32_get_sections(elf) == NULL)
                return NULL;
        }
#else
        if (!elf->e_shdrp && elf->e_ehdrp->e_shnum) {
        ...
        if (scn->s_fragments == 0) {
            ...
            if (ehdr) {
                if (scn->s_ndx < ehdr->e_shnum && ehdr->e_shentsize >= sizeof shdr &&
                    _elf_read(scn->s_elf, (char *) &shdr,
                              ehdr->e_shoff + scn->s_ndx * ehdr->e_shentsize,
                              sizeof shdr) == sizeof shdr && shdr.sh_size > 0) {
