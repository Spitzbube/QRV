...
static void append_file(char *file_name)
{
    ...
    if (fstat(handle, &stat_data) != 0)
        ...
        while (bytes_left > 0) {
            ...
            if (status == SAFE_READ_ERROR)
                read_fatal_details(file_name, stat_data.st_size - bytes_left, buffer_size);
            if (status == 0)
                FATAL_ERROR((0, 0,
                             ngettext("%s: File shrank by %s byte",
                                      "%s: File shrank by %s bytes",
                                      bytes_left),
                             quotearg_colon(file_name), STRINGIFY_BIGINT(bytes_left, buf)));
        ...
        case HEADER_SUCCESS:
            {
                ...
                if (subcommand_option == UPDATE_SUBCOMMAND
                    ...
                    if (deref_stat(dereference_option,
                        ...
                        && (tar_timespec_cmp(get_stat_mtime(&s), current_stat_info.mtime)
                            <= 0))
