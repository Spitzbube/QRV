         ...
         mode_t invert_permissions, enum permstatus permstatus, char typeflag)
{
    ...
    if (0 < same_permissions_option && permstatus != INTERDIR_PERMSTATUS) {
        ...
        if ((permstatus == ARCHIVED_PERMSTATUS
            ...
            && typeflag != DIRTYPE && typeflag != GNUTYPE_DUMPDIR)
            return;
...
{
    if (t.tv_sec <= 0)
        WARN((0, 0, _("%s: implausibly old time stamp %s"), file_name, tartime(t, true)));
         ...
         mode_t invert_permissions, enum permstatus permstatus, char typeflag)
{
    ...
    if (0 < same_owner_option && permstatus != INTERDIR_PERMSTATUS) {
        ...
        if (chown_result == 0) {
            ...
            if (cur_info && cur_info->st_mode & S_IXUGO && cur_info->st_mode & (S_ISUID | S_ISGID))
                cur_info = NULL;
        } else if (chown_result < 0)
            chown_error_details(file_name, st->stat.st_uid, st->stat.st_gid);
    }

    if (typeflag != SYMTYPE)
        set_mode(file_name, &st->stat, cur_info, invert_permissions, permstatus, typeflag);
}
               ...
               mode_t invert_permissions, enum permstatus permstatus)
{
    ...
    struct delayed_set_stat *data = xmalloc(offsetof(struct delayed_set_stat, file_name)
                                            + file_name_len + 1);
...
static void repair_delayed_set_stat(char const *dir, struct stat const *dir_stat_info)
{
    ...
    for (data = delayed_set_stat_head; data; data = data->next) {
        ...
        if (st.st_dev == dir_stat_info->st_dev && st.st_ino == dir_stat_info->st_ino) {
            ...
            data->invert_permissions = ((current_stat_info.stat.st_mode ^ st.st_mode)
    ...
    ERROR((0, 0, _("%s: Unexpected inconsistency when making directory"), quotearg_colon(dir)));
}
...
static int make_directories(char *file_name)
{
    ...
    for (cursor = cursor0; *cursor; cursor++) {
        ...
        if (cursor[-1] == '.' && (cursor == cursor0 + 1 || ISSLASH(cursor[-2])
                                  || (cursor[-2] == '.'
        ...
        *cursor = '\0';         /* truncate the name there */
        ...
        if (status == 0) {
            ...
            delay_set_stat(file_name, &current_stat_info, invert_permissions, INTERDIR_PERMSTATUS);
...
static bool file_newer_p(const char *file_name, struct tar_stat_info *tar_stat)
{
    struct stat st;
...
/* Fix the statuses of all directories whose statuses need fixing, and
   ...
   links are applied.  */
static void apply_nonancestor_delayed_set_stat(char const *file_name, bool after_links)
{
    size_t file_name_len = strlen(file_name);
...
static int extract_dir(char *file_name, int typeflag)
{
    int status;
    mode_t mode;
    int interdir_made = 0;
    ...
    if (status == 0
        || old_files_option == DEFAULT_OLD_FILES || old_files_option == OVERWRITE_OLD_FILES) {
        if (status == 0)
            delay_set_stat(file_name, &current_stat_info, ((mode ^ current_stat_info.stat.st_mode)
                                                           & MODE_RWX & ~current_umask),
                           ARCHIVED_PERMSTATUS);
        else                    /* For an already existing directory, invert_perms must be 0 */
            delay_set_stat(file_name, &current_stat_info, 0, UNKNOWN_PERMSTATUS);
    }
    return status;
}


static int open_output_file(char *file_name, int typeflag, mode_t mode)
{
    int fd;
    int openflag = (O_WRONLY | O_BINARY | O_CREAT
                    | (old_files_option == OVERWRITE_OLD_FILES ? O_TRUNC : O_EXCL));
...
static int extract_file(char *file_name, int typeflag)
{
    ...
    if (to_command_option)
        ...
        set_stat(file_name, &current_stat_info, NULL, invert_permissions,
                 (old_files_option == OVERWRITE_OLD_FILES ?
                  UNKNOWN_PERMSTATUS : ARCHIVED_PERMSTATUS), typeflag);
...
static int create_placeholder_file(char *file_name, bool is_symlink, int *interdir_made)
{
    int fd;
    struct stat st;
    ...
    if (fd < 0)
        ...
        struct delayed_link *p = xmalloc(offsetof(struct delayed_link, target)
...
static int extract_symlink(char *file_name, int typeflag)
{
#ifdef HAVE_SYMLINK
    int status;
    int interdir_made = 0;
    ...
    if (!absolute_names_option && (IS_ABSOLUTE_FILE_NAME(current_stat_info.link_name)
...
static int extract_node(char *file_name, int typeflag)
{
    ...
    do
        status = mknod(file_name, mode ^ invert_permissions, current_stat_info.stat.st_rdev);
...
static int extract_fifo(char *file_name, int typeflag)
{
    int status;
    int interdir_made = 0;
    mode_t mode = current_stat_info.stat.st_mode & ~current_umask;
...
static int extract_failure(char *file_name, int typeflag)
{
    return 1;
}
...
static int prepare_to_extract(char const *file_name, int typeflag, tar_extractor_t * fun)
{
    int rc = 1;
    ...
    case KEEP_NEWER_FILES:
        if (file_newer_p(file_name, &current_stat_info)) {
            WARN((0, 0, _("Current %s is newer or same age"), quote(file_name)));
            return 0;
        }
        break;
...
void extract_archive(void)
{
    ...
    if (!current_stat_info.file_name[0]
        || (interactive_option && !confirm("extract", current_stat_info.file_name))) {
        skip_member();
        return;
    }
    ...
    for (ds = delayed_link_head; ds;) {
        ...
        for (sources = ds->sources; sources; sources = sources->next) {
            ...
            if (lstat(source, &st) == 0
                && st.st_dev == ds->dev
                && st.st_ino == ds->ino && timespec_cmp(get_stat_mtime(&st), ds->mtime) == 0) {
...
bool rename_directory(char *src, char *dst)
{
    if (rename(src, dst)) {
        ...
        ERROR((0, e, _("Cannot rename %s to %s"), quote_n(0, src), quote_n(1, dst)));
        return false;
    }
    return true;
}
