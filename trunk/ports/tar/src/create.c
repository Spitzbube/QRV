...
static void exclusion_tag_warning(const char *dirname, const char *tagname, const char *message)
{
    if (verbose_option)
        WARN((0, 0,
              _("%s: contains a cache directory tag %s; %s"),
              quotearg_colon(dirname), quotearg_n(1, tagname), message));
}

static enum exclusion_tag_type check_exclusion_tags(char *dirname, const char **tag_file_name)
{
    static char *tagname;
    ...
    for (tag = exclusion_tags; tag; tag = tag->next) {
        ...
        if (access(tagname, F_OK) == 0 && (!tag->predicate || tag->predicate(tagname))) {
            if (tag_file_name)
...
{
    uintmax_t v = value;
    uintmax_t propagated_sign_bits = ((uintmax_t) - negative << (CHAR_BIT * sizeof v - LG_256));
    size_t i = size;
...
to_chars(int negative, uintmax_t value, size_t valsize,
         uintmax_t(*substitute) (int *), char *where, size_t size, const char *type);

static bool
to_chars_subst(int negative, int gnu_format, uintmax_t value, size_t valsize,
               uintmax_t(*substitute) (int *), char *where, size_t size, const char *type)
{
    uintmax_t maxval = (gnu_format ? MAX_VAL_WITH_DIGITS(size - 1, LG_256)
    ...
    if (substitute) {
        ...
        WARN((0, 0, _("value %s out of %s range %s..%s; substituting %s"),
              value_string, type, minval_string, maxval_string, sub_string));
...
bool gid_to_chars(gid_t v, char *p, size_t s)
{
    return to_chars(v < 0, (uintmax_t) v, sizeof v, gid_substitute, p, s, "gid_t");
}

bool major_to_chars(major_t v, char *p, size_t s)
{
    return to_chars(v < 0, (uintmax_t) v, sizeof v, 0, p, s, "major_t");
}

bool minor_to_chars(minor_t v, char *p, size_t s)
{
    return to_chars(v < 0, (uintmax_t) v, sizeof v, 0, p, s, "minor_t");
}

bool mode_to_chars(mode_t v, char *p, size_t s)
{
    ...
    if (S_ISUID == TSUID && S_ISGID == TSGID && S_ISVTX == TSVTX
        ...
        && archive_format != GNU_FORMAT && archive_format != OLDGNU_FORMAT) {
        negative = v < 0;
        u = v;
    } else {
        negative = 0;
...
bool size_to_chars(size_t v, char *p, size_t s)
{
...
bool uid_to_chars(uid_t v, char *p, size_t s)
{
    return to_chars(v < 0, (uintmax_t) v, sizeof v, uid_substitute, p, s, "uid_t");
}

bool uintmax_to_chars(uintmax_t v, char *p, size_t s)
{
    return to_chars(0, v, sizeof v, 0, p, s, "uintmax_t");
}

void string_to_chars(char const *str, char *p, size_t s)
{
...
bool file_dumpable_p(struct tar_stat_info *st)
{
    ...
    return !(st->archive_file_size == 0 && (st->stat.st_mode & MODE_R) == MODE_R);
}

...
static void write_gnu_long_link(struct tar_stat_info *st, const char *p, char type)
{
...
static size_t split_long_name(const char *name, size_t length)
{
    size_t i;
...
static union block *write_ustar_long_name(const char *name)
{
    ...
    if (length > PREFIX_FIELD_SIZE + NAME_FIELD_SIZE + 1) {
        ERROR((0, 0, _("%s: file name is too long (max %d); not dumped"),
               quotearg_colon(name), PREFIX_FIELD_SIZE + NAME_FIELD_SIZE + 1));
        return NULL;
    }
    ...
    if (i == 0 || length - i - 1 > NAME_FIELD_SIZE) {
        ERROR((0, 0,
               _("%s: file name is too long (cannot be split); not dumped"), quotearg_colon(name)));
        return NULL;
    }
...
static void write_long_link(struct tar_stat_info *st)
{
    ...
    case USTAR_FORMAT:
    case STAR_FORMAT:
        ERROR((0, 0, _("%s: link name is too long; not dumped"), quotearg_colon(st->link_name)));
        break;
...
union block *start_header(struct tar_stat_info *st)
{
...

    {
        ...
        if (archive_format == POSIX_FORMAT && MAX_OCTAL_VAL(header->header.uid) < uid) {
...

    {
        ...
        if (archive_format == POSIX_FORMAT && MAX_OCTAL_VAL(header->header.gid) < gid) {
...

    {
        ...
        if (archive_format == POSIX_FORMAT && MAX_OCTAL_VAL(header->header.size) < size) {
...

    {
        ...
        if (archive_format == POSIX_FORMAT) {
            if (MAX_OCTAL_VAL(header->header.mtime) < mtime.tv_sec || mtime.tv_nsec != 0)
    ...
    if (S_ISCHR(st->stat.st_mode)
        ...
        if (archive_format == POSIX_FORMAT && MAX_OCTAL_VAL(header->header.devmajor) < devmajor) {
        ...
        if (archive_format == POSIX_FORMAT && MAX_OCTAL_VAL(header->header.devminor) < devminor) {
    ...
    if (archive_format == V7_FORMAT || numeric_owner_option) {
        ...
        if (archive_format == POSIX_FORMAT
            && (strlen(st->uname) > UNAME_FIELD_SIZE || !string_ascii_p(st->uname)))
            xheader_store("uname", st, NULL);
        UNAME_TO_CHARS(st->uname, header->header.uname);

        if (archive_format == POSIX_FORMAT
            && (strlen(st->gname) > GNAME_FIELD_SIZE || !string_ascii_p(st->gname)))
...
void finish_header(struct tar_stat_info *st, union block *header, off_t block_ordinal)
{
    ...
    if (verbose_option
        ...
        && header->header.typeflag != XHDTYPE && header->header.typeflag != XGLTYPE) {
        ...
        if (count != bufsize) {
            ...
            WARN((0, 0,
                  ngettext("%s: File shrank by %s byte; padding with zeros",
                           ...
                           size_left),
                  quotearg_colon(st->orig_file_name), STRINGIFY_BIGINT(size_left, buf)));
...
static void dump_dir0(char *directory, struct tar_stat_info *st, int top_level, dev_t parent_device)
{
    ...
    if (one_file_system_option && !top_level && parent_device != st->stat.st_dev) {
        if (verbose_option)
            WARN((0, 0,
        ...
        case exclusion_tag_all:
            {
                ...
                for (entry = directory; (entry_len = strlen(entry)) != 0; entry += entry_len + 1) {
        ...
        case exclusion_tag_contents:
            exclusion_tag_warning(st->orig_file_name, tag_file_name, _("contents not dumped"));
...
static bool dump_dir(int fd, struct tar_stat_info *st, int top_level, dev_t parent_device)
{
...
{
    WARN((0, 0, _("%s: Unknown file type; file ignored"), quotearg_colon(p)));
    if (!ignore_failed_read_option)
...
static void file_count_links(struct tar_stat_info *st)
{
    if (st->stat.st_nlink > 1) {
        ...
        if (!((link_table || (link_table = hash_initialize(0, 0, hash_link, compare_links, 0)))
...
void check_links(void)
{
    ...
    for (lp = hash_get_first(link_table); lp; lp = hash_get_next(link_table, lp)) {
        if (lp->nlink) {
...
static void dump_file0(struct tar_stat_info *st, const char *p, int top_level, dev_t parent_device)
{
    union block *header;
    char type;
    off_t original_size;
    ...
    if (!(incremental_option && !is_individual_file(p))
        ...
        if (!incremental_option && verbose_option)
            WARN((0, 0, _("%s: file is unchanged; not dumped"), quotearg_colon(p)));
        return;
    }

    /* See if we are trying to dump the archive.  */
    if (sys_file_is_archive(st)) {
        WARN((0, 0, _("%s: file is the archive; not dumped"), quotearg_colon(p)));
        return;
    }
    ...
    if (is_dir || S_ISREG(st->stat.st_mode) || S_ISCTG(st->stat.st_mode)) {
        ...
        if (is_dir || file_dumpable_p(st)) {
            fd = open(p, (O_RDONLY | O_BINARY | (is_dir ? O_DIRECTORY | O_NONBLOCK : 0)
                          | (atime_preserve_option == system_atime_preserve ? O_NOATIME : 0)));
            if (fd < 0) {
                if (!top_level && errno == ENOENT)
                    WARN((0, 0, _("%s: File removed before we read it"), quotearg_colon(p)));
                else
                    open_diag(p);
                return;
            }
        ...
        if (ok) {
            /* If possible, reopen a directory if we are preserving
               ...
               _FIOSATIME.  */
            if (fd < 0 && is_dir && atime_preserve_option == replace_atime_preserve)
                fd = open(p, O_RDONLY | O_BINARY | O_DIRECTORY | O_NONBLOCK);

            if ((fd < 0 ? deref_stat(dereference_option, p, &final_stat)
                 : fstat(fd, &final_stat))
                != 0) {
                stat_diag(p);
                ok = false;
        ...
        if (ok) {
            if ((timespec_cmp(get_stat_ctime(&final_stat), original_ctime) != 0
                ...
                WARN((0, 0, _("%s: file changed as we read it"), quotearg_colon(p)));
...
void dump_file(const char *p, int top_level, dev_t parent_device)
{
