...
void report_difference(struct tar_stat_info *st, const char *fmt, ...)
{
    if (fmt) {
        va_list ap;
...
static int process_noop(size_t size __attribute__((unused)), char *data __attribute__((unused)))
{
    return 1;
}

static int process_rawdata(size_t bytes, char *buffer)
{
        ...
        if (status == SAFE_READ_ERROR) {
            ...
            report_difference(&current_stat_info,
                              ngettext("Could only read %lu of %lu byte",
                                       ...
                                       bytes), (unsigned long) status, (unsigned long) bytes);
        }
        return 0;
    }
...
static void read_and_process(struct tar_stat_info *st, int (*processor)(size_t, char *))
{
...
static void diff_dir(void)
{
    ...
    else if ((current_stat_info.stat.st_mode & MODE_ALL) != (stat_data.st_mode & MODE_ALL))
...
static void diff_file(void)
{
    ...
    if (!get_stat_data(file_name, &stat_data))
        ...
        if (tar_timespec_cmp(get_stat_mtime(&stat_data), current_stat_info.mtime))
        ...
        if (current_header->header.typeflag != GNUTYPE_SPARSE
            ...
            int atime_flag = (atime_preserve_option == system_atime_preserve ? O_NOATIME : 0);
...
static void diff_link(void)
{
    ...
    if (get_stat_data(current_stat_info.file_name, &file_data)
        ...
        report_difference(&current_stat_info,
                          _("Not linked to %s"), quote(current_stat_info.link_name));
}
...
static void diff_symlink(void)
{
    ...
    if (status < 0) {
        ...
        report_difference(&current_stat_info, NULL);
    } else if (status != len || strncmp(current_stat_info.link_name, linkbuf, len) != 0)
...
static void diff_special(void)
{
    ...
    if (current_header->header.typeflag == CHRTYPE ? !S_ISCHR(stat_data.st_mode)
        : current_header->header.typeflag == BLKTYPE ? !S_ISBLK(stat_data.st_mode)
    ...
    if ((current_header->header.typeflag == CHRTYPE || current_header->header.typeflag == BLKTYPE)
...
static int dumpdir_cmp(const char *a, const char *b)
{
    size_t len;
...
void diff_archive(void)
{
    ...
    default:
        ERROR((0, 0, _("%s: Unknown file type `%c', diffed as normal file"),
               quotearg_colon(current_stat_info.file_name), current_header->header.typeflag));
        /* Fall through.  */
...
void verify_volume(void)
{
    if (removed_prefixes_p()) {
        WARN((0, 0, _("Archive contains file names with leading prefixes removed.")));
        WARN((0, 0, _("Verification may fail to locate original files.")));
    }
...
#ifdef MTIOCTOP
    {
        ...
        if (status = rmtioctl(archive, MTIOCTOP, (char *) &operation), status < 0) {
            if (errno != EIO
                || (status = rmtioctl(archive, MTIOCTOP, (char *) &operation), status < 0)) {
#endif
        ...
        if (status == HEADER_FAILURE) {
            ...
            ERROR((0, 0,
                   ngettext("VERIFY FAILURE: %d invalid header detected",
                            "VERIFY FAILURE: %d invalid headers detected", counter), counter));
        }
