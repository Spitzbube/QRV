...
/* Allocate a copy of the string quoted as in C, and returns that.  If
   ...
   consumption.  */
char *quote_copy_string(const char *string)
{
...
static bool must_be_dot_or_slash(char const *file_name)
{
    ...
    if (ISSLASH(file_name[0])) {
        for (;;)
            if (ISSLASH(file_name[1]))
                file_name++;
            else if (file_name[1] == '.' && ISSLASH(file_name[2 + (file_name[2] == '.')]))
...
static int safer_rmdir(const char *file_name)
{
        ...
        case RECURSIVE_REMOVE_OPTION:
            {
                ...
                for (entry = directory; (entrylen = strlen(entry)) != 0; entry += entrylen + 1) {
...
bool maybe_backup_file(const char *file_name, bool this_is_the_archive)
{
    ...
    if (this_is_the_archive && (S_ISBLK(file_stat.st_mode) || S_ISCHR(file_stat.st_mode)))
        return true;
    ...
    if (rename(before_backup_name, after_backup_name) == 0) {
        if (verbose_option)
            fprintf(stdlis, _("Renaming %s to %s\n"),
                    quote_n(0, before_backup_name), quote_n(1, after_backup_name));
        return true;
    } else {
        /* The backup operation failed.  */
        int e = errno;
        ERROR((0, e, _("%s: Cannot rename to %s"),
               quotearg_colon(before_backup_name), quote_n(1, after_backup_name)));
        assign_string(&after_backup_name, 0);
        return false;
    }
}
...
void undo_last_backup(void)
{
    if (after_backup_name) {
        if (rename(after_backup_name, before_backup_name) != 0) {
            int e = errno;
            ERROR((0, e, _("%s: Cannot rename to %s"),
                   quotearg_colon(after_backup_name), quote_n(1, before_backup_name)));
        }
        if (verbose_option)
            fprintf(stdlis, _("Renaming %s back to %s\n"),
                    quote_n(0, after_backup_name), quote_n(1, before_backup_name));
        assign_string(&after_backup_name, 0);
    }
}
...
int set_file_atime(int fd, char const *file, struct timespec const timespec[2])
{
#ifdef _FIOSATIME
    if (0 <= fd) {
...
static inline void *ptr_align(void *ptr, size_t alignment)
{
    char *p0 = ptr;
