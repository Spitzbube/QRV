...
void sys_wait_for_child(pid_t child_pid)
{
}
...
void sys_child_open_for_uncompress(void)
{
    FATAL_ERROR((0, 0, _("Cannot use compressed or remote archives")));
}
...
void sys_detect_dev_null_output(void)
{
    ...
    dev_null_output = (strcmp(archive_name_array[0], dev_null) == 0 || (!_isrmt(archive)
                                                                        ...
                                                                        && stat(dev_null,
                                                                                &dev_null_stat) == 0
                                                                        && archive_stat.st_dev ==
                                                                        dev_null_stat.st_dev
                                                                        && archive_stat.st_ino ==
                                                                        dev_null_stat.st_ino));
}
...
void sys_drain_input_pipe(void)
{
    ...
    if (access_mode == ACCESS_READ && !_isrmt(archive)
...
void sys_wait_for_child(pid_t child_pid)
{
    if (child_pid) {
        ...
        if (WIFSIGNALED(wait_status))
            ERROR((0, 0, _("Child died with signal %d"), WTERMSIG(wait_status)));
        else if (WEXITSTATUS(wait_status) != 0)
            ERROR((0, 0, _("Child returned status %d"), WEXITSTATUS(wait_status)));
    }
}
...
{
    return stat_data->st_dev == link_data->st_dev && stat_data->st_ino == link_data->st_ino;
}
...
pid_t sys_child_open_for_compress(void)
{
    ...
    while (1) {
        ...
        for (length = 0, cursor = record_start->buffer;
             length < record_size; length += status, cursor += status) {
...
pid_t sys_child_open_for_uncompress(void)
{
    ...
    if (strcmp(archive_name_array[0], "-") != 0 && !_remdev(archive_name_array[0])
        && is_regular_file(archive_name_array[0])) {
...
static void dec_to_env(char *envar, uintmax_t num)
{
...
static void time_to_env(char *envar, struct timespec t)
{
...
static void oct_to_env(char *envar, unsigned long num)
{
...
static void str_to_env(char *envar, char const *str)
{
    if (str) {
...
static void chr_to_env(char *envar, char c)
{
    char buf[2];
    buf[0] = c;
    buf[1] = 0;
...
void sys_wait_command(void)
{
    ...
    if (WIFEXITED(status)) {
        ...
        ERROR((0, 0, _("%lu: Child terminated on unknown reason"), (unsigned long) pid));
