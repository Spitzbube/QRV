...
static void x_obstack_grow(struct xheader *xhdr, const char *ptr, size_t length)
{
...
static bool xheader_keyword_override_p(const char *keyword)
{
    struct keyword_list *kp;
...
static bool xheader_protected_pattern_p(const char *pattern)
{
    struct xhdr_tab const *p;
...
static bool xheader_protected_keyword_p(const char *keyword)
{
    struct xhdr_tab const *p;
...
decode_record(struct xheader *xhdr,
              char **ptr, void (*handler)(void *, char const *, char const *, size_t), void *data)
{
    char *start = *ptr;
    char *p = start;
    uintmax_t u;
    ...
    if (len_max < len) {
        ...
        ERROR((0, 0, _("Extended header length %*s is out of range"), len_len, p));
        return false;
    }
    ...
    for (p = len_lim; *p == ' ' || *p == '\t'; p++)
        continue;
    if (p == len_lim) {
        ERROR((0, 0, _("Malformed extended header: missing blank after length")));
        return false;
    }
...
static void decx(void *data, char const *keyword, char const *value, size_t size)
{
    ...
    if (t)
        ...
        WARN((0, 0, _("Ignoring unknown extended header keyword `%s'"), keyword));
}
...
static void
decg(void *data, char const *keyword, char const *value, size_t size __attribute__((unused)))
{
...
static void
xheader_print_n(struct xheader *xhdr, char const *keyword, char const *value, size_t vsize)
{
...
static bool decode_time(struct timespec *ts, char const *arg, char const *keyword)
{
    switch (_decode_time(ts, arg, keyword)) {
    case decode_time_success:
        return true;
    case decode_time_bad_header:
        ERROR((0, 0, _("Malformed extended header: invalid %s=%s"), keyword, arg));
        return false;
    case decode_time_range:
        out_of_range_header(keyword, arg, -(uintmax_t) TYPE_MINIMUM(time_t), TYPE_MAXIMUM(time_t));
        return false;
    }
    return true;
}
...
static bool decode_num(uintmax_t * num, char const *arg, uintmax_t maxval, char const *keyword)
{
    uintmax_t u;
    char *arg_lim;
...
dummy_coder(struct tar_stat_info const *st __attribute__((unused)),
            ...
            struct xheader *xhdr __attribute__((unused)), void const *data __attribute__((unused)))
{
}
...
dummy_decoder(struct tar_stat_info *st __attribute__((unused)),
              char const *keyword __attribute__((unused)),
              char const *arg __attribute__((unused)), size_t size __attribute__((unused)))
{
}
...
atime_decoder(struct tar_stat_info *st,
              char const *keyword, char const *arg, size_t size __attribute__((unused)))
{
    struct timespec ts;
...
gname_decoder(struct tar_stat_info *st,
              char const *keyword __attribute__((unused)),
              char const *arg, size_t size __attribute__((unused)))
{
                      ...
                      char const *keyword, char const *arg, size_t size __attribute__((unused)))
{
    ...
    if (decode_num(&u, arg, TYPE_MAXIMUM(off_t), keyword)) {
        if (st->sparse_map_avail < st->sparse_map_size)
            ...
            ERROR((0, 0, _("Malformed extended header: excess %s=%s"), "GNU.sparse.offset", arg));
    }
}
                        ...
                        char const *keyword, char const *arg, size_t size __attribute__((unused)))
{
    ...
    if (decode_num(&u, arg, SIZE_MAX, keyword)) {
        if (st->sparse_map_avail < st->sparse_map_size)
            ...
            ERROR((0, 0, _("Malformed extended header: excess %s=%s"), keyword, arg));
    }
}
                   ...
                   char const *keyword, char const *arg, size_t size __attribute__((unused)))
{
    ...
    if (!offset)
        ERROR((0, 0, _("Malformed extended header: invalid %s: odd number of values"), keyword));
}
...
dumpdir_decoder(struct tar_stat_info *st,
                char const *keyword __attribute__((unused)), char const *arg, size_t size)
{
...
static void
volume_size_decoder(struct tar_stat_info *st, char const *keyword, char const *arg, size_t size)
{
    uintmax_t u;
...
static void
volume_offset_decoder(struct tar_stat_info *st, char const *keyword, char const *arg, size_t size)
{
    uintmax_t u;
    if (decode_num(&u, arg, TYPE_MAXIMUM(uintmax_t), keyword))
...
static void
sparse_major_decoder(struct tar_stat_info *st, char const *keyword, char const *arg, size_t size)
{
    uintmax_t u;
...
static void
sparse_minor_decoder(struct tar_stat_info *st, char const *keyword, char const *arg, size_t size)
{
    uintmax_t u;
    if (decode_num(&u, arg, TYPE_MAXIMUM(unsigned), keyword))
         st->sparse_minor = u;
