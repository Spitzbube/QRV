...
static size_t hash_directory_name(void const *entry, size_t n_buckets)
{
...
static bool compare_directory_names(void const *entry1, void const *entry2)
{
...
static size_t hash_directory_meta(void const *entry, size_t n_buckets)
{
    struct directory const *directory = entry;
...
static bool compare_directory_meta(void const *entry1, void const *entry2)
{
    struct directory const *directory1 = entry1;
    struct directory const *directory2 = entry2;
...
static struct directory *make_directory(const char *name)
{
                                        ...
                                        dev_t dev, ino_t ino, bool nfs, bool found, char *contents)
{
    ...
    if (!((directory_table
           || (directory_table = hash_initialize(0, 0,
                                                 hash_directory_name, compare_directory_names, 0)))
...
static struct directory *procdir(char *name_buffer, struct stat *stat_data,
                                 dev_t device, enum children children, bool verbose)
{
    struct directory *directory;
    ...
    if ((directory = find_directory(name_buffer)) != NULL) {
            ...
            if (d) {
                if (verbose_option)
                    WARN((0, 0, _("%s: Directory has been renamed from %s"),
                          quotearg_colon(name_buffer), quote_n(1, d->name)));
                directory->orig = d;
                DIR_SET_FLAG(directory, DIRF_RENAMED);
                directory->children = CHANGED_CHILDREN;
            } else {
                if (verbose_option)
                    WARN((0, 0, _("%s: Directory has been renamed"), quotearg_colon(name_buffer)));
        ...
        directory = note_directory(name_buffer,
                                   ...
                                   stat_data->st_dev, stat_data->st_ino, nfs, true, NULL);
        ...
        if (d) {
            ...
            if (verbose)
                WARN((0, 0, _("%s: Directory is new"), quotearg_colon(name_buffer)));
            directory->children = (listed_incremental_option || (OLDER_STAT_TIME(*stat_data, m)
                                                                 || (after_date_option
...
const char *dumpdir_locate(const char *dump, const char *name)
{
    if (dump)
        while (*dump) {
...
static int compare_dirnames(const void *first, const void *second)
{
...
void makedumpdir(struct directory *directory, const char *dir)
{
    size_t i, dirsize,          /* Number of elements in DIR */
    ...
    if (directory->children == ALL_CHILDREN)
        ...
        dump = directory->orig->icontents ? directory->orig->icontents : directory->orig->contents;
    else
...
static char *scan_directory(char *dir_name, dev_t device)
{
    ...
    if (dirp && directory->children != NO_CHILDREN) {
        ...
        for (entry = directory->contents; (entrylen = strlen(entry)) != 0; entry += entrylen + 1) {
            ...
            if (excluded_name(name_buffer))
                *entry = 'N';
                ...
                if (S_ISDIR(stat_data.st_mode)) {
                    procdir(name_buffer, &stat_data, device, directory->children, verbose_option);
                    *entry = 'D';
                }
...
static void obstack_code_rename(struct obstack *stk, char *from, char *to)
{
    obstack_1grow(stk, 'R');
...
static bool rename_handler(void *data, void *proc_data)
{
...
static void read_incr_db_01(int version, const char *initbuf)
{
    int n;
    uintmax_t u;
    time_t sec;
    ...
    if (errno || buf == ebuf)
        ERROR((0, errno, "%s:%ld: %s",
               quotearg_colon(listed_incremental_option), lineno, _("Invalid time stamp")));
    else {
        sec = u;
    ...
    while (0 < (n = getline(&buf, &bufsize, listed_incremental_stream))) {
        ...
        if (errno || strp == ebuf || *ebuf != ' ') {
            ERROR((0, errno, "%s:%ld: %s",
                   quotearg_colon(listed_incremental_option), lineno, _("Invalid device number")));
            dev = (dev_t) - 1;
        } else
            dev = u;
        strp = ebuf;
        ...
        if (errno || strp == ebuf || *ebuf != ' ') {
            ERROR((0, errno, "%s:%ld: %s",
                   quotearg_colon(listed_incremental_option), lineno, _("Invalid inode number")));
            ino = (ino_t) - 1;
        } else
            ino = u;
        strp = ebuf;
...
static int read_obstack(FILE * fp, struct obstack *stk, size_t *pcount)
{
    int c;
    size_t i;
...
static void read_negative_num(FILE * fp, intmax_t min_val, intmax_t * pval)
{
    int c;
    size_t i;
...
static int read_unsigned_num(int c, FILE * fp, uintmax_t max_val, uintmax_t * pval)
{
    size_t i;
...
static int read_num(FILE * fp, uintmax_t max_val, uintmax_t * pval)
{
...
static void read_incr_db_2()
{
    ...
    FATAL_ERROR((0, 0, "%s: %s",
                 quotearg_colon(listed_incremental_option), _("Unexpected EOF in snapshot file")));
}
...
static bool write_directory_file_entry(void *entry, void *data)
{
    struct directory const *directory = entry;
    FILE *fp = data;
...
static bool dumpdir_ok(char *dumpdir)
{
    char *p;
    int has_tempdir = 0;
    int expect = 0;
    ...
    for (p = dumpdir; *p; p += strlen(p) + 1) {
        if (expect && *p != expect) {
            ERROR((0, 0, _("Malformed dumpdir: expected '%c' but found %#3o"), expect, *p));
            return false;
        }
        switch (*p) {
        case 'X':
            if (has_tempdir) {
                ERROR((0, 0, _("Malformed dumpdir: 'X' duplicated")));
                return false;
            } else
                has_tempdir = 1;
            break;
        ...
        case 'R':
            if (p[1] == 0) {
                if (!has_tempdir) {
                    ERROR((0, 0, _("Malformed dumpdir: empty name in 'R'")));
                    return false;
                } else
                    has_tempdir = 0;
            }
        ...
        case 'T':
            if (expect != 'T') {
                ERROR((0, 0, _("Malformed dumpdir: 'T' not preceeded by 'R'")));
                return false;
            }
            if (p[1] == 0 && !has_tempdir) {
                ERROR((0, 0, _("Malformed dumpdir: empty name in 'T'")));
                return false;
            }
            expect = 0;
            break;
    ...
    if (expect) {
        ERROR((0, 0, _("Malformed dumpdir: expected '%c' but found end of data"), expect));
        return false;
    }
...
static bool try_purge_directory(char const *directory_name)
{
    char *current_dir;
    char *cur, *arc, *p;
    char *temp_stub = NULL;
...
#define TEMP_DIR_TEMPLATE "tar.XXXXXX"
            ...
            temp_stub[len] = '/';
            memcpy(temp_stub + len + 1, TEMP_DIR_TEMPLATE, sizeof TEMP_DIR_TEMPLATE);
            if (!mkdtemp(temp_stub)) {
                ERROR((0, errno,
                       _("Cannot create temporary directory using template %s"), quote(temp_stub)));
                free(temp_stub);
                free(current_dir);
                return false;
            }
    ...
    for (cur = current_dir; *cur; cur += strlen(cur) + 1) {
        ...
        if (deref_stat(false, p, &st)) {
            if (errno != ENOENT) {  /* FIXME: Maybe keep a list of renamed
                ...
                stat_diag(p);
                WARN((0, 0, _("%s: Not purging directory: unable to stat"), quotearg_colon(p)));
            }
            continue;
        }
        ...
        if (!(entry = dumpdir_locate(current_stat_info.dumpdir, cur))
            ...
            if (one_file_system_option && st.st_dev != root_device) {
                WARN((0, 0,
                      _("%s: directory is on a different device: not purging"), quotearg_colon(p)));
                continue;
            }
