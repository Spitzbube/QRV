...
static bool dump_zeros(struct tar_sparse_file *file, off_t offset)
{
    ...
    while (file->offset < offset) {
        size_t size = (BLOCKSIZE < offset - file->offset ? BLOCKSIZE : offset - file->offset);
        ssize_t wrbytes;
...
static bool tar_sparse_member_p(struct tar_sparse_file *file)
{
...
static bool tar_sparse_init(struct tar_sparse_file *file)
{
...
static bool tar_sparse_done(struct tar_sparse_file *file)
{
    if (file->optab->done)
...
static bool tar_sparse_scan(struct tar_sparse_file *file, enum sparse_scan_state state, void *block)
{
...
static bool tar_sparse_dump_region(struct tar_sparse_file *file, size_t i)
{
...
static bool tar_sparse_extract_region(struct tar_sparse_file *file, size_t i)
{
...
static bool tar_sparse_dump_header(struct tar_sparse_file *file)
{
...
static bool tar_sparse_decode_header(struct tar_sparse_file *file)
{
...
static bool tar_sparse_fixup_header(struct tar_sparse_file *file)
{
...
static bool lseek_or_error(struct tar_sparse_file *file, off_t offset)
{
    if (file->seekable ? lseek(file->fd, offset, SEEK_SET) < 0 : !dump_zeros(file, offset)) {
...
static bool zero_block_p(char const *buffer, size_t size)
{
    while (size--)
        if (*buffer++)
            return false;
...
static bool sparse_scan_file(struct tar_sparse_file *file)
{
    ...
    while ((count = safe_read(fd, buffer, sizeof buffer)) != 0 && count != SAFE_READ_ERROR) {
        /* Analyze the block.  */
...
static bool sparse_dump_region(struct tar_sparse_file *file, size_t i)
{
    union block *blk;
        ...
        if (bytes_read == SAFE_READ_ERROR) {
            read_diag_details(file->stat_info->orig_file_name,
                              (file->stat_info->sparse_map[i].offset
                               + file->stat_info->sparse_map[i].numbytes - bytes_left), bufsize);
            return false;
        }
...
static bool sparse_extract_region(struct tar_sparse_file *file, size_t i)
{
    size_t write_size;
...
static bool check_sparse_region(struct tar_sparse_file *file, off_t beg, off_t end)
{
    ...
    while (beg < end) {
        ...
        if (!zero_block_p(diff_buffer, bytes_read)) {
            ...
            report_difference(file->stat_info,
                              _("File fragment at %s is not a hole"), offtostr(beg, begbuf));
            return false;
        }
...
static bool check_data_region(struct tar_sparse_file *file, size_t i)
{
    size_t size_left;
    ...
    while (size_left > 0) {
        ...
        if (bytes_read == SAFE_READ_ERROR) {
            read_diag_details(file->stat_info->orig_file_name,
                              (file->stat_info->sparse_map[i].offset
                               + file->stat_info->sparse_map[i].numbytes - size_left), rdsize);
            return false;
        }
        file->dumped_size += bytes_read;
...
bool sparse_diff_file(int fd, struct tar_stat_info *st)
{
    bool rc = true;
    struct tar_sparse_file file;
    size_t i;
    ...
    mv_begin(st);
    for (i = 0; rc && i < file.stat_info->sparse_map_avail; i++) {
        rc = check_sparse_region(&file, offset, file.stat_info->sparse_map[i].offset)
...
static bool oldgnu_sparse_member_p(struct tar_sparse_file *file __attribute__((unused)))
{
...
static bool oldgnu_fixup_header(struct tar_sparse_file *file)
{
    /* NOTE! st_size was initialized from the header
       which actually contains archived size. The following fixes it */
    file->stat_info->archive_file_size = file->stat_info->stat.st_size;
    file->stat_info->stat.st_size = OFF_FROM_HEADER(current_header->oldgnu_header.realsize);
    return true;
}

/* Convert old GNU format sparse data to internal representation */
static bool oldgnu_get_sparse_info(struct tar_sparse_file *file)
{
    size_t i;
    ...
    if (rc == add_fail) {
        ERROR((0, 0, _("%s: invalid sparse archive member"), file->stat_info->orig_file_name));
        return false;
    }
    return true;
}
                         ...
                         struct sparse *sp, size_t sparse_size)
{
    for (; *pindex < file->stat_info->sparse_map_avail
        ...
        OFF_TO_CHARS(file->stat_info->sparse_map[*pindex].offset, sp->offset);
        SIZE_TO_CHARS(file->stat_info->sparse_map[*pindex].numbytes, sp->numbytes);
    }
}

static bool oldgnu_dump_header(struct tar_sparse_file *file)
{
    ...
    i = 0;
    oldgnu_store_sparse_info(file, &i, blk->oldgnu_header.sp, SPARSES_IN_OLDGNU_HEADER);
    ...
    while (i < file->stat_info->sparse_map_avail) {
        ...
        oldgnu_store_sparse_info(file, &i, blk->sparse_header.sp, SPARSES_IN_SPARSE_HEADER);
...
static bool star_sparse_member_p(struct tar_sparse_file *file __attribute__((unused)))
{
    return current_header->header.typeflag == GNUTYPE_SPARSE;
}

static bool star_fixup_header(struct tar_sparse_file *file)
{
    /* NOTE! st_size was initialized from the header
       which actually contains archived size. The following fixes it */
    file->stat_info->archive_file_size = file->stat_info->stat.st_size;
    file->stat_info->stat.st_size = OFF_FROM_HEADER(current_header->star_in_header.realsize);
    return true;
}

/* Convert STAR format sparse data to internal representation */
static bool star_get_sparse_info(struct tar_sparse_file *file)
{
    size_t i;
    union block *h = current_header;
    int ext_p;
...
static bool pax_sparse_member_p(struct tar_sparse_file *file)
{
    return file->stat_info->sparse_map_avail > 0 || file->stat_info->sparse_major > 0;
}

static bool pax_dump_header_0(struct tar_sparse_file *file)
{
    off_t block_ordinal = current_block_ordinal();
    union block *blk;
    size_t i;
    ...
    if (xheader_keyword_deleted_p("GNU.sparse.map")
        ...
        for (i = 0; i < file->stat_info->sparse_map_avail; i++) {
            ...
            xheader_string_add(&file->stat_info->xhdr, umaxtostr(map[i].offset, nbuf));
            xheader_string_add(&file->stat_info->xhdr, ",");
            xheader_string_add(&file->stat_info->xhdr, umaxtostr(map[i].numbytes, nbuf));
        }
        if (!xheader_string_end(&file->stat_info->xhdr, "GNU.sparse.map")) {
...
static bool pax_dump_header_1(struct tar_sparse_file *file)
{
    off_t block_ordinal = current_block_ordinal();
    union block *blk;
    char *p, *q;
...
#define COPY_STRING(b,dst,src) do                \
 {                                               \
   ...
   while (*srcp)                                 \
    ...
    file->stat_info->file_name = xheader_format_name(file->stat_info, "%d/GNUSparseFile.%p/%f", 0);
...
static bool pax_dump_header(struct tar_sparse_file *file)
{
...
static bool decode_num(uintmax_t * num, char const *arg, uintmax_t maxval)
{
    uintmax_t u;
    char *arg_lim;
...
static bool pax_decode_header(struct tar_sparse_file *file)
{
